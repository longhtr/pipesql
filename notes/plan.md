...
## Next: characterize STRING grouping costs

The current representation reserves 65,536 bytes per declared STRING extremum
per group. This keeps replacement allocation-free and the spill fallback
bounded, but leaves substantial unused capacity for short strings. Qualification
should distinguish that explicit cost from allocator rounding and other owners.

Use a bounded investigation of at most 60 minutes before deciding whether a
representation change is justified. Reuse the current example/caller mechanics
and independent expected results. Compare few and many groups, short and
maximum-width values, and budgets that exercise memory and actual spill. Record
complete results, logical memory, temporary bytes, available allocator
observations, and query time on identified stock artifacts. Start with macOS;
available GNU/Linux can check portability without becoming a prerequisite.

The outcome is a reproducible workload, a concise explanation of the observed
cost, and a supported adopt/defer decision. Do not promise compact storage,
change semantics, or add another measurement framework. If the investigation
exposes a correctness or ownership defect, retain its counterexample and repair
that prerequisite before optional performance work. Keep qualification of
production memory limits separate from these workload observations.
...
| Current correctness checks; later storage-cost investigation | The same external-aggregation study varies group cardinality and measures operation beyond available memory. | Existing workloads check cardinality, budget pressure, and full-width text replay; exact admission checks cover the fallback minimum. Retain the fixed-slot cost described above. Before replacing it, compare few/many groups and short/long strings across budgets, checking complete results and recording logical memory, temporary bytes, and query time. The current checks do not establish performance. |
...
The STRING cost investigation informs the next representation decision; the
qualification gaps below remain separate obligations. Partitioned hash
aggregation, a shared buffer manager, parallel execution, and compact string
storage are possible alternatives, not scheduled
