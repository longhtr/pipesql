...
fn composition_query(db: &Database, derived: bool) -> Result<(), Error> {
    ...
    let queries = std::iter::once((sql, Value::Double(60.0)))
        ...
        .chain(derived.then_some((
            "FROM facts |> SELECT SAFE_DIVIDE(n, 0) AS ratio |> EXTEND COALESCE(ratio, 0) AS filled |> WHERE filled=0 |> AGGREGATE COUNT(NULLIF(filled, 0)) AS present |> SELECT COALESCE(present, DIV(1, 0)) AS present",
            Value::Int64(0),
        )))
        .chain(derived.then_some((
