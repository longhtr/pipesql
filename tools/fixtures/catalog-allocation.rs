...
const DERIVED_JOIN: &str = "FROM (FROM facts |> WHERE note IN ('first 雪','absent',NULL) |> SELECT amount) AS a |> JOIN (FROM facts |> SELECT amount) AS b ON a.amount = b.amount |> AGGREGATE COUNT(*) AS n";
