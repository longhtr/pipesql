...
pub(super) fn run(root: &Path, after: Option<usize>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = (|| -> Result<(), Error> {
        ...
        let query_result = (|| -> Result<(), Error> {
            ...
            let division = db.prepare("FROM facts |> SELECT ABS(-(measure/2)) AS ratio, CEIL(SIGN(MOD(amount, 2))) AS odd, COALESCE(DIV(amount, 1), DIV(1, 0)) AS exact |> WHERE ratio=1.75 AND odd=1 AND exact=9007199254740993 |> EXTEND SAFE_DIVIDE(ratio, 0) AS missing |> EXTEND FLOOR(COALESCE(missing, NULLIF(ratio, missing))) AS filled |> WHERE missing IS NOT DISTINCT FROM NULL AND filled=1 |> AGGREGATE COUNT(*) AS n")?;
