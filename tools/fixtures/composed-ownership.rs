...
pub(super) fn analytic_shapes(
    ...
    db.close()?;
    println!("analytic shapes passed: 19 cases; rows, attribution and release");
    Ok(())
}
...
/// Observe complete report calls without allocating caller-owned row storage
/// while the transient observer is armed. Literal answers remain independent of
/// both input transposition and production evaluation.
pub(super) fn event_report_history(
    ...
    for history in 0..3 {
        ...
        // Wrong attribution deliberately counts an extra resident owner. It must
        // make the same event-level check reject an otherwise healthy query.
        ...
        // Interrupt two executions of this same plan before the complete run.
        // The observer includes construction, terminal cleanup and the live error.
