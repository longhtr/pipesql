...
a zero denominator from overflow. `Error::ArithmeticDomain { operation, span }`
and its matching cause identify an argument outside the operation's domain,
such as a negative SQRT argument. The span remains valid as an offset after
the query and its source
