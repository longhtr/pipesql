// Check filesystem layouts against system headers independently of Rust bindings.
// Rust owns the runtime scenarios; this binding exposes only SDK facts.
...
#endif
size_t pipesql_header_mutex_size(void) { return sizeof(pthread_mutex_t); }
size_t pipesql_header_mutex_alignment(void) { return _Alignof(pthread_mutex_t); }
size_t pipesql_header_stack_minimum(void) { return PTHREAD_STACK_MIN; }
void pipesql_header_signatures(void) {
#if defined(__APPLE__)
    int (*bulk)(int, void *, void *, size_t, uint64_t) = getattrlistbulk;
    int (*canonical)(const char *, void *, void *, size_t, unsigned int) = getattrlist;
    (void)bulk;
    (void)canonical;
#else
