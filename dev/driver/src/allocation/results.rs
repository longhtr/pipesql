//! Check that finishing or failing a query releases its execution buffers.
//!
//! The input contains 0 through 255, followed by `i64::MAX`. Ordered projection
//! of `amount + 1` must return 256 correct rows before reporting overflow. Two
//! other queries read the values unchanged: one finishes, and one is cancelled
//! after returning rows. These cases exercise cleanup after useful output, when
//! a failure must not erase the rows already returned or retain execution storage.
//!
//! `allocation.rs` runs this probe against the public API. Its allocator
//! observer compares live buffers with reservations during each call. The checks
//! here require release in the terminal step, stable repeated terminal steps,
//! and an error that remains usable after the result and prepared plan are gone.
//! Controls change the expected prefix length or omit terminal observation;
//! each must fail its corresponding assertion.

use super::measurement::Live;
...
fn check(db: &Database, case: Case, control: Control) -> Result<(), Error> {
    ...
    // Formatting must use the supplied buffer without heap allocation, even
    // when that buffer is already full. The preparation events above prove that
    // the observer is working before we require zero events here.
    ...
    let terminal = loop {
        ...
        // Keep the early observations working in this negative control. They
        // must not substitute for observing frees in the terminal step itself.
    ...
    // Read the error and its source span after dropping the prepared query;
    // borrowing either from the query would leave a dangling diagnostic here.
    // Formatting uses a fixed buffer outside the event observation above.
