//! Check capacity admission before and during a real allocator call.
//!
//! A request above its capacity ceiling must fail before reaching the allocator.
//! An admitted request can still fail when the allocator refuses it. Literal cases
//! distinguish those paths from zero capacity and successful exact or spare
//! capacity; counters verify whether allocation was attempted.
//!
//! The probe includes the production resource helper directly to test this private
//! boundary without adding an engine hook. It holds a separate reservation until
//! after the returned vector is dropped, then checks that releasing the reservation
//! clears the charge. `allocation.rs` supplies the observing allocator
//! and invokes this module.
