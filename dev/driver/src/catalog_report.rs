//! Check which damaged files prevent opening a database or running its report.
//!
//! The supervisor changes copies of a database created here through the
//! public API. This driver requires distinct errors for an unsupported version,
//! corrupt metadata and a repair that cannot finish. A valid older root must not
//! let opening silently discard a newer committed catalog whose contents are bad.
//!
//! Column data is checked when a query reads it. Damaging `measurement` therefore
//! leaves the event report usable because it does not select that column. Selecting
//! `measurement` must fail before returning rows, release its resources, and leave
//! the same database handle able to run the report again. Literal groups below
//! check the report's answers independently of input construction.
...
// Groups are (year, label, row count, non-NULL amount count, sum).
const NEW: &[Group] = &[
...
pub(super) fn run(path: &Path, mode: &str) {
    ...
    if mode == "report-setup" {
        ...
        for (index, events) in event_data::EVENTS.chunks(8).enumerate() {
            let mut append = db
                ...
                .unwrap();
            if index == 1 {
                // Save the root after issuance but before publication. The Python
                // campaign pairs it with a newer root whose catalog it corrupts;
                // opening must refuse, rather than fall back to these older rows.
    ...
    // Opening validates metadata, but this report never reads measurement bytes.
    let query = db
    ...
    if mode == "report-measurement-reject" {
        ...
        assert_eq!(db.reserved_temp_bytes(), 0);
        drop(query);
        // The damaged column stays damaged. A query that does not read it must
        // still work after the failed query has released its resources.
        let query = db
            .prepare(include_str!("../../../examples/event_report.sql"))
            .unwrap();
        check_report(&db, &query);
