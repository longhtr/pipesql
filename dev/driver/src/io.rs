...
//! the public caller. The standard-library mode checks interception separately:
//! its exact-I/O methods retry interrupted calls, unlike the database boundary.
//!
...
//! when one was returned. Composed queries additionally compare literal answers,
//! check memory and temporary reservations after failure, then run again on both
//! the same handle and a reopened database. The development runner owns each directory.
