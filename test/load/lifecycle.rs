//! Check that a legacy load's commit can be identified after reopening.
//!
//! Load one row, copy the transaction token's bytes and close the database.
//! Reopening must resolve that token to the same durable commit. Tokens naming
//! a future attempt or another database must return NotFound.
//!
//! The separate token test checks byte validation without a database. A token
//! can have a valid shape even when no database has issued it; resolution is what
//! establishes its history. The load case also checks that its reservations are
