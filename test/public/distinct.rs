//! Test DISTINCT: keep one row for each distinct combination of input values.
//!
//! The expected rows come from a local set, independent of the engine's row
//! encoding and comparison code. For duplicate removal, NULLs match NULLs,
//! all NaNs match, and positive and negative zero match. The reference applies
//! those rules before comparing results, so it checks which rows are distinct
//! without requiring a particular NaN or zero representation in the output.
//!
//! Every input column affects duplicate removal, even when a later SELECT hides
//! it. The wide-row and overflow cases catch attempts to skip those columns.
//! Other cases check composition and the snapshot retained by a prepared query.
//! The small-stack variant measures its actual stack and requires a release build.

use super::*;

/// Give equivalent numbers the same test key; this discards their original bits.
...
fn public_distinct_scalar_equivalence_empty_input_and_composition() {
    ...
    let validity = [255, 1];
    // Derive the expected set once, then append the same rows in 40 batches.
    // Duplicates must collapse across batches as well as within each batch.
    ...
    // An unused expression after DISTINCT can be skipped. Moving it before
    // DISTINCT makes it part of the row comparison, so its overflow must fail.
    unordered(
        &db,
...
fn check_wide_distinct(small_stack: bool) {
    ...
    // The third row differs only in its last column. Comparing just c0 would
    // incorrectly collapse the two distinct rows into one output zero.
        ...
        thread
            .spawn_scoped(scope, || {
                ...
                support::query(
                    ...
                    support::integers(&[0, 0]),
                );
                // Each DISTINCT carries all 64 columns without naming them in SQL.
                // This reaches the 16-stage limit without exhausting the token limit.
...
fn public_distinct_keeps_pinned_inputs_through_append_reclaim_and_joins() {
    ...
    for sql in [
        ...
        unordered(&db, sql, support::integers(&[1, 2]));
    }
    drop(old);
    db.close().unwrap();
