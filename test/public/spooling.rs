//! Compare grouped results when they fit in memory and when they use disk.
//!
//! Input contains two copies of each of 1,024 groups. The input pattern also
//! determines expected rows: nullable values, floating-point bits, calendar
//! endpoints and long text. Output reorders the fields and repeats the text column
//! to expose mistakes in copying or locating saved values.
//!
//! Reopen the same data with two memory budgets. The larger budget must use no
//! temporary storage; the smaller must reserve some. Both runs must return the
//! same complete result and release their reservations. The measured memory limit
//! covers engine reservations, not total process memory.
...
fn mixed_typed_rows_survive_memory_and_disk_result_spooling() {
    ...
    // The integer identifies every group except the single NULL group at zero.
    // Other fields may tie; the two copies must still give COUNT(*) = 2 per group.
    ...
    db.close().unwrap();
    // Cover the retained sort-buffer allowance while leaving too little memory
    // for all 1,024 long-text groups. The assertion below requires actual spill.
    let spill_budget = if cfg!(target_os = "macos") {
        2_750_000
    } else {
        2_500_000
    };
    for (cap, require_disk) in [(8_000_000, false), (spill_budget, true)] {
