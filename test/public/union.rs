//! Test UNION ALL and UNION DISTINCT through the public query API.
//!
//! Both combine input columns by position and retain the left column names.
//! UNION ALL preserves repeated rows and can stop before reading a later input
//! when a LIMIT is satisfied. UNION DISTINCT compares complete rows, including
//! columns a later SELECT hides. Overflow cases make this difference observable
//! and check that failures point back to the responsible SQL expression.
//!
//! Literal typed results check copied values and snapshots across appends.
//! Separate cases request cancellation between successive step calls, then check
//! resource release. Larger text inputs exercise sorting and grouping with UNION:
//! they must use temporary storage with enough space and refuse a one-byte limit.
...
fn public_union_composes_with_spill_and_temporary_refusal() {
    ...
    db.close().unwrap();
    // The keys are unique. Concatenation returns two copies; grouping counts
    // both copies; duplicate removal returns one. Every case must actually spill.
...
fn public_union_cancellation_releases_each_scheduled_prefix() {
    ...
    for (sql, expected_rows) in [
        ...
        let mut prefixes = 0;
        // Repeat from the start, delaying cancellation by one step call each time.
        // Stop only when execution finishes before the cancellation request.
        for cancel_after in 0..1000 {
            ...
            if completed {
                ...
                // Completion can win a late cancellation. Try the next position
                // until there is also a complete run with no request.
...
fn public_union_distinct_keeps_typed_representatives_and_prepared_snapshot() {
    let directory = Directory::new();
    // Keep a sorted reader and append validation live together. On macOS the
    // reader also retains its large-buffer reuse allowance during publication.
    ...
    let db = Database::create_empty(
        &directory.database(),
        Config::new(memory, 2_000_000).unwrap(),
    )
    .unwrap();
    let cancel = CancellationToken::new();
    ...
    // Duplicate removal may keep either input zero or NaN. Check the original
    // bits first, then normalize them so the row comparison accepts either
    // choice. All other fields must match the literal expected values.
