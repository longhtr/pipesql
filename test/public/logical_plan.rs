//! Test the text returned by `PreparedQuery::logical_plan`.
//!
//! The report describes relations, column values and output positions. These are
//! different: selecting `total` twice produces two outputs that name the same
//! value, while a join introduces a second source. Literal reports check those
//! distinctions; smaller excerpts cover individual operators and expressions.
//!
//! Formatting must leave database reservations and the prepared snapshot intact.
//! A limited sink checks that formatting returns its first write error without
//! writing again. After an append, the old report and query result must remain
//! unchanged, while a newly prepared query sees the additional row.
...
fn logical_plan_propagates_sink_failure_and_keeps_the_prepared_snapshot() {
    ...
    // Allow each possible prefix of the report, including a partial number or
    // name. Only the full-length capacity may succeed; the sink catches retries
...
/// Count accepted bytes without storing text; panic if formatting ignores an error.
struct LimitedSink {
    remaining: usize,
    bytes: usize,
    failed: bool,
