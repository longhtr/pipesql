//! Test EXCEPT ALL and INTERSECT ALL, which count repeated rows.
//!
//! For each row, EXCEPT ALL subtracts the right count from the left, stopping at
//! zero. INTERSECT ALL keeps the smaller count. A local map computes these answers
//! from literal input rows without using the engine's row encoding or comparisons.
//!
//! Matching and copying are separate obligations: NaNs and signed zeros can match
//! despite having different bits, but each surviving occurrence must retain its
//! left-input bits. The typed test checks both. Preparing before an append also
//! checks that an existing query keeps the old counts while a new query sees the
//! added rows. Composition cases check column names, nullability and argument order.
...
fn public_multiset_preserves_typed_classes_original_bits_and_snapshots() {
    ...
    // Each row occurs twice on the left and once on the right at preparation.
    // These appends raise the right count to three only for new queries.
    append("right_rows", &right);
    append("right_rows", &right);
    // Check that surviving bits belong to the left input before normalizing
    // equivalent values. In particular, the right-only NaN must never escape.
    ...
    // Both left occurrences now survive. Checking their exact bits prevents
    // an implementation from emitting two copies of just one representative.
...
fn public_multiset_matches_independent_complete_row_counts() {
    ...
    for complete in [false, true] {
        ...
        // Count both the one-column and two-column projections. The duplicate
        // k = 1 rows also share a bucket, so they still contribute two copies.
        let counts = |limit| {
