//! Test wide schemas, repeated output columns and values retained for later use.
//!
//! A 64-column table cycles through all four stored types. Its values identify
//! their column and row, so a missing or misrouted late column changes the answer.
//! Queries scan, filter, compute and aggregate those values after reopening the
//! database. A smaller memory budget rejects the full scan but admits one column.
//!
//! Other cases distinguish visible output from the values execution must retain:
//! sorting needs an original value hidden by SET, and set operations can produce
//! 64 outputs by repeating 32 columns. Oversized or mismatched outputs must fail
//! preparation. The schema and set-width cases also run on a measured small stack;
//! those variants require release builds.
...
fn scan_memory_upper_bound_admits_a_full_text_schema() {
    ...
    // Measure the database and prepared-query reservations first. Reopening
    // leaves exactly the advertised execution allowance for this empty scan.
...
fn ordering_retains_original_values_beyond_visible_row_width() {
    ...
    append.commit(&cancel).unwrap();
    // SET replaces visible c0, but w.c0 still names its original value. Sorting
    // must carry that extra value alongside the 64 visible keys for the SELECT.
    let sql = format!(
        "FROM wide AS w |> SET c0=c0+1 |> ORDER BY {} |> SELECT w.c0",
        names.join(", ")
    );
    support::query(&db, &sql, support::integers(&[1, 2]));
}
...
fn check_set_width() {
    ...
    let baseline = db.reserved_memory_bytes();
    // Each input has 32 columns; repeating the selected names produces the
    // maximum 64 output positions without adding new source values.
