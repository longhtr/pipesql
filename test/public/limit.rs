//! Test which rows LIMIT and OFFSET consume, return or leave unevaluated.
//!
//! Expected prefixes come from slicing a literal ordered sequence. Moving LIMIT
//! before or after a filter, aggregate or join changes the result; explicit
//! answers check those differences. Invalid bounds must fail preparation and
//! release its memory reservation.
//!
//! LIMIT 0 OFFSET 0 need not read its input. A positive OFFSET still consumes
//! rows, even with LIMIT 0, and can therefore encounter an arithmetic error.
//! Cancellation and dropping a result after its first row check early cleanup.
...
fn public_limit_preserves_demanded_failures_and_releases_early_owners() {
    ...
    for sql in [
        ...
        for cancel_after in [0, 3, 12, usize::MAX] {
            ...
            assert!(terminal);
            // Stop after the first row or cancellation error. Dropping the
            // result must release its memory and temporary files without a final poll.
            drop(result);
            drop(query);
            assert_eq!(db.reserved_memory_bytes(), baseline);
            assert_eq!(db.reserved_temp_bytes(), 0);
