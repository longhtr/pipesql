//! Test numeric NULLIF results and the order in which its arguments are evaluated.
//!
//! NULLIF returns NULL when its converted arguments compare equal; otherwise it
//! returns the first argument. Literal cases use this to remove sentinel values
//! before aggregation and combine it with COALESCE, joins and set operations.
//! Mixed numeric types check conversion before equality, including rounding near
//! the largest consecutive integers representable as DOUBLE.
//!
//! A NULL first argument does not skip the second argument. Error cases check
//! that rule and the failing expression's source span. Outer query demand can
//! still skip the whole call. Numeric cases check NaN and signed-zero bits when
//! returned; stored-value cases also check pinned views and reopening.
...
fn public_nullif_preserves_argument_error_order_and_outer_demand() {
    ...
    // On a non-NULL key the first argument divides by zero. On a NULL key it
    // produces NULL, so evaluation reaches the second argument's overflow.
...
fn public_nullif_retains_stored_double_bits_and_prepared_snapshots() {
    ...
    // Comparing a value with itself removes every non-NULL value except NaN.
    // Each NaN must retain its original sign and payload bits.
