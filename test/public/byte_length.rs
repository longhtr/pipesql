//! Test BYTE_LENGTH on stored text and decoded SQL literals.
//!
//! Expected counts are written out for empty text, control bytes, accented text
//! and the maximum stored value. NULL stays NULL. Queries compare results before
//! and after sorting, through a derived table, and after reopening the database.
//!
//! Composition cases distinguish an original text column from its replacement
//! length and carry lengths through joins, grouping and set operations. This
//! module also tests invalid arguments, error spans, cancellation and early drop
//! for both length functions. `char_length` owns Unicode scalar-count results.
...
fn byte_length_literals_own_decoded_values_and_compose_with_constants() {
    ...
    // The SQL string dies before execution. The prepared query must retain the
    // decoded literals' lengths without borrowing their source spelling.
    let prepared = {
        let sql = String::from(
...
fn string_length_rejects_unsupported_forms_with_owned_spans() {
    ...
    for function in ["BYTE_LENGTH", "CHAR_LENGTH"] {
        for expression in [
            ...
            // LIMIT 0 skips execution, but cannot excuse invalid syntax or types.
            let sql =
