//! Test EXCEPT DISTINCT: remove rows found on the right and discard duplicates.
//!
//! Columns match by position, not by name, and the complete row determines a
//! match. The result keeps the left column names and nullability. Literal answers
//! cover these rules in composed queries; a standard-library set independently
//! computes differences for several pairs of filtered inputs.
//!
//! Typed cases check NULL matching and removal of equivalent NaNs and signed
//! zeros. Snapshot cases prepare before appending: an existing query must retain
//! its old result, while a newly prepared query sees the added right-hand rows.
//! The shared input-error checks for set operations live in `intersect.rs`.
...
fn public_except_preserves_typed_values_and_both_snapshot_inputs() {
    ...
    // Add a right-hand match for the snow row. Only a new query may remove it.
    append(
        "right_rows",
        &[
            ColumnInput {
...
fn public_except_compares_typed_nulls_in_every_scalar_position() {
    ...
    // The left contains one all-NULL row and one value row. Both right rows
    // are all NULL, regardless of the values hidden by their validity bits.
