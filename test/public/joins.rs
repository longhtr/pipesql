//! Test inner and left joins through the public query API.
//!
//! Small literal examples show how duplicate keys multiply matching pairs and
//! how a left join preserves an unmatched row with NULL right-hand values.
//! Projections, aggregates and nested joins check that each input keeps its own
//! column identities. A separate typed reference enumerates matching pairs using
//! Rust equality, then compares every returned field, including original DOUBLE
//! bits and maximum-length text. Successful runs must use temporary storage;
//! a smaller memory budget must refuse execution.
//!
//! A reader paused during appends to both tables must finish with its original
//! snapshot. Queries prepared between and after the appends check each later view.
//! The small-stack variant runs that reader on a measured stack in a release build.
//! `join_corpus.rs` extends the literal cases with generated integer/NULL inputs.
...
fn check_join_snapshots(small_stack: bool, left_join: bool) {
    ...
    // Adding a key-1 fact creates two pairs; adding a dimension then creates
    // three more. LEFT JOIN retains the same one unmatched row throughout.
    ...
    std::thread::scope(|scope| {
        ...
        // Starting two queries together can be refused while temporary storage
        // is being initialized. Finish the paused reader before executing the
        // next query; the snapshot check has already crossed both appends.
        worker.join().unwrap();
        assert_eq!(
...
fn typed_join_matches_an_independent_row_oracle_through_spill() {
    ...
    // Commit each row separately so matching duplicates span stored batches.
    // The first two rows also carry the maximum 65,536-byte text value.
    ...
    db.close().unwrap();
    // Retained maximum-text records need a larger accepted budget on macOS.
    // Both accepted cases still spill; the smaller budget still refuses.
    let spill_budget = if cfg!(target_os = "macos") {
        5_000_000
    } else {
        4_000_000
    };
    for cap in [16_000_000, spill_budget, 3_000_000] {
            ...
            for key in ["k", "day", "s"] {
                ...
                for left in 0..keys.len() {
                    ...
                    for right in 0..keys.len() {
                        // Ordinary equality matches signed zeros but never NaNs.
                        // Option::zip excludes NULL keys before comparison.
                        let equal = match key {
