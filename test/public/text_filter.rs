//! Test text comparisons, decoded literals and ownership of prepared SQL.
//!
//! Literal row IDs distinguish empty text, NULL, embedded control bytes and two
//! spellings of an accented character. Comparisons run after sorting, projection,
//! grouping and joins. Invalid escapes and mismatched types must fail preparation.
//!
//! The ownership case overwrites and drops the SQL string after preparation.
//! Repeated executions must still find the expected row, or report cancellation
//! when requested. Each result must release its reservations before a fresh query
//! checks that the database remains usable.
...
fn text_literal_ownership_cancellation_and_healed_reuse() {
    ...
    drop(source);
    // A run may finish before a later cancellation point. Accept completion
    // with the expected row, or cancellation after this run's token is set.
    for prefix in 0..64 {
        ...
        for step in 0..512 {
            ...
            match result.step() {
                ...
                QueryStep::Failed(error) => {
                    ...
                    assert!(cancel.is_cancelled(), "query cancelled before request");
                    complete = true;
                    break;
                }
            }
