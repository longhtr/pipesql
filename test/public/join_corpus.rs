//! Check composed joins against a small, independent row model.
//!
//! The model tries every pair of input rows. Equal non-NULL keys match; duplicate
//! keys produce multiple pairs. A left join also emits each unmatched left row
//! with NULLs on the right. Ordinary Rust maps then group these pairs to compute
//! expected counts and sums, without using the engine's join or grouping code.
//!
//! The corpus varies key overlap, missing keys and missing values independently,
//! then runs queries in both source directions. Explicit checks ensure it includes
//! no matches, all 64 possible matches, and a nonempty result whose values are all
//! NULL. Separate overflow cases test whether filters, ordering and joins require
//! an aggregate value or allow execution to skip it.

use super::*;
use std::collections::BTreeMap;

/// A nullable join key and a nullable value, kept separately to vary each pattern.
...
fn sum(values: impl Iterator<Item = Option<i64>>) -> Cell {
    // At most 64 pairs contribute values of magnitude at most 2^53 + 1.
    // Their sum fits INT64; the separate overflow test does not use this model.
...
/// Enumerate every matching pair, preserving duplicate rows and excluding NULL keys.
...
/// Add a NULL right row only when a left row found no match. Testing the result
/// length avoids confusing a matched row with a NULL value and an absent row.
...
fn generated_join_compositions_match_independent_multisets() {
    ...
    // Choose key overlap and the two NULL patterns independently. Otherwise a
    // missing value could accidentally imply a missing key and hide a join bug.
    for keys in 0..3 {
            ...
            for null_values in 0..3 {
                let make_rows = |side: usize| -> Vec<Row> {
                    (0..8)
                        .map(|i| {
                            let key = match keys {
                                0 => i as i64 + 20 * side as i64, // No keys shared between sides.
                                1 => ((i * (side + 1)) % 5) as i64,
                                2 => 0, // Every non-NULL key matches.
                                _ => unreachable!(),
                            };
                            ...
                            // The odd value above 2^53 catches accidental conversion to DOUBLE.
                ...
                for (left, right, l, r) in [
                    ...
                    let expected: Vec<_> = joined
                        ...
                        .collect();
                    // Reverse the equality operands and rename projected columns without
                    // changing the expected pairs. Select the left value twice
                    // to check repeated output positions too.
                    for prefix in [
                        format!(
...
fn joins_and_ordering_preserve_aggregate_demand_and_overflow_boundaries() {
    ...
    // Sorting by total must compute it even if a later stage drops the value,
    // rejects all rows or sorts again. Sorting by k leaves total unused.
    ...
    // The join needs the group key and count, so an unused SUM may be skipped.
    // Test both overflow in v+1 and overflow when converting the final sum.
    for expression in ["v", "v+1"] {
        let prefix = format!(
    ...
    // The join produces five pairs. Filtering l.v < 2 keeps only the key-1
    // pair, removing the values that would overflow when incremented.
    check(
        &db,
