//! Test EXTRACT(YEAR FROM date) against literal Gregorian calendar boundaries.
//!
//! Stored day offsets cover leap centuries, year transitions and both DATE
//! endpoints. Expected years are written out independently of the engine's date
//! conversion. Queries retain the original dates and NULLs alongside extracted
//! years across sorting, derived inputs and reopening.
//!
//! Other cases check constant date shifts, typed column replacement, joins and
//! grouping. Invalid extraction must report the relevant source bytes. Arithmetic
//! using an extracted year must fail only when demanded, and its error must remain
//! usable after the query is dropped. Early teardown checks reservation release.

use super::*;

// Day offsets are measured from 1970-01-01. The non-NULL pairs were checked
// with Python's datetime; the final offset is hidden beneath a NULL bit.
const DAYS: [i32; 17] = [
