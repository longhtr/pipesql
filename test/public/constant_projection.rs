//! Test STRING and DATE constants as columns in public query results.
//!
//! SELECT repeats a constant for each input row; EXTEND keeps the other columns;
//! SET replaces a visible column while its qualified original remains accessible.
//! Literal results check those distinctions through filtering, grouping and union.
//!
//! Prepared constants must survive the SQL string's lifetime and release their
//! reservations when a result is dropped early. Invalid constants must fail even
//! behind LIMIT 0. A wide case emits maximum-length literals in all 64 output
//! columns across multiple batches, then requires a 65th column to be rejected.
...
fn malformed_constants_fail_preparation_even_when_undemanded() {
    ...
    for constant in [
        ...
        "'123456789012345678901234567890123'", // 33 bytes between quotes exceeds the 32-byte limit
        "'\\uD800'",
        "'a'+'b'",
    ] {
