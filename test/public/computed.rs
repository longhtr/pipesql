//! Test computed columns and numeric expressions through the public query API.
//!
//! An expression should run only when another operation needs its value. The
//! opening cases move computations through SELECT, EXTEND, SET, filters, groups
//! and joins to check that rule. They pair queries that skip an overflowing value
//! with queries that must evaluate it and report the original SQL location.
//! Naming cases also distinguish a replacement column from its qualified original.
//!
//! Function cases check typed answers, NULLs, invalid calls and conditional
//! evaluation. Shared tests exercise cancellation, early result disposal and
//! stored DOUBLE values before and after reopening. Numerical references stay
//! beside their cases: exact bits for specified special values, and independently
//! calculated answers with explicit tolerances for approximate results.
//! Error-ownership cases destroy the SQL and query to check that error locations
//! survive. Individual scalar evaluation rules are tested in `src/scalar.rs`.

use super::*;

/// Require a repeatable overflow at the expected SQL location, then release the query.
/// Earlier rows are allowed; some expressions fail only on a later input row.
...
fn public_computed_names_types_scope_and_linear_dependencies() {
    ...
    // Each stage doubles the previous value by referring to it twice. Fourteen
    // stages fit with WHERE and SELECT; expanding every reference into a copy
    // of its expression would turn this short chain into 2^14 leaf evaluations.
    for _ in 0..14 {
...
fn extend_retains_typed_values_nulls_and_original_range_members() {
    ...
    // All expressions in this EXTEND read its input columns. The second id
    // therefore refers to the original value, not the new id+100 output.
    query(
        &db,
...
fn public_numeric_preserves_stored_double_bits_across_producers_and_reopen() {
    ...
    for reopened in [false, true] {
        ...
        for source in [
            ...
            // SIGN returns positive zero for either zero input. Rounding preserves
            // an input zero's sign. Literal answers also cover the smallest
            // nonzero DOUBLE values without calling the engine's rounding code.
...
fn public_sqrt_preserves_values_demand_and_domain_spans() {
    ...
    for (expression, expected) in [
        ("SQRT(4)", Some(2.0_f64)),
        // Convert these INT64 inputs to DOUBLE first, then take their square
        // roots at 100-digit Decimal precision and round the answers to DOUBLE.
        (
...
fn public_log10_preserves_promotion_composition_and_demand() {
    ...
    let (_directory, db) = join_fixture();
    let baseline = db.reserved_memory_bytes();
    // Reference: 10*Decimal(v/10).log10() at 100-digit precision, rounded
    // once to DOUBLE. One ULP is one step between neighboring representable
    // values. Allow three such steps for the logarithm and multiplication
    // together; the zero result remains exact.
    let prepared = db
...
fn check_public_logarithm(function: &str, references: &[(&str, u64)]) {
    let (_directory, db) = join_fixture();
    let baseline = db.reserved_memory_bytes();
    // References use 100-digit Decimal logarithms after converting inputs to
    // DOUBLE. Allow two ULPs of rounding error. For these positive finite
    // answers, subtracting the bit patterns counts representable steps.
...
fn public_exp_preserves_promotion_composition_and_demand() {
    let (_directory, db) = join_fixture();
    let baseline = db.reserved_memory_bytes();
    // Scalar references use Decimal.exp at 100-digit precision. The geometric
    // mean reference is sqrt(sqrt(10*20*30*40)), computed at the same precision,
    // so it does not repeat the SQL logarithm/exponential calculation. The
    // tolerances allow two ULPs for EXP alone and eight for the complete
    // geometric-mean query, which includes several rounded operations.
...
fn public_power_preserves_promotion_composition_and_demand() {
    ...
    for function in ["POW", "POWER"] {
        ...
        // Fifteen calls contribute fifteen constants and fifteen power operations
        // around one input value: 31 operations. Negation reaches the limit of
        // 32; another power call exceeds it while staying within the token limit.
        let nested = format!(
            "{}v{}",
    ...
    // Exact rational arithmetic gives 1331, 1728, 2197 and 2744. The six-ULP
    // tolerance covers the complete query: division, addition, power and
    // multiplication each contribute rounding error.
    let prepared = db
...
#[test]
fn public_numeric_null_literals_follow_function_result_types() {
    let (_directory, db) = join_fixture();
    for (expression, kind) in [
        ("NULL", DataType::Int64),
        ("-NULL", DataType::Int64),
        ("NULL+1", DataType::Int64),
        ("NULL*1.0", DataType::Double),
        ("NULL/0", DataType::Double),
        ("ABS(NULL)", DataType::Int64),
        ("SIGN(NULL)", DataType::Int64),
        ("DIV(NULL, 0)", DataType::Int64),
        ("MOD(1, NULL)", DataType::Int64),
        ("SAFE_DIVIDE(1, NULL)", DataType::Double),
        ("CAST(NULL AS DOUBLE)", DataType::Double),
        ("FLOOR(NULL)", DataType::Double),
        ("CEIL(NULL)", DataType::Double),
        ("CEILING(NULL)", DataType::Double),
        ("ROUND(NULL)", DataType::Double),
        ("SQRT(NULL)", DataType::Double),
        ("LN(NULL)", DataType::Double),
        ("LOG10(NULL)", DataType::Double),
        ("EXP(NULL)", DataType::Double),
        ("POWER(NULL, 1)", DataType::Double),
        ("POW(1, NULL)", DataType::Double),
    ] {
        let sql = format!("FROM facts |> SELECT {expression} AS n |> LIMIT 1");
        let prepared = db.prepare(&sql).unwrap();
        let column = prepared.result_column(0).unwrap();
        assert_eq!((column.data_type, column.nullable), (kind, true), "{sql}");
        drop(prepared);
        query(&db, &sql, vec![vec![Cell::Null]]);
    }
}
