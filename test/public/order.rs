//! Test ORDER BY results, key lookup and preservation of row values.
//!
//! Literal sequences check hidden sort keys, aliases, repeated keys and ordering
//! after joins or aggregation. A separate table assigns explicit ranks to numeric,
//! text and date values. Sorting those ranks produces expectations without using
//! the engine's comparator. NULL placement and direction are varied separately.
//!
//! Equal sort keys must not rewrite payloads: NaN bits, signed zero and long text
//! are compared with their original inputs. Invalid or ambiguous keys must fail
//! preparation and release reservations. A wide table checks all 64 visible keys
//! and rejects queries beyond the supported key and stage limits.
...
fn public_order_covers_scalar_domains_directions_nulls_and_raw_values() {
    ...
    // Rank non-NULL values in ascending order: NaNs precede negative infinity,
    // signed zeros tie, text follows UTF-8 bytes and dates follow day offsets.
    // Row 9 is NULL; the separate leading sort key determines its placement.
    let ranks = [
...
fn public_order_rejects_unsupported_and_ambiguous_keys_without_retaining_owners() {
    ...
    // Both columns are named x, but ORDER BY 2 selects one by position.
    query(
        &db,
