//! Test append recovery after a rejected write or an abandoned writer.
//!
//! Write one valid batch, then reject a batch with no columns. Further writes
//! must fail until the caller aborts. Explicit abort releases reservations and
//! resolves the transaction as aborted.
//!
//! Dropping a writer with unpublished rows leaves cleanup for reopening and
//! prevents another append in the meantime. Reopening must discard those rows;
//! a fresh append must then publish the expected value. A separate DATE test
//! checks the calendar endpoints accepted by caller input construction.
