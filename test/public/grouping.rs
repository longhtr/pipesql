//! Test grouped query results when groups fit in memory and when they spill to disk.
//!
//! Two passes over known integer keys give each group a predictable count and sum.
//! Varying the key count and concentrating one pass in a single group tests both
//! many groups and uneven group sizes. The same answers must hold under both
//! memory budgets, and temporary reservations identify which runs actually spill.
//!
//! Nullable COUNT cases check validity bits after grouping reorders the rows.
//! They also cancel after temporary storage appears and retry after storage is
//! refused, checking that execution releases its reservations. Separate small
//! cases cover typed grouping keys, equivalent NaNs and zeros, and nine distinct
//! key columns. Their expected answers do not call engine comparison helpers.
...
fn ordered_grouping_preserves_few_many_and_skewed_groups_across_memory_budgets() {
    for groups in [32, 4096] {
        for skewed in [false, true] {
            ...
            // macOS reserves cached-allocation space for the disk fallback even
            // when this larger budget lets every group stay in the hash table.
            let memory_budget = if cfg!(target_os = "macos") {
                3_000_000
            } else {
                2_000_000
            };
            let spill_budget = if cfg!(target_os = "macos") {
                1_800_000
            } else {
                1_200_000
            };
            for (memory, spill_many) in [(memory_budget, false), (spill_budget, true)] {
                    ...
                    match result.step() {
                        QueryStep::Rows(batch) => {
                            ...
                            for row in 0..batch.len() {
                                ...
                                // Each key gets `base` rows of amount 1. Skew sends
                                // all 4,096 amount-3 rows to key 0; otherwise
                                // each key gets another `base` rows.
                ...
                assert_eq!(
                    peak_temp > 0,
                    groups == 4096 && spill_many,
                    "groups={groups} skewed={skewed} memory={memory}: verify the intended grouping path"
                );
                drop(result);
...
// Enough memory to start execution, but not to hold all 4,096 groups. The
// successful case must observe temporary storage; a one-byte temp limit must fail.
...
fn nullable_count_releases_owners_when_spill_space_is_refused() {
    ...
    let query = db.prepare(PRESENCE_QUERY).unwrap();
    let baseline = db.reserved_memory_bytes();
    // A failed execution must release its reservations so the same prepared
    // query can start again and reach the same temporary-space refusal.
    for _ in 0..2 {
        let cancel = CancellationToken::new();
        let mut result = db.execute(&query, &cancel).unwrap();
...
/// Write each descending key twice. Amount and text are present for odd keys;
/// dates are present for keys whose remainder modulo four is two or three.
...
/// Write 4,096 rows of amount 1, then 4,096 of amount 3. Skew changes only the
/// second pass, assigning every row to region zero rather than cycling the keys.
...
fn declared_grouping_admits_nine_distinct_keys_with_typed_output() {
    ...
    db.declare_table("facts", &declarations, &cancel).unwrap();
    let mut writer = db.begin_append("facts", limits(), &cancel).unwrap();
    // The nine keys name different columns even though their values agree.
    // There are two complete-row groups, with counts one and two respectively.
