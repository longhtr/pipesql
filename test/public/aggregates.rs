//! Test aggregate results and expression failures through declared tables.
//!
//! Literal expected rows cover COUNT, SUM, AVG, MIN and MAX, both over a whole
//! table and within groups. NULL inputs, empty tables and repeated aggregation
//! distinguish counting rows from counting values. Typed cases check exact large
//! integers, text and date order, and floating-point NaNs, infinities and zeros.
//!
//! An unused aggregate may be skipped, but a needed argument must still be
//! evaluated even if its value cannot change the result. Error cases check this
//! distinction and the SQL span reported to the caller. SUM also has a separate
//! final conversion: a large intermediate total can become representable after
//! later inputs, and AVG can succeed when the requested SUM would overflow.
//! The final capacity test requires 256 short-text groups to fit without disk use.
...
fn extrema_nan_does_not_suppress_later_demanded_errors() {
    ...
    .unwrap();
    // Separate appends put NaN before the batch whose multiplication overflows.
    // Even after MIN or MAX becomes NaN, it must evaluate the later argument.
...
fn aggregates_follow_repeated_derived_and_joined_inputs() {
    ...
    // The two key-1 rows join to four pairs. Two pairs have a NULL left value,
    // so COUNT(f.v) counts only two; key 2 adds one to both COUNT forms.
    for (sql, expected) in [
        (
...
fn count_arguments_count_present_values_without_summing_them() {
    ...
    // COUNT needs only presence, but the expression still has to produce its
    // value. It cannot count this multiplication by inspecting validity alone.
...
fn declared_global_aggregates_preserve_types_null_counts_and_pinned_inputs() {
    ...
    // This odd integer exceeds exact DOUBLE precision. Converting inputs to
    // DOUBLE before summing would lose the low bit of the expected INT64 total.
    ...
    // Preparation fixes the input snapshot. Appending rows cannot change the
    // zero count of the query prepared while the table was empty.
    assert_eq!(
...
fn declared_integer_sum_overflow_is_final_and_demanded() {
    ...
    drop(query);
    // The running total exceeds INT64 after two inputs. A third value brings
    // the final sum back into range, so early INT64 overflow would be wrong.
    let mut writer = db.begin_append("facts", limits(), &cancel).unwrap();
    writer
        .write(
            &[ColumnInput {
...
fn short_text_extrema_keep_256_groups_in_memory_at_four_megabytes() {
    ...
    .unwrap();
    // Each group receives both eight-byte words. Reverse key order requires
    // ordered output to do real work; checking temp usage rules out silent spill.
