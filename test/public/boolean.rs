//! Test Boolean filter results and which expressions a filter evaluates.
//!
//! SQL predicates can be true, false or unknown; WHERE retains only true rows.
//! A local truth-table model checks nested AND, OR and NOT over all 27 combinations
//! of three nullable inputs. Literal cases separately cover NaN, operator
//! precedence and predicates following other query stages.
//!
//! Arithmetic failures distinguish required evaluation from skipped branches.
//! Memory checks compare simple conjunctions with predicates that need extra
//! Boolean scratch space. The same checks run on ordinary and measured small
//! stacks; the latter require the release test profile.
...
fn boolean_filters_match_independent_three_valued_model() {
    ...
    // Pair each generated SQL expression with its answers for all input rows.
    // None means unknown. The model below uses truth-table cases and does not
    // call the engine's predicate evaluator or branch planner.
        ...
        thread
            .spawn_scoped(scope, || {
                ...
                for (projection, column, rows) in [("id", "id", 4096_u64), ("id+0 AS x", "x", 256)]
                {
                    ...
                    // OR needs per-row Boolean state plus two allocation
                    // allowances. IN uses the same extra storage; AND needs none.
                    assert_eq!(charges[1] - charges[0], rows * 5 + 2 * 4096);
                    assert_eq!(charges[2], charges[1]);
                }
                // Run the native math calls inside the measured thread too.
                // Their result feeds both a conditional filter and an aggregate.
                query(
                    &db,
