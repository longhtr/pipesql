//! Test CAST from INT64 or DOUBLE to DOUBLE, also spelled FLOAT64.
//!
//! Literal bit patterns check integer rounding near 2^53 and at the INT64 limits.
//! Already-DOUBLE inputs must keep their bits, including NaNs and signed zero.
//! Queries repeat these checks across sorting and derived inputs; grouping also
//! shows how two distinct integers can become one key after conversion.
//!
//! A cast cannot rescue an overflow in its argument. Failure cases distinguish
//! preparation from execution, preserve error spans after dropping the query,
//! and check when outer expressions may skip the cast. Shared teardown checks
//! cover cancellation and abandoned results.
...
fn public_cast_rounds_integer_boundaries_and_preserves_nullable_schema() {
    ...
    // 2^53 and 2^53 + 1 convert to the same DOUBLE. Grouping the converted
    // values must therefore produce one group with two rows.
    query(
        &db,
...
// Retain the error after dropping both source and plan. Its byte range must
// identify the requested expression without borrowing either allocation.
