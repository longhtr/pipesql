//! Test IS NULL and IS NOT NULL on stored and computed values.
//!
//! Each nullable type has a NULL on a different input row. Expected row IDs
//! distinguish those NULLs from zero, NaN, infinity and empty text. Queries carry
//! the predicates through derived tables, sorting, joins and aggregation, with
//! separate expectations for an empty aggregate result.
//!
//! A non-nullable expression can still overflow. Asking whether its result is
//! NULL must evaluate it when demanded and retain the error's source span.
//! Earlier filters and LIMIT 0 check when execution may skip that work.
...
fn null_tests_preserve_demanded_errors_and_prior_predicate_order() {
    ...
    for test in [
        ...
        for boundary in ["", " |> ORDER BY x"] {
            ...
            // This fact rules out a NULL value, not an evaluation error.
