//! Test query-result lifetimes and errors that outlive their query.
//!
//! The success case loads and reopens one row, checks Q6 and selected Q1 values,
//! then calls step again after completion. Dropping the Q6 result must leave the
//! prepared plan's reservation intact; dropping that plan must release the rest.
//!
//! Failure cases trigger arithmetic overflow during preparation or execution.
//! They drop the original SQL and prepared query before inspecting the error.
//! Its operation and byte range must remain available without retaining query
...
fn arithmetic_errors_own_byte_ranges_after_query_and_source_drop() {
    ...
    // Repeated expressions make the demanded occurrence matter: an unused SUM
    // must not steal the span from the AVG that actually fails.
