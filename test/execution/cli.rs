//! Check query results and command failures through the built CLI.
//!
//! Load one literal row, then run query files in new processes. Q6 must print
//! its result type, value and floating-point bits; Q1 checks selected fields.
//! A division-by-zero query must be explainable but fail when executed.
//!
//! The remaining cases reject oversized, invalid UTF-8 and symlink query files.
//! Closing the output receiver before launch makes write failure deterministic
//! for both query and explain. These checks use normal command-line interfaces.
...
fn stock_cli_executes_query_file() {
    ...
    // EXPLAIN binds this expression without evaluating rows. QUERY must demand
    // the division and report its error before producing a result row.
    ...
    // Close the receiving end before launch. Closing it afterward would race
    // with a child that could finish writing before the parent runs again.
