//! Protect a Rust value with a native mutex allocated at a fixed address.
//!
//! A pthread mutex must stay where it was initialized. `new` first allocates its
//! final storage, reporting allocation failure to the caller, then initializes
//! the native lock there. Moving the returned Rust owner moves only a pointer.
//! Locking needs no further Rust allocation.
//!
//! A guard lends access to the value and unlocks when dropped. It cannot move
//! between threads because pthread requires the acquiring thread to unlock.
//! If a panic begins while the guard is held, dropping it marks the value poisoned;
//! later acquisitions refuse access to potentially interrupted mutation.
//!
//! `try_lock` makes one attempt; `lock` waits. Calling `lock` while already holding
//! it deadlocks, so callers must avoid reentry. Forgetting a guard leaves the mutex
//! locked. Destruction then leaks its storage rather than destroy a live lock.
...
pub struct Mutex<T> {
    // The one-element allocation is fixed in place after native initialization.
    // Moving this Box preserves that address.
...
// SAFETY: the native NORMAL mutex allows only one guard to access T at a time.
// Its allocation never moves and a guard stays on its acquiring thread. T must
// be Send because successive guards may belong to different threads.
unsafe impl<T: Send> Send for Mutex<T> {}

unsafe impl<T: Send> Sync for Mutex<T> {}

// Mark poisoning before unlocking when a panic begins under the guard. Later
// callers receive an error instead of accessing the interrupted mutation.
...
impl<T> Mutex<T> {
    ...
    /// Wait for exclusive access. Reentry on the holding thread deadlocks.
    /// This provides no bounded wait or fairness guarantee; the caller must ensure
    /// lock holders finish and that scheduling allows waiting work to proceed.
...
impl<T> Drop for Mutex<T> {
    fn drop(&mut self) {
        if let Some(storage) = self.release_storage() {
            // Forgetting a guard is legal in safe Rust. Keep both the locked
            // native mutex and its value alive rather than destroying either.
...
impl<T> Mutex<T> {
    // Return locked storage to the caller instead of destroying it. Drop leaks
    // it; the forgotten-guard test can unlock and reclaim the same allocation.
    fn release_storage(&mut self) -> Option<Box<[Inner<T>; 1]>> {
        ...
        if status == libc::EBUSY {
            // pthread forbids destroying a locked mutex. A forgotten guard must
            // therefore retain both the lock and its protected value.
            return Some(storage);
        }
...
mod tests {
    ...
    fn forgotten_guard_keeps_native_storage_alive() {
        ...
        // Exercise Drop's teardown directly so this test can reclaim the leak
        // afterward without weakening the production behavior.
        let storage = value
            .release_storage()
