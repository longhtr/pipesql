//! Check that the filesystem API reads names and changes files as the OS does.
//!
//! Most tests create temporary files and use Rust's standard library to obtain
//! expected names, metadata and errors. Symlink chains exercise the intermediate
//! work needed to resolve a path, which can be much larger than the final name.
//! Concurrent readers check that one traversal does not change another's cursor.
//!
//! The decoder tests instead build native directory records from literal bytes.
//! They can supply malformed lengths and offsets that a normal OS call should
//! never return. No production encoder supplies these expected layouts.
//!
//! Run with `cargo test -p pipesql-filesystem --lib`. These are filesystem API
//! and decoder checks; database recovery and native fault injection have separate
//! campaigns described in `DEVELOPMENT.md`.
...
fn fixture_record() -> Vec<u8> {
    // A 40-byte getattrlistbulk record: returned flags, zero error, then a name
    // reference at byte 28. Its offset 8 points to "x\0" at byte 36.
...
#[cfg(target_os = "linux")]
fn fixture_record() -> Vec<u8> {
    // A 24-byte getdents64 record with a nonzero inode, type DT_REG (8), and
    // "x\0" at byte 19. Zero-filled bytes supply the terminator and padding.
...
fn native_canonical_names_match_reference_on_joined_workers() {
    ...
    // Obtain expected paths without the resolver under test. All workers read
    // the same fixtures, so any shared traversal state can corrupt their answers.
...
fn native_deep_absolute_links_preserve_the_33_link_boundary() {
    // Remove children before parents without recursing through hundreds of
    // directories. A failed assertion must not trigger stack overflow in cleanup.
    struct Deep {
        root: Temp,
...
fn directory_total_bound_is_independent_of_per_call_work() {
    ...
    assert_eq!(count, 80);
    // The OS may return dot entries as well as the 80 named files. Measure the
    // total records so the next cases test the exact boundary on either platform.
...
fn canonicalization_preserves_native_directory_permission_errors() {
    // Require an unprivileged run: root could read the child and make the
    // permission-denied case disappear. Restore access before checking results
    // so a failed assertion does not leave an inaccessible fixture behind.
...
fn canonicalization_preserves_errors_at_the_native_name_ceiling() {
    // A 4095-byte name fits Linux's pathname buffer with its NUL terminator.
    // Appending a slash exceeds that buffer before the OS can reject traversal
    // through the regular file. A short symlink must preserve this error order.
    let root = Temp::new();
    let base = std::fs::canonicalize(&root.0).unwrap();
...
fn bounded_thread_observation_accepts_small_and_rejects_oversized_threads() {
    // Check the observer itself: rejecting the large thread prevents a stub
    // that always succeeds from certifying the small-stack tests. No database
    // operation runs here, so this does not measure its stack use.
