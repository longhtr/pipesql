//! Resolve macOS paths using the names reported by the filesystem.
//!
//! Walk one component at a time, keeping the resolved prefix separate from the
//! unread suffix. A symlink replaces that suffix with its target plus the remaining
//! path. Relative targets continue from the link's parent; absolute targets start
//! at root. Fixed buffers and limits on symlinks and native calls bound the work.
//!
//! Native name attributes supply the filesystem's spelling for ordinary names.
//! When traversal crosses devices, mount lookup may supply a matching mount path.
//! These steps preserve native naming behavior; they do not establish ownership
//! of a database file. The engine checks opened-file identities separately.
//!
//! Failure returns no partial canonical name to the caller. Optional mount lookup
//! may fall back after a native error, but it must still stop when the call budget
//! is exhausted. Apple/FreeBSD adaptation terms are in `THIRD_PARTY.md`.
...
// Darwin traversal retains native-sized path buffers, including their final NUL.
// The engine's larger input limit does not enlarge these intermediate buffers.
const PATH_BYTES: usize = 1024;
const SYMLINKS: usize = 33;
// Independently asserted by dev/bindings/platform.c against native headers.
const NATIVE_SYMLINK_TYPE: u32 = 5;
// Keep native errors separate from our work limit so an optional native fallback
// cannot swallow exhaustion and continue making calls.
...
// An unavailable mount name may leave the original path usable. Running out of
// permitted calls is different: no fallback may continue native work afterward.
...
// Use the mount's spelling only when its object matches and its prefixes are
// directories. This naming heuristic is not an opened-file identity check: the
// supplied object number may be Darwin's legacy name-related ATTR_CMN_OBJID.
fn matched_mount(
    path: &CStr,
    device: i32,
    object: u64,
    ...
    // Check each parent of the candidate mount path. Removing a suffix on every
    // iteration makes this walk finite even before the native-call limit applies.
...
fn resolve(path: &[u8], output: &mut PathSlot, work: &mut NativeWork) -> Outcome<()> {
    ...
    // Advance within the suffix instead of shifting it after every component.
    // Only symlinks replace it and reset the cursor; each spends a link allowance.
    let mut cursor = 0;
    while cursor != left.len {
        ...
        // Some filesystems do not support this attribute request. Their stat
        // result still supplies type and device, but cannot correct name spelling.
        ...
        if is_link {
            ...
            // Replace the caller's component spelling with the filesystem's name.
            // Do this only after symlink handling has chosen the actual target.
