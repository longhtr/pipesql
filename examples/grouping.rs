//! Explore how group count, uneven input and memory budget affect aggregation.
//!
//! The program writes 8,192 sales in two passes. The first spreads amount 1
//! evenly across regions. The second either spreads amount 3 the same way or
//! assigns every row to region zero. This construction gives simple expected
//! counts, sums, minima and maxima for checking each returned group.
//!
//! Pass a new absolute database path and a query memory limit in bytes. Optional
//! arguments choose 32 or 4096 groups (default 4096), then `even` or `skewed`
//! distribution (default `even`). The report samples engine memory and temporary
//! reservations and times execution together with result checks and disposal.
//! These samples do not measure total process memory. Run `cargo run --release
//! --offline --locked --example grouping -- /absolute/new-sales 1200000`.
//! Compare 1200000 and 2000000 with identical inputs and fresh paths. Observe
//! temporary reservations rather than assuming a smaller budget causes spill.
//! The caller owns each remaining database.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Measure execution, checking its answers and dropping its result. Database
    // creation, opening and query preparation have already finished.
        ...
        match result.step() {
            QueryStep::Rows(batch) => {
                for row in 0..batch.len() {
                    ...
                    // Every region receives `first` ones. Only regions receiving
                    // second-pass rows contain threes, so their maximum is 3;
                    // all other regions have both minimum and maximum 1.
