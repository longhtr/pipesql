//! Explore the cost of grouping short and maximum-length strings.
//!
//! Each key receives two values: a string of a's and a string of z's. The minimum,
//! maximum and count are therefore known without sorting the input in a reference
//! implementation. The program checks those answers while varying the number of
//! groups, string width, query memory budget and number of rows per input batch.
//!
//! Pass a new absolute database path, group count, text width in bytes and query
//! memory limit in bytes. An optional fifth argument sets the batch size; it
//! defaults to one row. The accepted combinations are checked before creation.
...
//! so it does not isolate hash-table cost. The reported time includes
//! execution, answer checks and result disposal. Reservation samples measure
//! engine accounting; they exclude the example's own strings and process overhead.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // These strings belong to the example, so database reservation counters
    // do not include them. Keep them to check the returned minimum and maximum.
