...
//!
//! It then starts the query again and cancels after temporary storage appears,
//! checking that cancellation releases the query's reservations. Pass a new
//! absolute database path and a query memory limit in bytes. The printed time
//! includes execution, result checks and result disposal. Memory samples are
//! engine reservations, not total process memory. Run `cargo run --release
//! --offline --locked --example composed -- /absolute/new-sales 2200000`.
//! Compare with 12000000 on a fresh path. Both can use temporary storage because
//! the join and ORDER BY sort their input; the total alone cannot attribute
//! spill to one operator. Timing excludes setup, preparation and later cancellation.
//! Progress counts work returns; the reported rows counter counts batches.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Start before execution reserves its memory. Stop after checking every
    // row and dropping the result. The later cancellation exercise is outside
    // this measurement.
    ...
    // Wait for temporary storage before cancelling, so cleanup has something
    // to release beyond the initial execution workspace.
