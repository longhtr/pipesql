...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Both plans stay live for every run, so the shared baseline is identical.
    // Warm both variants. Alternate which variant runs first in each sample pair.
    for _ in 0..WARMUPS {
    ...
    loop {
        ...
        match result.step() {
            ...
            QueryStep::Rows(batch) => {
                // 327 complete cycles of 0..99 and a final 0..67 give
                // 1,620,928. Six ordered additions add another 196,608.
                if seen
                    || batch.len() != 1
