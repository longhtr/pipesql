//! Create dated events and report their amounts by calendar year.
//!
//! The input includes a leap day, a year boundary and one missing date. Grouping
//! by EXTRACT(YEAR FROM happened) combines the two events in 2000 and keeps the
//! missing date in a separate NULL group. The program reopens before querying, checks
//! each year total, and prints the report only after the query finishes.
//!
//! Pass a new absolute database path as the first argument. The database remains
//! there for inspection. Run with `cargo run --release --offline --locked
//! --example calendar_year -- PATH`. The query is in `calendar_year.sql`.
//! The expected rows below distinguish NULL from a date and include leap day;
//! `src/date.rs` explains conversion through 400-year Gregorian cycles.
//! YEAR means the calendar year, not an ISO week-numbering year. See
//! `docs/sql/expressions.md#text-and-dates`
//! for the accepted expression forms.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // DATE stores whole days relative to 1970-01-01. These offsets represent
    // 1999-12-31, 2000-02-29, 2000-12-31 and 2001-01-01. The fifth date is
    // NULL because its validity bit is clear; its zero payload is ignored.
