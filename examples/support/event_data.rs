//! Build the shared input for event reports and their allocation checks.
//!
//! Events are convenient to describe as records, but Append accepts columns.
//! `write_events` converts up to 256 records at a time into typed column arrays
//! and validity bits. The arrays can be reused after each write returns. The
//! caller owns the transaction and decides whether to commit or abort it.
//!
//! `declare` creates both tables and commits four dimension rows. ID 2 has two
//! labels, so joining an event with that ID produces two rows; ID 3 has an empty
//! label, which must remain distinct from NULL. Literal event inputs live here.
//! Expected report answers stay with callers so sharing setup does not also
//! share the calculation used to judge the engine.
...
/// One input event. Absent optional fields become SQL NULL. `day` holds whole
/// days from 1970-01-01; writing validates that it lies within the DATE range.
#[derive(Clone, Copy)]
pub struct Event {
    pub id: i64,
...
// The four date offsets represent 1999-12-31, 2000-02-29, 2000-12-31 and
// 2001-01-01. Missing dates form a separate group in the yearly report.
...
/// Create the event and dimension schemas and publish the fixed dimension rows.
...
/// Write events in chunks of at most 256 rows without committing the transaction.
/// The caller must provide valid DATE offsets and sufficient batch and byte limits.
/// Invalid dates panic because they indicate a mistake in this example's input;
/// append errors are returned to the caller.
pub fn write_events(
    append: &mut Append<'_>,
    events: &[Event],
    ...
    for chunk in events.chunks(ROWS) {
        // Start every bitmap clear. A missing field may carry a zero placeholder
        // in its value array, but only a set validity bit makes it a SQL value.
