...
// The example copies only numeric cells out of borrowed batches. DOUBLE stores
// raw bits here: Rust floating equality would lose zero signs and reject NaNs.
...
fn main() -> Result<()> {
    ...
    // Report only after every query has finished, released its owners and passed.
...
// This is a fixture oracle, not an implementation of general SQL equality.
// Accept either original zero or NaN representation, never invented key bits.
...
fn create_samples(path: &Path) -> Result<()> {
    ...
    // The two NULL payloads deliberately differ. Their validity bits, not their
    // numeric payloads, determine whether SQL sees a value.
    let values = [
        NEGATIVE_ZERO,
        0,
        NAN_A,
        ...
        match result.step() {
            ...
            QueryStep::Rows(batch) => {
                ...
                for row in 0..batch.len() {
                    // Every query in this fixture has at most eight result rows.
                    if rows.len() == 8 {
