//! Check the legacy scan's results, memory ownership and read failures.
//!
//! Fixtures pass repeating rows through the public loader and query executor.
//! Projection cases cover all nonempty subsets of the seven stored columns,
//! including reordered and repeated outputs. Separate cases cross the different
//! DATE and numeric block boundaries, and count rows without reading payloads.
//!
//! Exact and one-byte-short budgets check refusal before I/O. Injected read
//! failures and cancellation must stop queries and release their reservations,
//! even after a result prefix has been emitted. Thread cases exercise shared
//! readers; the reported-stack case checks the worker's actual stack size.
//! Readiness timeouts and panics must let the worker exit before the test
//! removes its database.
//!
//! These tests own their expected values and fault schedules. Native syscall
//! observation and independent format decoding belong to separate campaigns.
...
use crate::query::{FilterLiteral, MAX_ROW_VALUES, Predicate};
use crate::storage::format::{BlockDescriptor, RootState};
...
fn emitted_batch_outlives_source_and_keeps_its_charge() {
    ...
    for (sql, expected) in [
        ...
        assert!(emitted);
        // Extract the owned batch through the internal boundary. The public
        // borrowed batch API intentionally cannot outlive its QueryResult.
        let batch = {
...
fn shared_readers_refuse_repair_without_mutating_namespace() {
    ...
    // Give both readers enough workspace so they fail on the missing root,
    // rather than contend for the single-query fixture's memory budget.
    let path = database.path().to_owned();
    database.close().unwrap();
    let database = Database::open(&path, Config::new(4_000_000, 1_000_000).unwrap()).unwrap();
    let missing = database.path().join(crate::storage::recovery::ROOT_B_NAME);
...
fn cancellation_at_every_effect_and_from_real_thread() {
    ...
    for source in [
        ...
        let delayed = case == "scan-delayed-readiness";
        let fail_before_ready = case == "scan-worker-failure";
        std::thread::scope(|scope| {
            // Drop channels after a panic before the scope waits for the worker.
            let (entered_send, entered_receive) = std::sync::mpsc::sync_channel(0);
            let (resume_send, resume_receive) = std::sync::mpsc::sync_channel(0);
            let (database, query, token) = (&database, &query, &token);
            let worker = scope.spawn(move || {
                assert!(!fail_before_ready, "scan worker before readiness");
                ...
                let mut effects = Effects::with_faults(Faults {
                    action: Some(Box::new(move |_, effect| {
                        if !notified && effect == Effect::Query(QueryEffect::ReadPayload) {
                            notified = true;
                            if delayed {
                                assert!(resume_receive.recv().is_err());
                            }
            ...
            entered_receive
                .recv_timeout(if delayed {
                    std::time::Duration::from_millis(20)
                } else {
                    std::time::Duration::from_secs(10)
                })
                .expect("scan worker readiness");
            assert_ne!(case, "scan-coordinator-panic", "scan coordinator failure");
            token.cancel();
...
#[test]
fn scan_thread_failure_controls_terminate() {
    for (case, diagnostic) in [
        ("scan-worker-failure", "scan worker readiness: Disconnected"),
        ("scan-coordinator-panic", "scan coordinator failure"),
        ("scan-delayed-readiness", "scan worker readiness: Timeout"),
    ] {
        crate::test_subprocess::run(
            concat!(
                module_path!(),
                "::cancellation_at_every_effect_and_from_real_thread"
            ),
            case,
            false,
            diagnostic,
...
fn every_projection_mask_has_exact_buffers_and_values() {
    ...
    for mask in 1_u8..128 {
        // Reverse output order and repeat one value. Output positions may need
        // separate cells, but each source column must still have only one buffer.
...
fn count_without_values_retains_metadata_scratch_without_payload_reads() {
    ...
    assert_eq!(
        ...
        // Include COUNT's u32 row counter in addition to its output batch.
        106_496
            + 28_672
...
fn computed_scan_reserves_before_io_at_exact_and_next_byte() {
    let (_fixture, database) = loaded(1);
    for sql in [
        "FROM lineitem |> SELECT l_quantity*2 AS x |> WHERE x >= 0",
        "FROM lineitem |> SELECT CASE WHEN l_quantity>=0 THEN l_quantity*2 ELSE DIV(1, 0) END AS x |> WHERE x >= 0",
    ] {
        let query = database.prepare(sql).unwrap();
        let cancel = CancellationToken::new();
        let physical = lower(&database, &query, RootState::Empty, 0).unwrap();
...
fn check_shared_query_threads(small_stack: bool) {
    ...
    thread
        .spawn(move || {
            ...
            // The child readers above have their own stacks. Run these queries
            // on this worker too, so the small-stack case covers opening metadata,
            // decoding and execution within the stack whose size was checked.
            let query = database.prepare(SQL).unwrap();
            let cancellation = CancellationToken::new();
            let mut result = database.execute(&query, &cancellation).unwrap();
            drain(&mut result, &mut Effects::default()).unwrap();
            ...
            let query = database
                .prepare(include_str!("../../../test/data/upstream/q1-upstream.pipe.sql"))
                .unwrap();
            let mut result = database.execute(&query, &cancellation).unwrap();
            let mut rows = 0;
            let mut finished = false;
