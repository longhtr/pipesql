//! Check EXCEPT and INTERSECT, each in DISTINCT and ALL modes, through the runtime.
//!
//! Both branches project the same source column twice. Expected key ranges and
//! copy counts check positional mapping and multiplicity without another set
//! implementation. Exact-budget runs compare actual buffer capacities with
//! their charges; one byte less must fail before I/O.
//!
//! The fixture reaches each controller phase on both inputs, including spills.
//! Tests replay retained rows after partial output, cancel at each phase, damage
//! either input and exhaust temporary space. Failed queries must stop issuing
//! I/O and release their accounts. Fresh queries check reuse after file damage
//! and temporary-space refusal.

use super::*;
use crate::Value;
use crate::effects::Faults;
use crate::execution::blocking::test_support::{
    Directory, expected_buffer_charge, two_column_database,
};
use crate::execution::blocking::{Files, RECORD_HEADER};
use crate::execution::{QueryResult, QueryStep, State};

// The left input has two copies of keys 0..90; the right omits keys 88 and 89.
// Each expected range names the surviving keys; the last field gives copies.
const CASES: [(&str, std::ops::Range<i64>, usize); 4] = [
    (
        "FROM facts |> SELECT k AS a, k AS b |> EXCEPT DISTINCT (FROM facts |> WHERE k<88 |> SELECT k, k)",
        88..90,
        1,
    ),
    (
        "FROM facts |> SELECT k AS a, k AS b |> INTERSECT DISTINCT (FROM facts |> WHERE k<88 |> SELECT k, k)",
        0..88,
        1,
    ),
    (
        "FROM facts |> SELECT k AS a, k AS b |> EXCEPT ALL (FROM facts |> WHERE k<88 |> SELECT k, k)",
        88..90,
        2,
    ),
    (
        "FROM facts |> SELECT k AS a, k AS b |> INTERSECT ALL (FROM facts |> WHERE k<88 |> SELECT k, k)",
        0..88,
        2,
    ),
];
const STEPS: usize = 100_000;

fn sorted_set<'a, 'db>(result: &'a mut QueryResult<'db, '_>) -> &'a mut SortedSet<'db> {
    let State::Running(runtime) = &mut result.state else {
        panic!("live sorted set");
    };
    runtime.first_sorted_set_mut()
}

fn phase_index(phase: Phase) -> usize {
    match phase {
        Phase::Create(side) => side * 7,
        Phase::Read(side) => side * 7 + 1,
        Phase::Await(side) => side * 7 + 2,
        Phase::Capture(side, _) => side * 7 + 3,
        Phase::Push(side, _) => side * 7 + 4,
        Phase::Spill(side, _) => side * 7 + 5,
        Phase::Sort(side) => side * 7 + 6,
        Phase::Seek => 14,
        Phase::Emit => 15,
        Phase::Done => 16,
        Phase::Failed => panic!("healthy sorted set phase"),
    }
}

// Measure the live allocations separately from constructor sizing. Include
// each input's scratch-creation reservation only while creation is pending.
fn check_physical_account(result: &mut QueryResult<'_, '_>) {
    let State::Running(runtime) = &result.state else {
        panic!("live sorted set");
    };
    let inline = runtime.controller_inline_bytes(&result.plan);
    assert_eq!(inline, size_of::<SortedSet<'_>>() as u64);
    let sorted_set = sorted_set(result);
    let mut expected = size_of::<SortedSet<'_>>();
    let mut creation = 0;
    for side in &sorted_set.sides {
        expected += expected_buffer_charge(side.record.bytes.capacity())
            + side
                .sort
                .allocation_capacities()
                .into_iter()
                .map(expected_buffer_charge)
                .sum::<usize>();
        if matches!(side.files, Files::Pending(_)) {
            creation += crate::storage::scratch::Creation::memory_requirement_bytes();
        }
    }
    assert_eq!(
        expected as u64 + creation,
        sorted_set.memory_bytes() + inline,
        "inline owner and actual capacities and allocator allowances are charged once"
    );
}

fn collect(result: &mut QueryResult<'_, '_>, effects: &mut Effects) -> Vec<(i64, i64)> {
    let mut rows = Vec::new();
    for _ in 0..STEPS {
        check_physical_account(result);
        match result.step_with_effects(effects) {
            QueryStep::Progress => (),
            QueryStep::Rows(batch) => {
                for row in 0..batch.len() {
                    let (Some(Value::Int64(a)), Some(Value::Int64(b))) =
                        (batch.value(row, 0), batch.value(row, 1))
                    else {
                        panic!("two required integer positions");
                    };
                    rows.push((a, b));
                }
            }
            QueryStep::Finished => {
                // Set operations promise multiplicity, not output order.
                rows.sort_unstable();
                return rows;
            }
            QueryStep::Failed(error) => panic!("sorted set failed: {error}"),
        }
    }
    panic!("sorted set exceeded fixture work bound");
}

#[test]
fn exact_admission_spill_and_replay_reconcile_owned_capacities() {
    let directory = Directory::new();
    let db = two_column_database(&directory);
    for (sql, keys, copies) in &CASES {
        let expected: Vec<_> = keys
            .clone()
            .flat_map(|key| std::iter::repeat_n((key, key), *copies))
            .collect();
        let cancel = CancellationToken::new();
        let query = db.prepare(sql).unwrap();
        let baseline = db.reserved_memory_bytes();
        let result = db.execute(&query, &cancel).unwrap();
        // Source opening needs a transient catalog buffer as well as the
        // retained workspace. Both branches open sequentially, so one suffices.
        let peak = result.accounted_memory_bytes() + crate::storage::catalog::MAX_BYTES as u64;
        drop(result);
        for shortfall in [0, 1] {
            let pressure = db
                .reserve_memory(
                    db.config().memory_limit_bytes() - baseline - peak + shortfall,
                    "sorted set minimum test",
                )
                .unwrap();
            let mut effects = Effects::default();
            let admitted = db.execute_with_effects(&query, &cancel, &mut effects);
            if shortfall == 1 {
                assert!(matches!(admitted, Err(Error::Resource { .. })));
                assert_eq!(effects.count(), 0);
            } else {
                let mut result = admitted.unwrap();
                assert_eq!(collect(&mut result, &mut effects), expected);
            }
            drop(pressure);
            assert_eq!(db.reserved_memory_bytes(), baseline);
            assert_eq!(db.reserved_temp_bytes(), 0);
        }

        let mut result = db.execute(&query, &cancel).unwrap();
        let mut effects = Effects::default();
        // Stop after the first output row, then restart both retained files.
        // This calls the controller directly so its second-replay rejection
        // can be inspected without failing the enclosing QueryResult.
        for step in 0..STEPS {
            if matches!(result.step_with_effects(&mut effects), QueryStep::Rows(_)) {
                break;
            }
            assert!(step + 1 < STEPS, "sorted set did not emit");
        }
        let owner = sorted_set(&mut result);
        assert!(owner.sides.iter().all(|side| side.ordinal > 0));
        owner.replay(&cancel).unwrap();
        assert!(matches!(owner.replay(&cancel), Err(Error::Corrupt(_))));
        assert_eq!(collect(&mut result, &mut effects), expected);
        drop(result);
        assert_eq!(db.reserved_memory_bytes(), baseline);
        assert_eq!(db.reserved_temp_bytes(), 0);
    }
}

#[test]
fn cancellation_reaches_every_input_sort_merge_and_emission_phase() {
    let directory = Directory::new();
    let db = two_column_database(&directory);
    for (sql, _, _) in &CASES {
        let query = db.prepare(sql).unwrap();
        let baseline = db.reserved_memory_bytes();
        for target in 0..17 {
            let cancel = CancellationToken::new();
            let mut result = db.execute(&query, &cancel).unwrap();
            let mut effects = Effects::default();
            let mut reached = false;
            for _ in 0..STEPS {
                if phase_index(sorted_set(&mut result).phase) == target {
                    cancel.cancel();
                    let before = effects.count();
                    for _ in 0..2 {
                        let step = result.step_with_effects(&mut effects);
                        if target == 16 {
                            assert!(matches!(step, QueryStep::Finished));
                        } else {
                            assert!(
                                matches!(step, QueryStep::Failed(Error::Cancelled)),
                                "phase {target}"
                            );
                        }
                    }
                    assert_eq!(effects.count(), before);
                    reached = true;
                    break;
                }
                let step = result.step_with_effects(&mut effects);
                assert!(
                    matches!(step, QueryStep::Progress | QueryStep::Rows(_)),
                    "phase {target} was not reached"
                );
            }
            assert!(reached, "phase {target}");
            drop(result);
            assert_eq!(db.reserved_memory_bytes(), baseline);
            assert_eq!(db.reserved_temp_bytes(), 0);
        }
    }
}

#[test]
fn both_sorted_inputs_reject_corruption_truncation_and_read_failure() {
    let directory = Directory::new();
    let db = two_column_database(&directory);
    for (sql, keys, copies) in &CASES {
        let expected: Vec<_> = keys
            .clone()
            .flat_map(|key| std::iter::repeat_n((key, key), *copies))
            .collect();
        let query = db.prepare(sql).unwrap();
        let baseline = db.reserved_memory_bytes();
        let cancel = CancellationToken::new();
        for side in 0..2 {
            for fault in 0..3 {
                let mut result = db.execute(&query, &cancel).unwrap();
                let mut effects = Effects::default();
                let mut reached = false;
                for _ in 0..STEPS {
                    if matches!(sorted_set(&mut result).phase, Phase::Seek) {
                        reached = true;
                        break;
                    }
                    assert!(matches!(
                        result.step_with_effects(&mut effects),
                        QueryStep::Progress
                    ));
                }
                assert!(reached);
                if side == 1 && fault == 2 {
                    // Seek stops after loading the left record. Complete that
                    // step first so the injected read failure targets the right.
                    assert!(matches!(
                        result.step_with_effects(&mut effects),
                        QueryStep::Progress
                    ));
                    let owner = sorted_set(&mut result);
                    assert!(owner.sides[0].sort.sorted_cursor().record().is_some());
                    assert!(owner.sides[1].sort.sorted_cursor().record().is_none());
                }
                let input = &mut sorted_set(&mut result).sides[side];
                let slot = input.sort.merge.input.slot;
                let offset = input.sort.merge.result.unwrap().start;
                let Files::Open(scratch) = &mut input.files else {
                    panic!("sorted scratch input");
                };
                match fault {
                    0 => scratch
                        .write(
                            slot,
                            offset + RECORD_HEADER as u64 + 10,
                            &[255],
                            &cancel,
                            &mut effects,
                        )
                        .unwrap(),
                    1 => scratch.reset(slot, &cancel, &mut effects).unwrap(),
                    2 => {
                        effects = Effects::with_faults(Faults {
                            fail_at: Some(0),
                            ..Faults::default()
                        })
                    }
                    _ => unreachable!(),
                }
                let mut failed = false;
                for _ in 0..STEPS {
                    match result.step_with_effects(&mut effects) {
                        QueryStep::Progress => (),
                        QueryStep::Failed(error) => {
                            if fault == 2 {
                                assert!(matches!(error, Error::Io { .. }));
                            } else {
                                assert!(matches!(error, Error::Corrupt(_)));
                            }
                            failed = true;
                            break;
                        }
                        _ => panic!("corrupt sorted set input emitted or completed"),
                    }
                }
                assert!(failed);
                let before = effects.count();
                assert!(matches!(
                    result.step_with_effects(&mut effects),
                    QueryStep::Failed(_)
                ));
                assert_eq!(effects.count(), before);
                drop(result);
                assert_eq!(db.reserved_memory_bytes(), baseline);
                assert_eq!(db.reserved_temp_bytes(), 0);
            }
        }
        let mut result = db.execute(&query, &cancel).unwrap();
        assert_eq!(collect(&mut result, &mut Effects::default()), expected);
    }
}

#[test]
fn temporary_refusal_is_terminal_and_allows_healthy_reuse() {
    let directory = Directory::new();
    let db = two_column_database(&directory);
    for (sql, keys, copies) in &CASES {
        let expected: Vec<_> = keys
            .clone()
            .flat_map(|key| std::iter::repeat_n((key, key), *copies))
            .collect();
        let query = db.prepare(sql).unwrap();
        let baseline = db.reserved_memory_bytes();
        let cancel = CancellationToken::new();
        let mut result = db.execute(&query, &cancel).unwrap();
        db.temporary
            .reserve(db.config().temp_limit_bytes())
            .unwrap();
        let mut effects = Effects::default();
        let mut failed = false;
        for _ in 0..STEPS {
            match result.step_with_effects(&mut effects) {
                QueryStep::Progress => (),
                QueryStep::Failed(Error::Resource { .. }) => {
                    failed = true;
                    break;
                }
                _ => panic!("sorted set must refuse exhausted temporary storage"),
            }
        }
        assert!(failed);
        let before = effects.count();
        assert!(matches!(
            result.step_with_effects(&mut effects),
            QueryStep::Failed(Error::Resource { .. })
        ));
        assert_eq!(effects.count(), before);
        drop(result);
        db.temporary.release(db.config().temp_limit_bytes());
        assert_eq!(db.reserved_memory_bytes(), baseline);
        assert_eq!(db.reserved_temp_bytes(), 0);
        let mut healthy = db.execute(&query, &cancel).unwrap();
        assert_eq!(collect(&mut healthy, &mut Effects::default()), expected);
        drop(healthy);
        assert_eq!(db.reserved_memory_bytes(), baseline);
        assert_eq!(db.reserved_temp_bytes(), 0);
    }
}
