//! Test aggregate values, shared memory limits and release on failure.
//!
//! SQL cases run against a small legacy table and compare results with values
//! specified here. Direct accumulator tests reach boundaries that a small table
//! cannot: the last permitted row count, malformed state and different mappings
//! from groups to accumulator slots. These tests inspect production internals;
//! the separate semantic campaign checks results through the public interface.
//!
//! Cancellation and injected I/O failures must stop execution and release its
//! reservations. Tests under `grouping` cover the additional storage and replay
//! needed when groups do not fit in memory.
...
use crate::execution::*;
use crate::query::MAX_STAGES;
use crate::query::scalar::{Expression, Op};
use crate::query::{FilterLiteral, Predicate};
use crate::storage::format::RootState;
use crate::storage::recovery::inspect_namespace;
...
fn concurrent_aggregate_readers_reconcile_one_memory_account() {
    ...
    for (source, limit, workers, expected) in [
        (
            include_str!("../../../test/data/q6.pipe.sql"),
            6_000_000,
            4,
            4,
        ),
        (
            include_str!("../../../test/data/upstream/q1-upstream.pipe.sql"),
            4_000_000,
            2,
            2,
        ),
        ...
        let outcome = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            std::thread::scope(|scope| {
                // Keep every admitted result alive for the shared measurement.
                // Endpoints belong to this closure, so a parent panic releases
                // waiting workers before the scope tries to join them.
                let handles: Vec<_> = (0..workers)
                    .map(|index| {
                        if index == 1 {
                            assert_ne!(
                                case, "aggregate-partial-start",
                                "aggregate partial startup"
                            );
                        }
                        ...
                        let handle = scope.spawn(move || {
                            ...
                            let admitted = database.execute(query, &token);
                            if index == 0 {
                                assert_ne!(
                                    case, "aggregate-worker-panic",
                                    "aggregate worker failure"
                                );
                                if case == "aggregate-delayed-readiness" {
                ...
                for (_, entered, _) in &handles {
                    entered
                        .recv_timeout(if case == "aggregate-delayed-readiness" {
                            Duration::from_millis(20)
                        } else {
                            Duration::from_secs(5)
                        })
                        .expect("aggregate worker readiness");
                }
                assert_ne!(
                    case, "aggregate-coordinator-panic",
                    "aggregate coordinator failure"
                );
        ...
        if !case.is_empty() {
            let panic = outcome.expect_err("aggregate control must panic");
            let expected = match case.as_str() {
                "aggregate-partial-start" => "aggregate partial startup",
                "aggregate-worker-panic" => "aggregate worker readiness: Disconnected",
                "aggregate-delayed-readiness" => "aggregate worker readiness: Timeout",
                "aggregate-coordinator-panic" => "aggregate coordinator failure",
                _ => panic!("unknown aggregate failure control"),
            };
            ...
            assert_eq!(database.reserved_temp_bytes(), 0);
            let cancel = CancellationToken::new();
            let retry = database.execute(&query, &cancel).unwrap();
            assert_eq!(finish_query(retry, &mut Effects::default()).unwrap(), 1);
        } else {
            outcome.unwrap();
        }
        assert_eq!(
        ...
        database.close().unwrap();
        if !case.is_empty() {
            println!("aggregate workers exited, reservations released and retry passed");
            return;
        }
    }
}
...
#[test]
fn aggregate_worker_failure_controls_terminate() {
    for case in [
        "aggregate-partial-start",
        "aggregate-worker-panic",
        "aggregate-delayed-readiness",
        "aggregate-coordinator-panic",
    ] {
        crate::test_subprocess::run(
            concat!(
                module_path!(),
                "::concurrent_aggregate_readers_reconcile_one_memory_account"
            ),
            case,
            true,
            "aggregate workers exited, reservations released and retry passed",
        );
    }
}
...
fn source_occurrences_do_not_split_shared_aggregate_state() {
    ...
    eprintln!(
        ...
        size_of::<Expression>(),
        size_of::<crate::query::AggregateEntry>(),
...
fn integer_aggregate_range_covers_the_last_admitted_row_and_refuses_the_next() {
    ...
    .unwrap();
    // Seed the state just before the row limit instead of loading billions of
    // rows. SUM can exceed INT64 here while AVG still has a representable result.
...
fn aggregate_positions_preserve_group_isolation_across_batch_and_lane_widths() {
    ...
    // Group lookup chooses where each group's accumulators live. Moving a
    // group to another slot must preserve its values across batch boundaries.
...
fn aggregate_consumes_derived_values_without_storage_metadata() {
    ...
    // Give the consumer a column produced by an earlier aggregate. It has a
    // type and identity but no stored-column metadata. Constructing this input
    // directly tests the accumulator's interface without a second query stage.
    ...
    let crate::query::AggregateArgument::Numeric(expression) =
...
fn aggregate_workspace_and_mappings_are_independently_checked() {
    ...
    for mutation in 0..10 {
        ...
        match mutation {
            ...
            8 => groups.aggregate.lanes = BATCH_ROWS + 1,
            9 => {
...
fn aggregate_cancellation_at_every_public_step_releases_owners() {
    ...
    for sql in [
        ...
        assert_eq!(rows, 1);
        drop(control);
        // Include the step that would finish: cancellation must win even after
        // the result row has been delivered, until Finished has been reported.
