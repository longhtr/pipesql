//! Check that grouping can replay its producer and its own retained output.
//!
//! Small hash capacity forces grouping to discard partial groups and request
//! input again. Public-query cases exercise scans, sorts, joins, set operations
//! and earlier aggregates, with literal expected rows. Producer flags establish
//! replay for retained operators; the scan cases observe the switch to disk.
//! A separate component case supplies batches after destroying the scan owner.
//!
//! Retained-output cases use a private runtime hook to restart after an output
//! prefix or after completion. Memory replay must perform no I/O, and a second
//! replay request must fail. Damaging already-read disk bytes before replay must
//! expose corruption instead of reusing cached output. Cancellation must likewise
//! stop replay before it returns another row.
//!
//! Full-length text and the runnable event report exercise values across the
//! switch to disk. Explicit hash capacities isolate some controller cases from
//! admission policy; memory pressure drives the public cases. Each test releases
//! its owners and checks memory and temporary-file accounts.
...
fn grouping_consumes_computed_batches_and_requests_replay_without_a_scan() {
    ...
    for hash_groups in [1, 3] {
        // Build the consumer with the usual test helper, then drop the scan.
        // Supplying batches below must be enough: grouping cannot depend on a
        // hidden source owner to satisfy Input or Replay.
...
fn grouping_fallback_replays_sorted_producers_without_reopening_sources() {
    ...
    // Identify the retained producer to inspect alongside each SQL case and its
    // literal answer. LIMIT and computed stages must forward replay to it.
    for (producer, sql, expected) in [
        ...
        let general = &owner[0];
        let retry_peak = result.accounted_memory_bytes()
            + crate::storage::catalog::MAX_BYTES as u64
...
fn runtime_replays_retained_output_after_prefix_or_completion() {
    ...
    for mode in 0..5 {
        ...
        let disk_peak = if mode == 2 {
            ...
            initial.accounted_memory_bytes() + crate::storage::catalog::MAX_BYTES as u64
        ...
        for complete in [false, true] {
            ...
            assert!(reached);
            // Keep the workspace alive and request replay internally. Public
            // QueryResult completion would release this workspace.
...
fn aggregate_replay_rechecks_consumed_disk_bytes_and_cancellation() {
    ...
    let disk_peak = initial.accounted_memory_bytes() + crate::storage::catalog::MAX_BYTES as u64
        - owner[0].memory[0].memory_bytes();
    drop(initial);
    let pressure = database
        .reserve_memory(
    ...
    for cancelled in [false, true] {
        ...
        if cancelled {
            ...
            // Damage the first record after it was read successfully. Replay
            // must discard the reader cache and notice the changed header.
            scratch
...
fn downstream_hash_fallback_replays_retained_aggregate_input() {
    ...
    for disk in [false, true] {
        ...
        // Choose the upstream storage path and give the downstream hash table
        // one group slot. Its second distinct key must request upstream replay.
        // The prepared query and scheduler still drive both aggregates.
...
fn full_length_text_extrema_survive_hash_fallback_and_cancelled_reduction() {
    ...
    // Each long value fits a source batch by itself. Sorting puts both into
    // group 1, forcing reduction to fold its text buffer before the group ends.
    for (key, word) in [
    ...
    for scenario in [
        ...
        let general = &mut owner[0];
        // Three hash slots retain all groups; one slot forces input replay.
        // Keep the existing disk workspace when replacing optional hash storage.
        general.memory.clear();
        let groups = MemoryGroups::new(
            &database,
            &general.aggregate,
...
fn event_report_replays_nullable_joined_dates_and_labels() {
    ...
    let general = &owner[0];
    // Limit optional hash storage while retaining the disk workspace. The
    // observations below must confirm both disk use and replay of the join;
    // they do not depend on which year/label pair exhausts hash capacity.
    let retry_peak = result.accounted_memory_bytes() + crate::storage::catalog::MAX_BYTES as u64
        - general.memory.first().map_or(0, MemoryGroups::memory_bytes)
        + MemoryGroups::requirement(&general.aggregate, &general.keys, 1, 9)
            .unwrap()
            .1;
    ...
    // Dimension 2 has two labels, so its event contributes to two groups.
    // Missing dimension matches retain NULL labels through the left join.
    let expected = [
        (None, None, 1, 0, None),
