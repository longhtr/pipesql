//! Check grouped output through forced memory/disk paths and public SQL queries.
//!
//! Component cases use the parent module's small-buffer controller to force
//! replay, sorting and reduction. They record phases to establish which path ran
//! and compare nullable results with literal answers. Floating results without a
//! literal expectation must agree across paths and capture capacities.
//!
//! Public-query cases combine grouping with another aggregate, joins and derived
//! tables. Expected answers come from the small input rows; malformed syntax and
//! invalid name scopes must fail preparation. Wide text projections check that
//! result buffers fit the output, which can be much larger than an input record.
//!
//! A count over grouped output must preserve one row per group even when no
//! grouped values are retained. Separate nesting tests prepare and drop plans on
//! ordinary and reported small stacks; they do not execute those nested queries.

use super::*;
use crate::query::MAX_COLUMNS;
...
fn projected_maximum_text_keys_fit_the_reserved_result_frame() {
    for (width, temporary) in [(20, 8_000_000), (MAX_COLUMNS, 16_000_000)] {
        ...
        for hash_groups in [1, 2] {
            ...
            // Repeat the same maximum-width key across the output. Its result
            // frame is larger than the key-and-arguments record used by sorting.
        ...
        let minimum = initial.accounted_memory_bytes() + crate::storage::catalog::MAX_BYTES as u64
...
fn derived_inputs_bind_and_execute_independent_scopes() {
    ...
    let cancel = CancellationToken::new();
    let baseline = database.reserved_memory_bytes();
    // Key 1 totals 30 and joins two source rows; key 2 totals 90 and joins one.
    // The joined aggregate must therefore produce 30 * 2 + 90 = 150.
    for (sql, expected) in [
        (
    ...
    std::thread::scope(|scope| {
        thread
            .spawn_scoped(scope, || {
                ...
                for depth in 0..=crate::query::MAX_STAGES {
                    let query = database
                        .prepare(&sql)
...
fn zero_field_group_results_preserve_cardinality_through_hash_fallback() {
    ...
    let general = &owner[0];
    // The window needs the number of groups, not their keys or sums. Disk
    // records must still represent both groups despite having no value fields.
