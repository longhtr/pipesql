//! Check memory admission across grouping, its sources and downstream operators.
//!
//! Execute public queries at their complete minimum budget, then one byte below
//! it. The minimum must produce the literal expected rows; the shortfall must
//! refuse before any source I/O. Queries include successive grouped aggregates
//! and a join whose two inputs are separately grouped.
//!
//! The expected budget comes from live owners with optional capacity removed,
//! rather than the production combined-minimum formula. It includes catalog
//! scratch needed while opening sources, even though that scratch is gone by
//! the time execution returns its handle.
//!
//! A separate controller test reserves every free byte after each step, including
//! memory just released by hashing or sorting. Disk processing must finish using
//! its retained workspace. Teardown must restore memory and temporary-file
//! accounts. These checks concern reservations, not a process-memory bound.
...
fn joined_sources_are_admitted_before_grouping_growth_and_open_without_new_charge() {
    ...
    // Exercise opening and driving both admitted sources, with grouping on the
    // left. This component case does not run the join itself.
...
fn public_grouping_admits_the_exact_complete_minimum_before_io() {
    ...
    // The one-row numeric record is small enough to need no allocation padding.
    // Subtract observed capacities and their allocator allowances above that
    // minimum, not row/byte limits that could omit spare capacity.
    assert!(maximum_record <= 16_384);
    let run_extra = general
        .sort
    ...
    let minimum_peak = result.accounted_memory_bytes() + crate::storage::catalog::MAX_BYTES as u64
        - scalar_extra as u64
...
fn controller_retains_the_disk_minimum_under_competing_memory_pressure() {
    ...
    for skip_hash in [false, true] {
        ...
        let mut done = false;
        for _ in 0..2000 {
            // Model another query taking every released byte. Finishing on disk
            // must not depend on reserving the freed hash, path or run buffers
            // again.
            check_controller_account(&general);
            let available =
...
fn check_combined_grouping_minimum(sql: &str, expected: &[Vec<Option<i64>>], ordered: bool) {
    ...
    // Both aggregates must fit at once. Start from the live owners, add back
    // catalog scratch used during source opening, then remove optional capacity
    // from each aggregate. Do not reuse the combined-minimum sizing function.
    let mut minimum_peak =
