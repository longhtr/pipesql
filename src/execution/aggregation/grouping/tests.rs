//! Drive grouping through the real query scheduler with test-controlled limits.
//!
//! The fixture appends at most three rows per unit, so larger inputs and their
//! replay cross stored-unit boundaries. `connect` replaces a global aggregate
//! with a grouped controller whose small buffers force hash limits and disk
//! spills. `step` lends that controller to the runtime and takes it back for
//! inspection after each step, including a returned error.
//!
//! This setup deliberately bypasses normal GROUP BY preparation and admission.
//! The child suites own expected rows, failure schedules and checks through the
//! public query path. The helpers here provide source data, controller access and
//! resource comparisons; they do not supply an independent answer model.

use super::*;
use crate::effects::Faults;
use crate::execution::blocking::MAX_ARGUMENT_RECORD_BYTES;
use crate::execution::blocking::test_support::expected_buffer_charge;
...
// Keep the controller in its existing allocation while moving ownership between
// the test and runtime. The test can inspect private phases between steps.
...
// Reuse the prepared aggregate expressions, but add k as a grouping key and
// replace the first projected aggregate with that key. Shift the other output
// and filter positions past the new key column. This is a component-test setup,
// not a valid transformation offered by the query planner.
fn connect<'db>(
...
// These fixtures emit one group at a time with a non-NULL DOUBLE average. Keep
// its bits so comparisons catch a changed floating result, including zero signs.
...
fn check_controller_account(general: &General<'_>) {
    ...
    // This numeric SUM/AVG/COUNT fixture has no text or extrema buffers. Measure
    // its actual capacities and inline controller size independently of admission.
    // AggregateState charges only arrays; sorter and argument charges also include
    // their inline fields. Hash storage and pending scratch creation are separate.
    let expected = size_of::<General<'_>>()
        + expected_buffer_charge(bytes(&general.record.bytes))
        + bytes(&cells.values)
    ...
    assert_eq!(
        expected as u64, charged,
        "retained fields, capacities and allocator allowances are charged once"
    );
}
