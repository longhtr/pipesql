//! Check that grouping can construct its owners within the stated memory budget.
//!
//! Reserve the rest of the database budget, then construct disk-path buffers or
//! aggregate state with exactly the calculated amount. One byte below the minimum
//! must fail and release partial construction. These cases vary key types, widths,
//! requested aggregates and text argument buffers; they do not run disk sorting.
//!
//! Numeric accumulator checks measure live vector capacities separately from the
//! sizing formula. Wider expression workspaces may shrink under pressure, so the
//! refusal check uses the one-row minimum. A separate case checks that hash-table
//! sizing includes all nine numeric extrema in its query.
//!
//! These are component allocation and reservation checks, not process-memory
//! bounds. Public query admission and combinations of operators are covered by
//! `tests::admission`.

use super::*;
use crate::execution::blocking::test_support::{Directory, schema};
use crate::execution::*;
use crate::value::fixed_text::KEY_DOMAIN;
...
fn fallback_minimum_matches_constructed_owners_and_refusal_releases_them() {
    ...
    let database = Database::create_empty(
        &directory.0.join("db"),
        // The widest text output plus sort-buffer reuse allowances exceeds 16 MB.
        // Each case still admits its exact minimum and refuses one byte less.
        crate::Config::new(32_000_000, 16_000_000).unwrap(),
    )
    .unwrap();
            ...
            for groups in [1, KEY_DOMAIN * KEY_DOMAIN] {
                for lanes in [1, BATCH_ROWS] {
                    ...
                    owner
                        ...
                        .unwrap();
                    // Measure each live vector's capacity. Reusing the sizing
                    // formula here could repeat its mistake about shared sums
                    // or nullable counters in the expected answer.
                    ...
                    drop(pressure);
                    assert_eq!(database.reserved_memory_bytes(), baseline);
                    // One lane evaluates one row at a time. A wider workspace
                    // may shrink under pressure; the minimum cannot shrink again.
                    if lanes == 1 {
                        let pressure = database
                            .reserve_memory(
