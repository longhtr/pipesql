//! Accumulate groups in a bounded hash table while memory permits.
//!
//! Each distinct key gets a group number. The hash table maps encoded keys to
//! those numbers, and parallel arrays hold the groups' counts, sums and extrema.
//! Hashes locate candidates; comparing the actual keys decides equality.
//!
//! `begin` accepts a captured batch. Each `step` assigns one row to a group until
//! all positions are known, then reserves any needed text growth and folds the
//! arguments. Keys and group counts may already have changed when a limit is
//! reached. Fallback tells the parent controller to discard this owner and replay
//! the input through its reserved disk path; partial groups are never results.
//!
//! Group count, key bytes, collision work and text memory have separate limits.
//! Text growth retains both old and new reservations while copying in bounded
//! steps. Errors and cancellation during a step leave that operation failed.
//!
//! Once input ends, ordered output reuses the hash buckets as two arrays of group
//! numbers for merge sort. Keys and aggregate cells stay in place. `load_group`
//! copies one group's cells into the shared accumulator, which interprets final
//! values and reports aggregate overflow.
...
// Alignment makes each slot's width a power of two, as array_capacity requires.
// Memory reservations include this padding.
#[repr(align(16))]
#[derive(Clone, Copy)]
struct KeySlot {
    start: usize,
...
// Keep the replacement buffer and its reservation together until copying ends.
// The current text buffer remains usable and charged throughout the copy.
...
impl<'db> MemoryGroups<'db> {
    ...
    pub(super) fn capacities(
        ...
        // Split the remaining budget between encoded keys and group storage.
        // Budget up to four buckets per group to cover rounding: requirement()
        // rounds twice the group count up to a power of two.
        let per_group = cell_bytes + size_of::<KeySlot>() + 4 * size_of::<u32>();
        // Text extrema add a span per cell. Cap their group count so metadata
        // cannot grow with every input row; larger inputs can use the disk path.
        ...
        // Round group capacity down to a power of two. This keeps typed arrays
        // within the budget when array_capacity rounds their allocations up.
        let mut capacity = 1 << capacity.ilog2();
        let (_, mut arrays) = Self::requirement(aggregate, keys, capacity, 0)?;
        // Check actual rounded capacities before giving the remaining budget
        // to encoded keys.
        ...
        // array_capacity rounds large buffers up to a power of two. Round the
        // affordable key bytes down first so that allocation stays in budget.
    ...
    fn text_requirement(&self, arguments: &ArgumentBatch<'_>) -> Result<usize, Error> {
        ...
        // Reserve enough even if every wider value replaces its extremum.
        // Repeated rows for one group can overestimate growth: the first may
        // enlarge its region enough for later rows, or some may not replace it.
        // This estimate allows folding to proceed without allocating.
    ...
    // The inner result distinguishes a found/inserted group from a capacity
    // limit that permits replay. The outer error means this operation failed.
    fn lookup(&mut self, keys: &RowLayout, hash: u64) -> Result<Result<usize, HashLimit>, Error> {
        ...
        // Count bytes examined as well as bucket probes. Include encoding,
        // validation, hashing and a possible copy, then both keys for each
        // matching hash. A probe-count limit alone permits too much work when
        // stored keys are long.
    ...
    // Reuse the accumulator's final-value logic instead of implementing a second
    // interpretation of counts, scaled sums, NULLs and text extrema here.
...
// Round large allocations up, but keep Vec lengths at the requested cell count.
// Folding derives each group's width from those lengths, not spare capacity.
