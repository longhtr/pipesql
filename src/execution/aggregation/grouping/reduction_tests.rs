//! Check that disk sorting preserves the rows used by aggregate arithmetic.
//!
//! A literal batch mixes NULLs, large integers, infinities and NaN across five
//! groups. It runs through direct accumulation and through captured records,
//! small disk runs and sorted reduction. Results must agree, including floating
//! bits and overflow spans, across several capture capacities and expression
//! chunk sizes.
//!
//! Both paths use the production accumulator. Their comparison checks transport
//! and input order; the literal NULL, integer-sum and overflow assertions provide
//! separate expectations for those cases. Broader independent arithmetic checks
//! live in `numerical_tests`.
//!
//! Cancellation at three reducer states must make later steps fail without I/O.
//! A failed or short first read must consume no record. The buffers remain owned
//! until teardown, which must restore memory and temporary-file accounts. Empty
//! sorted input must finish without producing a group.
...
fn evaluated_sorted_arguments_use_the_existing_aggregate_kernel() {
    ...
    // The query supplies real bound aggregate expressions. Add a synthetic key
    // in the third batch column to isolate sorting and reduction from binding
    // the GROUP BY clause.
    ...
    for capacity in [1, 3, BATCH_ROWS] {
        for lanes in [1, 2, BATCH_ROWS] {
            ...
            .unwrap();
            // Keep the allocated scratch but vary how many rows are evaluated
            // together. Chunk boundaries must not change aggregate results.
            ...
            if capacity == BATCH_ROWS && lanes == BATCH_ROWS {
                ...
                // Unlike a global aggregate, a grouped aggregate has no result
                // when there is no key in the input.
                let mut empty_sort =
