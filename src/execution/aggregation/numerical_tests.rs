//! Check SUM and AVG against numbers computed outside the engine.
//!
//! `dev/src/oracle/rounding.rs` checks the retained inputs and answers using
//! rational arithmetic for SUM and bounded rounding intervals for overflow-prone
//! averages. These tests call the production arithmetic directly and check that
//! cases actually enter scaled-sum and running-mean states and leave scaling.
//!
//! The shared-state test also prepares a real aggregate plan and supplies batches
//! to its controller. Reading SUM must report overflow while reading AVG must
//! still return a finite value in the expected interval. This checks stored
//! results separately; a query requesting both results would fail on SUM.
//!
//! Run in release mode: the final loop adds the largest finite DOUBLE once per
//! permitted legacy row. Public query behavior belongs to the separate aggregate
//! semantic campaign, which runs the stock CLI.
...
fn shared_scaled_average_matches_independent_intervals() {
    ...
    let mut checked = 0;
    for line in include_str!("../../../test/data/aggregate-semantics/rounding.txt").lines() {
...
fn aggregate_states_match_independent_rational_vectors() {
    let vectors = include_str!("../../../test/data/aggregate-semantics/rounding.txt");
    ...
    let mut is_mean = false;
    for count in 2..=u32::try_from(format::MAX_ROWS).unwrap() {
