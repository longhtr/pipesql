//! Check captured arguments against literal counts, sums and extrema.
//!
//! These tests prepare real aggregate plans, then supply small typed batches
//! directly to capture and accumulation. COUNT shares a value with SUM or extrema
//! when needed; alone, it keeps only NULL presence. Input batches are dropped to
//! check that captured numeric and text values have their own storage.
//!
//! Invalid masks and cell mappings must fail validation. A demanded arithmetic
//! error must expose zero captured rows, while an unused expression must not be
//! evaluated. Exact and one-byte-short budgets check allocation refusal and the
//! release of reservations.
//!
//! The replay case encodes a SortRecord in memory and appends it through the
//! record-input path. It checks text capacity and retained values; disk I/O and
//! complete spill/reduction behavior are covered by the grouping tests.
...
fn argument_capture_rejects_mask_tails_before_reserving_memory() {
    ...
    for shape in [
        ...
        // A retained DATE uses integer day bits. A presence-only argument
        // discards that value and must not also claim to retain a DATE.
...
fn extrema_capture_keeps_values_and_validation_rejects_misdirected_slots() {
    ...
    // Point MIN at MAX's cell, remove MAX's cell, then assign a cell to an
    // unused argument. Validation must reconstruct which slots each result needs.
    ...
    // Dropping the input must not invalidate captured arguments. Fold them now
    // to detect an accidental borrow from the former batch.
    assert_eq!(arguments.value(0, 0), None);
...
fn argument_batches_preserve_demand_and_refuse_before_publication() {
    ...
    database
        ...
        .unwrap();
    // The hidden SUM would overflow on the rows below, but only COUNT(*) is
    // demanded. No argument values or expression scratch are needed for it.
    let query = database
    ...
    // Demand that same multiplication now. Its first NULL row succeeds, then
    // overflow must prevent the partially evaluated range from becoming usable.
    let query = database
    ...
    assert!(
        crate::query::scalar::NumericOutput::from_bits(
            &[0; BATCH_ROWS + 1],
...
fn text_capture_and_replay_stop_at_byte_capacity_before_row_capacity() {
    ...
    // There is room for more rows but no text bytes. Refusing the next record
    // must retain the first one so the caller can fold it before clearing.
    assert!(!arguments.can_append(&record).unwrap());
