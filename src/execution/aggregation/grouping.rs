//! Group rows by typed keys, with a disk path for inputs that exceed memory.
//!
//! The controller first reserves enough memory to sort rows and accumulate one
//! group at a time. It then tries to allocate a hash table, which can accumulate
//! many groups without writing files. Both paths ask the runtime for input batches
//! and use the same captured arguments and aggregate arithmetic.
//!
//! If the hash table reaches a capacity limit, discard it and replay the input
//! once. Sort each row's key and argument values by key, then by its original row
//! number. Equal keys can now be accumulated together in their original order.
//! Replaying rows instead of merging partial sums preserves floating arithmetic.
//!
//! No result leaves this controller until every retained group's demanded values
//! have been computed successfully. Hash results remain in memory. The disk path
//! writes completed results to a scratch file and checks each record's sequence,
//! shape and checksum when reading it for output. An I/O or corruption error can
//! therefore still occur during emission.
//!
//! Output can be replayed once without accumulating input again; disk replay
//! clears cached bytes so it checks the file again. A failed step is terminal.
//! The owned buffers, reservations and scratch files live until query cleanup.
...
use crate::execution::blocking::{
    ArgumentShape, Files, IO_MEMORY_BYTES, KeyColumn, MAX_FRAME_BYTES, MAX_KEY_BYTES, MAX_KEYS,
    RECORD_HEADER, ReadAt, RowLayout, RowSort, SortPhase, SortRecord, append_bytes, append_value,
    buffer_memory_bytes, read_value, run_allocation_bytes,
};
...
use crate::query::{AggregatePlan, MAX_ROW_VALUES, SemanticColumn, SourceColumn};
use crate::query::{Direction, NullPlacement};
...
// Map each grouping column to its input position and describe its encoded type.
// The layout checksum records key count, types and nullability for readers.
...
enum Phase {
    // Scratch creation is deferred until the disk path is needed.
    Create,
    Read,
    Await,
    Capture(usize),
    Hash,
    // A captured row stays pending across a full sort buffer and its spill.
    Encode(usize),
    Push(usize),
    Spill(usize),
    Sort,
    ...
    Flush,
    Order,
    // Check all in-memory results before emitting any of them.
    Check(usize),
    EmitMemory(usize),
    EmitDisk,
    Done,
...
impl OutputLayout {
    ...
    fn from_columns(
        ...
        // COUNT(*) downstream may need only the number of groups. Even with
        // no value fields, one frame per group preserves that number on disk.
...
impl<'db> Minimum<'db> {
    fn new(
        ...
        let bytes = inline
            .checked_add(buffer_memory_bytes(frame_bytes)?)
    ...
    fn required(
        ...
        [
            size_of::<General<'_>>(),
            buffer_memory_bytes(record.max(output.max_bytes))?,
            shape.max_payload_bytes(),
            run_allocation_bytes(record, 1)?,
            2 * buffer_memory_bytes(record)?,
            3 * IO_MEMORY_BYTES,
            buffer_memory_bytes(keys.max_bytes)?,
            crate::storage::scratch::Creation::memory_requirement_bytes() as usize,
        ]
        .into_iter()
...
pub(in crate::execution) struct General<'db> {
    // Empty on the disk path, otherwise one separately allocated hash owner.
    // Discarding it releases its memory while preserving the disk workspace.
...
impl<'db> General<'db> {
    ...
    fn assemble(
        ...
        // A refused hash-table reservation leaves the disk workspace usable.
        // Other errors still fail construction.
    ...
    // A rejected group does not demand its projected expressions. Check the
    // predicate first so an unused overflowing SUM does not fail the query.
    ...
    fn read_result(
        ...
        bytes[28..32].fill(0);
        let actual = format::crc32c(bytes);
    ...
    pub(in crate::execution) fn step(
        ...
        // Restore a next phase only after the whole step succeeds. Any early
        // error leaves the controller failed, including errors after mutation.
        let phase = std::mem::replace(&mut self.phase, Phase::Failed);
        output.clear();
        ...
        self.phase = match phase {
            ...
            Phase::Hash => {
                match self.memory[0].step(input.batch, &self.arguments, &self.keys, cancel)? {
                    ...
                    HashStep::Fallback(_) => {
                        // Partial hash totals cannot be merged into the disk path:
                        // that would change floating addition order. Start over
                        // from the child producer's retained input instead.
