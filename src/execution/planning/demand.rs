//! Find which logical columns each stage needs from its inputs.
//!
//! Start with the query's output columns and walk backward. Each stage adds the
//! values needed to produce that output: a filter adds its predicate column,
//! sorting adds its keys, and a computed projection adds its expression inputs.
//! A mask is a set of column identities for one relation, not a set of row positions.
//!
//! Row comparison may need columns hidden by a later SELECT. DISTINCT needs the
//! complete comparison row to decide which rows are duplicates. UNION ALL does
//! no comparison and only needs the positions used later in the query.
//!
//! These sets guide buffer sizing and source-column selection. They include
//! possible conditional inputs; evaluation still skips unused branches for each
//! row. Lowering and physical validation share this analysis, so independent SQL
//! results must check the semantics beyond their shared assumptions.

use super::MAX_PIPELINES;
use crate::Error;
use crate::query::{self, ColumnId, RelationId, Stage};

pub(super) use crate::query::ColumnSet;
...
pub(super) fn demand_masks(plan: &query::Plan) -> Result<[ColumnSet; MAX_PIPELINES], Error> {
        ...
        match node.stage {
            Stage::SetOperation { right, descriptor } => {
                ...
                for position in 0..usize::from(node.columns) {
                    ...
                    if set.kind() != query::SetKind::UnionAll || output.contains(column.identity())
                    {
            ...
            Stage::Join {
                ...
                // A LEFT JOIN gives right-side columns new nullable identities.
                // Translate those identities back before requesting source values.
            ...
            Stage::Aggregate(aggregate_index) => {
                ...
                // Hidden group keys still separate groups. Aggregate arguments,
                // by contrast, are needed only for results used by later stages.
