//! Choose how to group rows and produce one aggregate result per group.
//!
//! For legacy one-byte keys, `Groups` assigns an array slot to every possible key
//! combination. No hash lookup is needed. A global aggregate also uses this path
//! with one slot, even for empty input. Other grouped queries go through
//! `grouping::General`, which handles arbitrary typed keys and can spill to disk.
//!
//! The dense controller here asks the runtime for batches, maps each row to its
//! group slot and passes it to `AggregateState`. The accumulator owns arithmetic
//! and expression storage; this controller owns when to consume, check and emit.
//!
//! After input ends, Check evaluates every retained group's demanded results
//! before Emit returns any rows. An overflow in a later group therefore cannot
//! follow an apparently successful result prefix. Replay emits these retained
//! results once more without accumulating the input again.
...
use crate::value::fixed_text::{KEY_DOMAIN, StringValue as FixedKey};
...
// The runtime stores these controllers in aggregate-plan order. Dense state is
// inline; General owns a separate one-element allocation so its larger fallback
// machinery does not enlarge every controller slot.
#[expect(
...
impl<'db> Groups<'db> {
    ...
    fn consume(&mut self, batch: &Batch) -> Result<(), Error> {
        ...
        // Encode the key tuple as digits in base KEY_DOMAIN. With two keys,
        // their indices a and b select slot a * KEY_DOMAIN + b. Keep each row
        // with its group's updated count for the accumulator's incremental folds.
    ...
    pub(super) fn step(
        ...
        match self.phase {
            // Check the same filters and demanded output expressions that Emit
            // will use. A filtered-out group need not produce its output values.
    ...
    fn value(&self, group: usize, column: usize) -> Result<Value<'_>, Error> {
        ...
        // Reverse the slot calculation to recover fixed keys; the dense table
        // does not need a separate key allocation for every group.
        if column < keys {
