//! Read table rows, apply filters and fill batches for the next query operator.
//!
//! `ScanCursor::advance` works in three phases. Begin selects a bounded range of
//! source rows. Filter removes rows or follows each row's Boolean branch. Output
//! copies the retained rows into a batch. Calls return progress between reads
//! and phases, allowing the runtime to check cancellation and schedule more work.
//!
//! A filter branch reached by no row skips its column reads. For a branch that
//! is reached, the scan loads the expression's possible input columns before
//! evaluation; scalar short-circuiting then skips unused calculations. These are
//! separate decisions: skipping a calculation does not always skip its input I/O.
//!
//! The declared and legacy sources decode their respective storage formats. This
//! controller owns row selection and computation scratch; the runtime owns the
//! returned batch and makes errors terminal. If text fills a batch, Output emits
//! only complete rows and retries the next row in an empty batch.
...
use crate::query::{MAX_COLUMNS, MAX_ROW_VALUES};
...
pub(super) struct ScanCursor<'db> {
    ...
    phase: ScanPhase,
    // Drop source buffers and scratch before releasing their memory budget.
...
impl ScanCursor<'_> {
    ...
    pub(super) fn restart(&mut self, cancel: &CancellationToken) -> Result<(), Error> {
        ...
        // Replaying storage alone is insufficient: selected row numbers and
        // computed values still refer to the previous pass.
    ...
    fn filter_branch(
        ...
        // Only rows whose next decision is this filter can require its values.
        // An empty branch must not load a column just to discover it is unused.
        if !self.branches.selected.is_empty() {
            ...
            result?;
            // Filtering preserves row order. Merge the surviving subset back
            // into the original selection to choose each row's next decision.
            let mut matched = 0;
    ...
    fn decode_source(
        ...
        // A later text constant can require UTF-8 cells even for legacy input.
        // Use typed values here because the legacy bulk decoder writes fixed
        // string keys and cannot fill that variable-length layout.
            ...
            Source::Declared(owner) => {
                ...
                for index in start..end {
                    for (position, column) in plan.columns[..plan.column_count].iter().enumerate() {
                        ...
                        match input.set(index - start, position, value) {
                            // Earlier rows are complete, but this row may be
                            // partially written. Publish only the earlier rows;
                            // advance clears the batch before retrying this one.
                            Err(Error::Resource {
                                owner: "batch UTF-8 bytes",
                                ..
                            }) if index > start => return Ok(index),
    ...
    pub(super) fn advance(
        ...
        match self.phase {
            ...
            ScanPhase::Output(start) => {
                // Yield after loading one column. Reenter this phase to load
                // any remaining inputs before assembling the output batch.
