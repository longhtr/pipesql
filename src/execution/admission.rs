//! Turn a prepared query into an execution with its required resources reserved.
//!
//! `Database::execute` checks that the prepared query belongs to this database
//! and still refers to a usable snapshot. It then translates logical column
//! identities into positions in execution batches. A separate validator checks
//! that translation before the execution starts reading table values.
//!
//! Runtime construction reserves required buffers before allowing optional
//! growth. Otherwise an early operator could take space a later operator needs
//! to run at all. If construction fails, the owners already created are dropped.
//! A returned result is ready to step; it has not necessarily read any rows yet.
//!
//! Declared-table queries use the snapshot held by their prepared query, even
//! while an append builds newer data. Legacy queries have one published data set
//! and must check its current files. They refuse execution while publication or
...
use crate::storage::recovery::inspect_query_namespace;
use crate::{CancellationToken, Database, Error};

impl Database {
    /// Upper bound for one scan's workspace and execution setup.
    /// Includes controller metadata, physical mappings, transient scratch and the
    /// result handle. Add the database, prepared query and any aggregate, join,
    /// sort or additional output buffers separately. This is not a memory bound
    ...
    /// Start an execution of a prepared query against the snapshot it retained.
    ///
    /// The result borrows the database, query, and cancellation token. Successful
    /// construction does not mean the query completed: call [`crate::QueryResult::step`]
    /// until [`crate::QueryStep::Finished`] or handle its terminal failure.
    ///
    ...
    pub(in crate::execution) fn execute_with_effects<'db, 'cancel>(
        ...
        // A declared query retains its committed snapshot. Space reserved by
        // an unfinished append does not change the data that query can read.
        ...
        let state = match namespace.state {
            ...
            RootState::Data {
                ...
                let layout =
                    format::layout(rows).map_err(|_| Error::Corrupt("invalid stored row count"))?;
