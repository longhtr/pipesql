//! Select a range of input rows for LIMIT and OFFSET.
//!
//! Skip OFFSET rows, then take at most LIMIT rows. Apply later filters only to
//! that selection: `LIMIT 2 |> WHERE x > 0` examines the first two rows even if
//! neither passes. Replacing rejected rows would change the meaning of the pipe.
//!
//! This controller keeps the remaining skip and take counts. The runtime owns
//! its input cursor and output batch, and supplies one input batch at a time.
//! Replay resets both counts; the runtime must also rewind the input to select
//! the same range again. A failed controller cannot replay.
...
impl Limit {
    pub(super) fn new(bounds: query::LimitBounds) -> Self {
        Self {
            bounds,
    ...
    pub(super) fn step(
        ...
        match phase {
            ...
            Phase::Read => {
                // OFFSET still consumes input when LIMIT is zero. Keep the two
                // counters separate: their sum could exceed the SQL INT64 range.
            ...
            Phase::Await => {
                ...
                // Spend the quota before filtering. A later WHERE may discard
                // selected rows, but it must not cause LIMIT to select more.
