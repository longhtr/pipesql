...
pub(super) struct Union {
    // Raw union positions map to each child's demanded payload. u8::MAX marks
    // an undemanded position; evaluating it is a physical-plan defect.
        ...
        match phase {
            ...
            Phase::Read => {
                ...
                if self.replay_pending & bit != 0 {
                    // Rewind only branches previously visited. A LIMIT can
