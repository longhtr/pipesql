//! Check success-history copying, lookup and refusal of damaged evidence.
//!
//! Histories with gaps distinguish transaction numbers from committed generations.
//! A copy crosses the read-block boundary and reaches the final u64 attempt.
//! Mutated headers and entries, interrupted I/O and exhausted capacity must return
//! errors rather than a usable reference or a successful lookup.
//!
//! These tests run the real reader and writer on temporary files. Fixture encoding
//! shares production field helpers, so it is not an independent format oracle.
//! Expected transaction membership and generations are stated in each case.
...
impl Fixture {
    ...
    // Build every-other-attempt successes directly, without exercising append.
    // install recomputes the checksum, including for deliberate semantic damage.
...
fn copied_history_crosses_blocks_and_preserves_final_sequence() {
    ...
    // A 64-KiB block holds 8,192 sequences. One more forces a second read.
...
fn malformed_history_and_full_capacity_never_publish_a_reference() {
    ...
    for (at, value) in [
        ...
        bad[at] = value;
        // Keep the checksum valid so field/order checks must detect the defect.
        // Looking up the first entry must not bypass a bad later entry.
    ...
    // No full history file exists: the declared capacity must refuse before
    // opening it or needing scratch, leaving the supplied output file empty.
