//! Check column encoding, selective reads and failures while constructing a unit.
//!
//! Cases use real files and compare typed values, NULLs, UTF-8 and exact numeric
//! bits. Input, schema and file orders deliberately differ to expose readers that
//! confuse a column's ID with its position. A damaged string payload must not
//! prevent reading a different column, but reading that string must fail.
//!
//! Retained binary fixtures provide answers independent of the Rust codecs.
//! Other cases give malformed metadata or values correct checksums, requiring
//! structural validation to reject them. Capacity and I/O failures must return
//! no unit reference or usable column view; partial files remain caller-owned.

use super::*;
use crate::effects::Faults;
use crate::storage::schema::{ColumnSpec, TableId};
...
fn schema_bytes(columns: &[ColumnSpec<'_>]) -> Vec<u8> {
    let mut bytes = vec![0; schema::MAX_BYTES];
    let length = schema::encode(&mut bytes, database(), table(), columns).unwrap();
    bytes.truncate(length);
    bytes
}

fn schema(bytes: &[u8]) -> Schema<'_> {
    schema::decode(bytes, database(), table(), crc32c(bytes)).unwrap()
}
...
fn data<'a>(
    ...
    // Store count first although specs declares note first. Positional matching
    // would read the wrong type or value; the persistent IDs must choose columns.
    [
        InputColumn {
            id: id(42),
            values: ColumnValues::Int64(ints),
            validity: &[0b1111],
        },
        InputColumn {
            id: id(9),
...
fn demanded_corruption_fails_without_reading_other_columns() {
    ...
    // The second physical payload is text. Damage it without changing metadata,
    // leaving the integer column available to a query that does not request text.
...
fn construction_failures_never_return_a_unit_reference() {
    ...
    let input = [InputColumn {
        id: id(2),
        values: ColumnValues::Double(&[1., 2.]),
        validity: &[1],
    }];
...
fn catalog_schema_drives_independent_native_bytes_and_reads() {
    let fixture = Fixture::new();
    let catalog_bytes = include_bytes!("../../../test/data/catalog-schema/catalog.bin");
    let schema_bytes = include_bytes!("../../../test/data/catalog-schema/columns.bin");
    let expected = include_bytes!("../../../test/data/catalog-schema/native-unit.bin");
    for (object, bytes) in [
...
fn matching_checksums_do_not_authorize_bad_metadata_or_text() {
    let fixture = Fixture::new();
    let original = include_bytes!("../../../test/data/catalog-schema/native-unit.bin");
    let schema_bytes = include_bytes!("../../../test/data/catalog-schema/columns.bin");
    ...
    let mut payload = [0; 64];
    let cancel = CancellationToken::new();
    // Replace the metadata checksum too: each changed field must be rejected
    // on its own meaning, not merely because the bytes differ.
    for (at, value) in [
        (12, 1),
        (16, 8),
        (32, 1),
    ...
    // Repair both checksum levels after corrupting a payload. Metadata can now
    // pass; reading the column must reject its bitmap, offsets or UTF-8.
...
fn exact_row_text_byte_and_column_limits_are_constructible() {
    ...
    // Eight strings need one bitmap byte and nine four-byte offsets: 37 bytes
    // of overhead. Shorten the last string by exactly 37 bytes to fill the column.
...
fn retained_payload_invalidates_before_failed_refill() {
    ...
    // Successful reuse does not make future bytes trustworthy. A checksum
    // failure on refill must remove the previous column view too.
    let mut file = std::fs::read(fixture.path()).unwrap();
    let offset = u64_at(&file, HEADER + DESCRIPTOR + 8) as usize;
    file[offset] ^= 1;
    std::fs::write(fixture.path(), file).unwrap();
