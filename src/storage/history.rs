//! Keep a record of successful attempts so uncertain commits can be resolved.
//!
//! Transaction numbers advance even when an attempt aborts. The success index
//! stores only successful numbers in increasing order; their one-based position
//! gives the committed generation. For example, the list [1, 3] means attempt 3
//! created generation 2, while attempt 2 did not commit in that snapshot.
//!
//! `append` copies validated history into a new file and adds the latest issued
//! attempt. Old snapshots keep their original file. The caller owns file creation,
//! memory reservations, synchronization and cleanup; returning a reference here
//! does not publish or make the new history durable.
//!
//! `find` reads and validates the entire history even if it finds a match early.
//! A valid prefix cannot excuse later corruption. A missing entry alone does not
//! prove an abort: the live resolver also checks whether the attempt is active.

use crate::effects::{Effect, Effects, MetadataKind};
use crate::storage::catalog::{self, ObjectId, ObjectRef};
use crate::storage::format::{CatalogCommit, Crc32c, MAX_SUCCESSES, RootState, WalRecord, crc32c};
...
impl Reader {
    ...
    // Exhaustion verifies both the full checksum and the last successful attempt.
    // Callers must reach None before accepting membership or publishing a copy;
    // blocks already returned can still belong to a corrupt file.
    fn next(
        &mut self,
        buffer: &mut [u8],
        cancel: &CancellationToken,
...
fn write_verified(
    ...
    cancel.check()?;
    // Save the intended checksum before readback overwrites the same buffer.
    // Comparing only the returned bytes with themselves would miss a bad write.
...
pub(super) fn append(
    ...
    // The body was copied only after checking its order, and the old reader has
    // now verified its complete checksum. Write the new header last; failures
    // still leave a private file for the caller to remove.
    ...
    while let Some(length) = reader.next(buffer, cancel, effects)? {
        for bytes in buffer[..length].as_chunks::<8>().0 {
            ...
            if u64::from_le_bytes(*bytes) == transaction.sequence() {
                // Save the match but keep reading to verify the remaining file.
                found = Some(ordinal);
            }
        }
    }
