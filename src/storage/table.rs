//! List the data units that make up a table, in row order.
//!
//! A unit stores one batch of rows as column payloads. This index records each
//! unit's file reference, row count and starting row in the table. Row ranges
//! must join without gaps or overlaps. A new index can retain references to old
//! units and add new ones without copying their payloads.
//!
//! `write` builds a private index in the caller's file. `open` reads the complete
//! index, checks its entries and verifies the checksum supplied by the catalog.
//! It returns no cursor for an empty table, which has no index file.
//!
//! A cursor keeps one page of entries in memory and a checksum for each page.
//! Before returning entries from a newly read page, it compares that page with
//! the checksum saved during opening. Rewind discards the cached page but keeps
//! those checksums, so a second pass cannot silently accept changed bytes. A
//! failed read or cancellation ends the cursor; later calls cannot resume it.

use crate::DatabaseId;
use crate::effects::{Effect, Effects, MetadataKind};
use crate::storage::catalog::{self, MAX_UNITS, ObjectId, ObjectRef, TableEntry};
use crate::storage::format::{Crc32c, FormatError, crc32c};
use crate::storage::schema::Schema;
use crate::storage::unit::UnitRef;
...
// The caller supplies an empty, exclusively owned file and reserved scratch.
// On success it must synchronize the file before publishing its reference. On
// failure it must remove the partial file; this function does not perform cleanup.
pub(super) fn write(
    file: &File,
    object: ObjectId,
    schema: &Schema<'_>,
    ...
    for (page, entries) in units.chunks(PAGE_ENTRIES).enumerate() {
        ...
        // Preserve checksums of the intended bytes before readback replaces the
        // buffer. Otherwise damaged output could become its own expected answer.
...
// Page bytes and checksums are inline, with no allocation when a page changes.
// The owner retaining this cursor must account for the size of the whole value.
...
pub(super) fn open(
    ...
    let mut whole = Crc32c::new();
    whole.update(header);
    // These page checksums become trustworthy only after the complete file
    // matches the catalog reference below. Do not expose a cursor before then.
...
impl Cursor {
    pub(crate) fn rewind(&mut self) -> Result<(), Error> {
        ...
        // Keep the file and original checksums, but force a new page read.
        // Reusing the cached page would hide changes since the previous pass.
        self.index = 0;
    ...
    pub(crate) fn next(
        ...
        // Every early error leaves the cursor failed. Restore Active only after
        // a complete entry is ready; a later call must not skip a failed read.
