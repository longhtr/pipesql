//! Test which catalog files may be trusted when a database opens.
//!
//! A root names the committed catalog, which in turn names schemas and data
//! files. Recovery must validate those files before repairing a stale root.
//! When the surviving root cannot account for the recorded commit, open must
//! refuse and leave the files intact for a later recovery attempt.
//!
//! Retained binary fixtures check encoding and history against fixed bytes.
//! Other tests build databases through production code, then damage files or
//! inject I/O failures. They check rejection, memory admission and lease release;
//! healthy retries distinguish a rejected open from a database left unusable.
...
use crate::storage::format::{self, CatalogCommit, Replica, Root};
use crate::storage::recovery::{ROOT_A_NAME, ROOT_B_NAME, UNITS_NAME, WAL_NAME};
use crate::storage::schema::TableId;
...
fn independent_catalog_roots_reach_the_complete_graph_and_history() {
    ...
    for (id, bytes) in [
        ...
        (
            object(5, 3),
            include_bytes!("../../../../test/data/catalog-roots/table-data.bin").as_slice(),
        ),
        (
            object(5, 4),
            include_bytes!("../../../../test/data/catalog-roots/catalog.bin").as_slice(),
        ),
        (
            object(5, 5),
            include_bytes!("../../../../test/data/catalog-roots/successes.bin").as_slice(),
        ),
    ] {
    ...
    for (name, bytes) in [
        ...
        (
            ROOT_B_NAME,
            include_bytes!("../../../../test/data/catalog-roots/ROOT.B").as_slice(),
        ),
        (
            WAL_NAME,
    ...
    for (replica, bytes) in [
        ...
        let root = format::decode_root(bytes).unwrap();
        assert_eq!(root.replica, replica);
        assert_eq!(format::encode_root(root).unwrap(), bytes);
    }
    assert_eq!(
        format::encode_wal(snapshot).unwrap(),
        include_bytes!("../../../../test/data/catalog-roots/WAL").as_slice()
    );
    let genesis_bytes = include_bytes!("../../../../test/data/catalog-roots/GENESIS");
    let root = format::decode_root(genesis_bytes).unwrap();
    assert_eq!(root.fence(), genesis());
    assert_eq!(format::encode_root(root).unwrap(), genesis_bytes.as_slice());
}

#[test]
fn catalog_root_fields_and_history_shape_fail_with_matching_checksums() {
    let original = include_bytes!("../../../../test/data/catalog-roots/ROOT.A");
    // Alter reserved bytes, generation, transaction identity and object
    // references. Recompute the CRC so rejection must come from field validation.
    for (at, value) in [
        (33, 1),
        (40, 0),
        (48, 1),
        ...
        bad[at] = value;
        bad[108..112].fill(0);
        let crc = format::crc32c(&bad);
        bad[108..112].copy_from_slice(&crc.to_le_bytes());
        assert!(format::decode_root(&bad).is_err(), "mutation {at}");
    }
    for length in 0..original.len() {
        assert!(format::decode_root(&original[..length]).is_err());
    }
    let mut newer = *original;
    newer[8..12].copy_from_slice(&(format::CATALOG_FORMAT_VERSION + 1).to_le_bytes());
    assert!(matches!(
        format::decode_root(&newer[..12]),
        Err(format::FormatError::Version)
    ));
    let catalog = graph(format::decode_root(original).unwrap().fence()).catalog();
    let successes = graph(format::decode_root(original).unwrap().fence()).successes();
...
fn catalog_open_preserves_unresolved_graph_when_only_stale_root_survives() {
    // Compare names and file contents in each fixture directory. Checking only
    // the known roots would miss an unexpected file created during a refused open.
    fn namespace(path: &Path) -> Vec<(PathBuf, Option<Vec<u8>>)> {
        let mut entries = Vec::new();
        for directory in [
            Path::new(""),
            Path::new(UNITS_NAME),
            Path::new(crate::storage::recovery::PRIVATE_NAME),
        ] {
    ...
    for prior_table in [false, true] {
        for (stale_name, stale_replica, damaged_name) in [
            ...
            for missing in [false, true] {
                ...
                let mut stale =
                    format::decode_root(&fs::read(path.join(stale_name)).unwrap()).unwrap();
                let commit = db.declare_table("facts", &columns, &cancel).unwrap();
                db.close().unwrap();
                let newest = fs::read(path.join(damaged_name)).unwrap();
                // Recreate the peer just before commit replaced the first root:
                // it records the issued transaction but still names the old catalog.
                stale.issued = commit.transaction().sequence();
                stale.replica = stale_replica;
                fs::write(path.join(stale_name), format::encode_root(stale).unwrap()).unwrap();
                if missing {
                ...
                // Restore only the damaged root. Successful open now shows that
                // the refusals released the lock and preserved the committed files.
...
fn selected_graph_damage_refuses_before_root_repair() {
    ...
    // Leave ROOT.B one commit behind. Selection still chooses the new catalog,
    // but recovery must validate its children before replacing ROOT.B.
    let older = format::encode_root(Root {
        database: database(),
...
fn graph_admission_bounds_and_effect_failures_are_read_only() {
    ...
    for (mode, cut) in (0..3).flat_map(|mode| (0..baseline.count()).map(move |cut| (mode, cut))) {
        ...
        match mode {
            ...
            // Shortening has no effect at operations without a byte transfer.
            1 => assert!(
    ...
    // Refuse a nonempty graph with one byte too little scratch before I/O.
    // An empty catalog needs neither that workspace nor any file reads.
...
fn database_open_resolves_catalog_history_and_owns_its_lease() {
    ...
    let (second, _) = fixture.prepare(issued, Some(unit));
    publish(&fixture, issued, second);
    // Add 80 objects from an older attempt so directory inspection must handle
    // more than 64 entries. These names are valid even though no table uses them.
    ...
    let config = crate::Config::new(
        path_bytes
            ...
            + crate::storage::snapshot::REGISTRY_BYTES,
        1_000_000,
    )
    .unwrap();
    ...
    // Opening and receipt lookup fit the budget; preparation needs another
    // 8,192 bytes for two paths. It must refuse for memory before reaching the
    // missing-table error, which the larger-budget control below must report.
    assert!(matches!(
...
fn catalog_namespace_names_and_object_capacity_are_bounded() {
    ...
    .unwrap();
    // Two valid names may not share an inode: each object must own its file.
    ...
    // A well-formed name still fails if its attempt was never issued.
...
fn catalog_bootstrap_builds_a_fresh_database_and_reopens_values() {
    assert_eq!(
        format::encode_catalog_control(database()),
...
fn catalog_bootstrap_effect_failures_clean_before_retry() {
    ...
    assert!(count < 128);
    // Fail each effect observed during creation. A failed create must remove
    // its directory and release the lease so the same pathname can be retried.
    for cut in 0..count {
