//! Store a batch of table rows so each column can be read separately.
//!
//! A unit file starts with metadata, followed by one payload per column. Each
//! column descriptor records its persistent ID, type, location and checksum.
//! Readers use the ID to match the schema, so file order need not match schema
//! order. The table-data index stores a `UnitRef` with the metadata checksum.
//!
//! `read` validates that metadata and returns a `Unit` owning the open file.
//! `read_column` then reads and validates one column into a caller-owned buffer.
//! A query selecting only an amount column need not read a description column;
//! damage to that unread payload is detected when it is read, not at unit open.
//!
//! Writers use `requirements` to size their buffers, then reserve resources.
//! `write` checks those buffers, encodes each column and reads back the result.
//! It returns a reference to the new unit. The caller must still synchronize and
//! publish the file, or remove it if construction fails.
...
use crate::storage::schema::{self, ColumnId, ColumnSpec, Schema};
...
const FORMAT: u32 = 6;
pub(crate) const MAX_ROWS: usize = 32_768;
pub(crate) const MAX_COLUMN_BYTES: usize = 524_288;
// The allocator may need padding beyond the encoded bytes. Reserve that space
// without allowing a larger column in the file format.
pub(crate) const MAX_COLUMN_ALLOCATION_BYTES: usize =
...
pub(super) const MAX_METADATA_BYTES: usize = HEADER + DESCRIPTOR * schema::MAX_COLUMNS;
pub(super) const MAX_UNIT_BYTES: usize =
    MAX_METADATA_BYTES + schema::MAX_COLUMNS * MAX_COLUMN_BYTES;
...
// The index stores a checksum of the metadata; each descriptor stores a column
// checksum.
// Separate checks let readers verify only the columns they need.
...
#[derive(Clone, Copy)]
pub(crate) struct InputColumn<'a> {
    pub(crate) id: ColumnId,
    pub(crate) values: ColumnValues<'a>,
    // One bit per row: 1 means present, 0 means NULL. Row 0 uses the low bit
    // of byte 0. Bits beyond the last row must be zero.
    pub(crate) validity: &'a [u8],
}
...
// Reserve enough for one whole column, including the NULL bitmap. Numeric and
// DATE columns have a tighter bound than variable-length strings.
pub(crate) fn column_capacity(kind: DataType) -> usize {
...
fn payload_length(
    ...
    let values = if let Some(width) = fixed_width(spec.data_type()) {
        ...
        let ColumnValues::String(strings) = &column.values else {
...
// Every payload begins with its validity bitmap. Fixed-width values follow it
// directly, in little-endian order. Strings store a four-byte offset per row
// and a final end offset, followed by concatenated UTF-8. Adjacent offsets delimit
// a value. NULL strings consume no text bytes, while
// NULL fixed-width slots remain zero regardless of the caller's unused value.
fn encode_payload(out: &mut [u8], input: &InputColumn<'_>, rows: usize) {
    ...
    if let ColumnValues::String(strings) = &input.values {
        ...
        for row in 0..rows {
            if valid(input.validity, row) {
                ...
                match &input.values {
                    ...
                    ColumnValues::Date(v) => out[start..start + 4]
                        .copy_from_slice(&v[row].days_since_unix_epoch().to_le_bytes()),
                    ColumnValues::String(_) => unreachable!("fixed input kind"),
                }
            }
        }
    }
...
// Size the two reusable buffers and the output file before allocating or
// creating it. column_bytes is the largest single payload, not their sum. These
// numbers do not reserve resources; the caller obtains those reservations.
...
// Validate every input and buffer before touching the file. Once writing starts,
// any error can leave partial bytes in the caller's private file. The caller owns
// cleanup and must not treat a returned UnitRef as a committed transaction.
pub(super) fn write(
    file: &File,
    object: ObjectId,
    schema: &Schema<'_>,
    ...
    for (index, input) in columns.iter().enumerate() {
        ...
        // Save the intended checksum before readback overwrites scratch. A
        // checksum computed only afterward could accept a damaged write.
        ...
        entry[4] = schema::tag(column.data_type());
...
fn validate_metadata(
    ...
    zero(&bytes[60..64])?;
    // Every schema column must occur exactly once. Unique IDs plus the checked
    // count establish that even when descriptor order differs from the schema.
    ...
    for (index, entry) in entries.iter().enumerate() {
        ...
        if entry[4] != schema::tag(column.data_type())
            || entry[5] != u8::from(column.nullable())
        ...
        // Requiring each offset to follow the previous payload excludes gaps
        // and overlaps; the final offset must also match the complete file size.
        offset = offset
...
// Keep metadata inline with the file handle. A scan can move or pause this
// reader without borrowing a temporary read buffer. Its owner reserves memory
// for the complete Unit value, while column buffers are accounted separately.
...
impl Unit {
    pub(super) fn read_column<'a>(
        ...
        let kind = schema::data_type(entry[4]).expect("validated descriptor kind");
        let column = Column {
            bytes: &buffer[..length],
            rows: self.rows,
            kind,
...
// Own reusable column bytes and record whether their last read succeeded. The
// query reserves this value and the supplied allocation before construction;
// this type neither grows the buffer nor holds a separate reservation.
...
impl ColumnBuffer {
    ...
    pub(crate) fn read(
        ...
        // Invalidate the old view even if the new read fails before touching
        // bytes. Otherwise the caller could mistake the old column for this one.
        self.validated = None;
    ...
    pub(crate) fn column(&self) -> Option<Column<'_>> {
...
impl<'a> Column<'a> {
    // A matching checksum does not prove that offsets, NULL slots or DATE values
    // obey the format. Validate them before any typed accessor can borrow bytes.
    ...
    pub(crate) fn int64(&self, row: usize) -> Result<Option<i64>, FormatError> {
    ...
    pub(crate) fn double(&self, row: usize) -> Result<Option<f64>, FormatError> {
    ...
    pub(crate) fn date(&self, row: usize) -> Result<Option<DateValue>, FormatError> {
    ...
    pub(crate) fn string(&self, row: usize) -> Result<Option<&'a str>, FormatError> {
