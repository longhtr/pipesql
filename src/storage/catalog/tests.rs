//! Check catalog lookup and schema reads against valid and deliberately damaged files.
//!
//! A healthy read must preserve table and column IDs. The failure cases replace
//! files during opening, substitute another table's schema, interrupt I/O and
//! cancel reads. Each must return an error before exposing an accepted schema.
//! Buffer limits and invalid file sizes must stop the read before content I/O.
//!
//! Byte mutations carry fresh checksums to test the field rules themselves.
//! Separate retained binary fixtures constrain the codecs with independently
//! encoded bytes and literal checksums. These tests cover catalog readers and
//! encoders; they do not publish a root or establish crash durability.

use super::*;
use crate::effects::Faults;
use crate::storage::schema::{ColumnId, ColumnSpec};
...
impl Fixture {
    ...
    // Install exact test bytes directly. This bypasses synchronization and root
    // publication so a reader failure can be isolated from writer behavior.
...
fn every_graph_read_effect_refuses_without_returning_a_schema() {
    ...
    let mut buffer = [0; MAX_BYTES];
    let mut schema_buffer = [0; schema::MAX_BYTES];
    // Each file requires pathname inspection, open, handle inspection and read.
    // The healthy case above fixes the combined catalog/schema count at eight.
    for cut in 0..8 {
...
fn schema_substitution_fails_even_when_parent_checksum_matches() {
    ...
    let mut bytes = [0; MAX_BYTES];
    // The catalog expects table 3 but points to table 29's schema. Its checksum
    // matches that file, so the table-identity check must reject the substitution.
    let root = encode(
        &mut bytes,
        database(),
        id(4, 1),
    ...
    let mut buffer = [0; schema::MAX_BYTES];
    let cancel = CancellationToken::new();
    assert!(matches!(
    ...
    // Now restore table 3's schema but keep the checksum for table 29. Identity
    // alone is insufficient: the catalog must also match the actual bytes.
    assert!(matches!(
        catalog.read_schema(&fixture.0, 0, &mut buffer, &cancel, &mut Effects::default()),
        Err(Error::Corrupt(_))
    ));
...
fn malformed_catalog_fields_fail_with_fresh_checksums() {
    ...
    // Change one field at a time and supply its new CRC. This forces the decoder
    // to reject the field itself rather than stop at a checksum mismatch.
    for (offset, value) in [
        (12, 65),
        (16, 8),
        (32, 5),
...
fn independent_catalog_and_schema_graph_matches_both_codecs() {
    let expected = include_bytes!("../../../test/data/catalog-schema/catalog.bin");
...
fn namespace_replacement_is_rejected_before_content_read() {
    ...
    // Use identical contents in a different file: only the identity comparison
    // can detect this replacement before reading bytes.
