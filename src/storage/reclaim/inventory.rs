//! Find files that exist in the directory but have no protected reference.
//!
//! Each object ID receives two independent marks: its name exists, and a retained
//! catalog or history references it. Both marks mean keep the file. A name alone
//! means it may be deleted. A reference alone means a required file is missing;
//! the entire inventory must fail before returning any deletion candidate.
//!
//! The inventory can exceed memory, so `scan` sorts fixed-size chunks into
//! scratch files and merges them. A run is one sorted sequence on disk. Each
//! merge combines at most eight runs, folding repeated references into one record.
//! Two scratch files alternate as input and output until one run remains.
//!
//! `finish` checks that complete run and records page checksums. `next` then
//! checks each reread page against those checksums before returning unreferenced
//! names. This module never deletes database files; its caller holds maintenance
//! admission and owns deletion. Dropping the inventory releases its scratch.

use super::{Maintenance, Reachable};
use crate::effects::{Effect, Effects};
use crate::error::io_error;
use crate::storage::catalog::{self, ObjectId};
use crate::storage::format::{Crc32c, crc32c};
use crate::storage::scratch::Scratch;
use crate::storage::snapshot::SLOTS;
...
const MAX_RECORDS: usize = crate::storage::recovery::MAX_CATALOG_OBJECTS + MAX_REFERENCES;
...
// Offsets and checksums refer to the currently selected scratch file. A merge
// replaces these descriptors only after consuming the corresponding input runs.
#[derive(Clone, Copy)]
struct Run {
    offset: u64,
    records: usize,
...
impl Cursor {
    ...
    // Check the run's complete checksum when exhausted, in addition to checking
    // each record's shape and strictly increasing ID as it is read.
    fn next(
        &mut self,
        scratch: &Scratch<'_>,
        slot: usize,
...
// Many retained views may reference one object. Two directory-name records for
// that object instead mean the enumeration or scratch contents are inconsistent.
...
pub(super) struct Inventory<'a, 'db> {
    ...
    merge_passes: usize,
    // Declared last so the buffers are freed before their reservation is released.
...
impl<'a, 'db> Inventory<'a, 'db> {
    ...
    // Finish enumeration and validation before the caller can delete anything.
    // Scratch writes use already-unlinked files, so they cannot add directory
    // entries to the object inventory being scanned.
    pub(super) fn scan(
        &mut self,
        cancel: &CancellationToken,
        effects: &mut Effects,
    ...
    fn scan_inner(
        ...
        let mut reachable = Reachable::open(self.writer)?;
        {
                ...
                .map_err(|e| io_error("read reclamation entry", e))?
            {
                ...
                if count == crate::storage::recovery::MAX_CATALOG_OBJECTS {
                    return Err(Error::Resource {
    ...
    fn merge(&mut self, cancel: &CancellationToken, effects: &mut Effects) -> Result<(), Error> {
        // Read from one file and write from offset zero in the other. Reusing
        // an input file here would overwrite records not yet consumed.
        ...
        for start in (0..self.runs.len()).step_by(FAN_IN) {
            ...
            // Hold one candidate per input run. Delay writing pending until a
            // different ID arrives, so equal IDs from separate runs can combine.
            // Every iteration consumes one record from an admitted finite run.
    ...
    fn finish(&mut self, cancel: &CancellationToken, effects: &mut Effects) -> Result<(), Error> {
        ...
        if let Some(run) = self.runs.first().copied() {
            ...
            // A missing protected file anywhere invalidates the scan, even if
            // an earlier record looks deletable. Save page checksums now so a
            // later scratch change cannot turn a protected name into garbage.
