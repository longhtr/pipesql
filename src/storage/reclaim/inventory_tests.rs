//! Check which names the production inventory offers for reclamation.
//!
//! Small committed databases have literal expected garbage sets, including a
//! retained old query. Larger cases supply records directly and compare results
//! with a BTreeSet built from a separate membership rule. They exercise external
//! merging without claiming those records form a valid public database.
//!
//! Missing references and corrupt scratch must prevent completion. Failure
//! injection, cancellation and resource refusal check that dropping an inventory
//! releases its buffers and scratch charges. These tests do not delete the
//! inventory's returned names; the reclamation cleanup tests cover that step.
...
fn garbage(
    ...
    for _ in 0..=crate::storage::recovery::MAX_CATALOG_OBJECTS {
...
fn external_merge_matches_independent_membership_and_detects_changed_output() {
    ...
    let mut inventory = Inventory::new(&writer, scratch(&directory), &mut effects).unwrap();
    // Exceed one merge's fan-in and protect every third object twice. Duplicate
    // references must collapse without turning a protected object into garbage.
    ...
    // Force a reread after changing the second output page. The saved page CRC,
    // not a checksum recomputed from the damaged file, must reject it.
...
fn maximum_input_uses_three_merge_passes_with_fixed_memory() {
    ...
    // Fill the input allowance with names, matching references, then repeated
    // references. Every name is protected, so no output is expected.
    for n in (1..=crate::storage::recovery::MAX_CATALOG_OBJECTS).rev() {
        inventory
            .push(id(n as u64, 1), false, &cancel, &mut effects)
            .unwrap();
    }
    for n in 1..=crate::storage::recovery::MAX_CATALOG_OBJECTS {
        inventory
            .push(id(n as u64, 1), true, &cancel, &mut effects)
            .unwrap();
    }
    for _ in 2 * crate::storage::recovery::MAX_CATALOG_OBJECTS..MAX_RECORDS {
        inventory
    ...
    assert_eq!(
        ...
        crate::storage::recovery::MAX_CATALOG_OBJECTS
    );
