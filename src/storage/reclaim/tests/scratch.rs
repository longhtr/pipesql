//! Check scratch-file lifetime alongside committed writes and old readers.
//!
//! Scratch construction briefly owns names that must be unlinked and synchronized
//! before payload writes begin. Failures during that setup can leave cleanup for
//! reopen. The tests distinguish this pending cleanup from live scratch extents,
//! whose bytes remain charged until their owner is dropped.
//!
//! Reentrant effect hooks and a separate thread exercise setup overlapping other
//! operations. Only another scratch setup is excluded. Recovery cases verify
//! that an unknown file or damaged committed graph cannot be treated as permission
//! to delete private files.
//! Worker failures, coordinator panics and delayed readiness check that the
//! worker exits before the test removes its files.
                ...
                && !captured.replace(true)
            {
                assert!(
                    shared
                        ...
                        .join(crate::storage::recovery::SCRATCH_NAMES[0])
                        .exists()
                );
                assert!(matches!(
...
fn scratch_extent_admission_and_failed_writes_keep_exact_shared_charge() {
    ...
    // The failed write reserved four bytes before I/O. Its empty file does not
    // justify releasing that allowance while the scratch owner remains alive.
...
fn scratch_constructor_failure_preserves_debt_until_exclusive_reopen() {
    ...
    for index in 0..effects.count() {
        ...
        assert_eq!(db.reserved_temp_bytes(), 0);
        assert_eq!(db.reserved_memory_bytes(), baseline);
        let pending = db.path().join(crate::storage::recovery::PRIVATE_NAME);
        ...
        // Pending scratch cleanup blocks another scratch setup, not catalog
        // publication or queries that do not need scratch.
        append(&db, 2);
        assert_eq!(
            query_values(&db, &db.prepare("FROM facts").unwrap()),
            [1, 2]
...
fn scratch_bootstrap_real_thread_excludes_only_another_bootstrap() {
    ...
    let fail_before_ready = case == "scratch-worker-failure";
    let delayed = case == "scratch-delayed-readiness";
    let directory = Directory::new();
    let db = database(&directory);
    append(&db, 1);
    let old = db.prepare("FROM facts").unwrap();
    std::thread::scope(|scope| {
        // Keep channels inside this closure so a panic drops them before the scope
        // waits for the worker. This releases a worker blocked on send or receive.
        let (entered_send, entered_receive) = sync_channel(0);
        let (release_send, release_receive) = sync_channel(0);
        let shared = &db;
        let worker = scope.spawn(move || {
            let mut first = true;
            let mut effects = Effects::with_faults(Faults {
                fail_at: fail_before_ready.then_some(0),
                after_action: Some(Box::new(move |_, effect| {
                    if first && effect == Effect::Load(crate::effects::LoadEffect::CreateStaging) {
                        first = false;
                        if delayed {
                            // Wait for the readiness timeout to disconnect release,
                            // so lateness is deterministic even on a busy host.
                            assert!(release_receive.recv().is_err());
                        }
        ...
        entered_receive
            ...
            .expect("scratch worker readiness");
        assert_ne!(
            case, "scratch-coordinator-panic",
            "scratch coordinator failure"
        );
        assert!(matches!(
...
#[test]
fn scratch_bootstrap_failure_controls_terminate() {
    for (case, diagnostic) in [
        (
            "scratch-worker-failure",
            "scratch worker readiness: Disconnected",
        ),
        ("scratch-coordinator-panic", "scratch coordinator failure"),
        (
            "scratch-delayed-readiness",
            "scratch worker readiness: Timeout",
        ),
    ] {
        crate::test_subprocess::run(
            concat!(
                module_path!(),
                "::scratch_bootstrap_real_thread_excludes_only_another_bootstrap"
            ),
            case,
            false,
            diagnostic,
