//! Remove obsolete files while preserving readers and transaction history.
//!
//! Publishing a new catalog leaves older immutable files in place. A prepared
//! query may still need their data, and a transaction lookup may still need their
//! history. Reclamation captures both kinds of protected view while holding
//! maintenance admission, which excludes writers and other reclamation.
//!
//! `Reachable` walks the protected references. The `inventory` child compares
//! them with directory entries and checks the entire result before offering a
//! name for deletion. Missing protected files therefore cause failure before
//! reclamation removes anything.
//!
//! Deletion can stop partway through on cancellation or I/O failure. Once an
//! unlink has been attempted, this module still synchronizes the directory;
//! if that synchronization fails, the database requires reopen. Success reports
//! the number of names whose removal was synchronized.

use super::snapshot::{Active, Maintenance, SLOTS};
...
impl Database {
    ...
    ///
    /// Readers and receipt lookup may continue, but a writer or another cleanup
    /// returns contention. Reclamation needs no free publication slot, issues no
    /// transaction and does not advance the generation. After an unlink, even
    /// cancellation completes the directory synchronization before returning.
    /// A failed synchronization requires recovery. Legacy databases are unsupported.
...
fn reclaim(
    ...
    let scratch = crate::storage::scratch::Scratch::new(owner.database, cancel, effects)?;
    ...
    let result = (|| {
        ...
        while let Some(id) = inventory.next(cancel, effects)? {
            ...
            cancel.check()?;
            // Mark before the effect hook: any attempted deletion requires
            // the barrier, including a failure reported before its outcome is known.
            dirty = Dirty::Units;
    ...
    match dirty {
        Dirty::None => result,
        Dirty::Units => {
            match crate::storage::recovery::sync_directory(&objects, effects, DirectoryKind::Units)
            {
                Ok(()) => result,
...
struct Reachable<'a, 'db> {
    // Maintenance excludes publication and other reclamation for the whole walk.
    // A reader dropping its pin can leave extra files protected, never too few.
    ...
    tables: usize,
    phase: Phase,
    index: Option<crate::storage::table::Cursor>,
    bytes: Vec<u8>,
    objects: PathBuf,
...
impl<'a, 'db> Reachable<'a, 'db> {
    ...
    fn open(writer: &'a Maintenance<'db>) -> Result<Self, Error> {
        ...
        let views = {
            // New readers select current, which is always protected. An old view
            // cannot acquire its first pin while publication is excluded.
            ...
            match self.phase {
                ...
                Phase::Index => {
                    ...
                    if let Some(reference) = table.data() {
                        self.index = crate::storage::table::open(
                            &self.objects,
                            self.writer.database.database_identity(),
                            table,
                            cancel,
