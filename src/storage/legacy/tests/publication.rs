//! Check that publication and recovery preserve transaction identities.
//!
//! Issuing a transaction reserves its number without committing data. These tests
//! interrupt that step and its recovery: an earlier committed load must survive,
//! and a later load must not reuse a number already issued.
//!
//! Recovery also needs enough trustworthy metadata to choose a state. The tests
//! build conflicting or damaged root/WAL records and check both decisions: repair
//! when the records authorize it, or refuse without changing files. Literal
//! expected outcomes and direct byte checks supplement the production decoder.

use crate::effects::{Effect, Effects, Faults, LoadEffect};
use crate::storage::format::{self, Replica, Root, RootState, WalRecord};
use crate::storage::legacy::staging::COLUMNS;
use crate::storage::legacy::tests::{
    TempDir, config, independent_crc32c, independent_namespace_matches, row,
};
...
fn issuance_after_data_uses_issuance_effects_and_preserves_reopened_data() {
    ...
    // The public legacy loader allows only one successful load. Call the shared
    // publisher directly to check issuance while a prior data snapshot exists.
...
fn interrupted_issuance_after_data_preserves_prior_commit_through_reopen() {
    ...
    let mut unknown = 0;
    let mut aborted = 0;
    // First record how many operations a successful issuance attempts. Then fail
    // each operation once, using a fresh database so earlier cuts cannot affect it.
    for selected in 0..128_u64 {
        ...
        // Direct publication bypassed the Database state update. Close this
        // handle before inspecting the result through ordinary open and recovery.
        database.close().unwrap();
        let reopened = Database::open(&root, limits).unwrap();
        assert_eq!(
            reopened.resolve_commit(committed.transaction()).unwrap(),
...
fn invalid_publication_transition_refuses_before_effects() {
    let database = crate::DatabaseId::new([7; 16]).unwrap();
    let prior = WalRecord {
        database,
        issued: 1,
    ...
    for record in [
        ...
        WalRecord {
            database: crate::DatabaseId::new([8; 16]).unwrap(),
            issued: 2,
            ..prior
        },
    ] {
...
fn publication_refuses_a_fence_alias_added_after_admission() {
    // A hard link outside the database would let a WAL write change that file
    // too. Add the alias just before each publication opens WAL, after earlier
    // namespace checks, and require the saved outside bytes to remain unchanged.
...
fn checksummed_unknown_root_fields_do_not_authorize_repair() {
    for (name, generation) in [
        ...
        // Keep the checksum valid so refusal must account for the unknown field,
        // rather than dismiss the record as an ordinary checksum failure.
...
fn root_fence_recovery_preserves_the_issued_prefix_or_refuses_without_cleanup() {
    // A always records attempt 1. Vary B and WAL; expected gives the highest
    // issued attempt recovery may retain, or None when it must leave files alone.
...
fn interrupted_prefix_repair_then_corruption_never_reuses_an_attempt() {
    fn fixture() -> (TempDir, PathBuf, crate::DatabaseId) {
        let temp = TempDir::new();
        let path = temp.0.join("database");
        let database = Database::create(&path, config(2_000_000, 1_000_000)).unwrap();
        let id = database.database_identity();
        database.close().unwrap();
        // Issuance stopped after replacing A: attempt 2 was recorded there and
        // in WAL, while B still records attempt 1.
