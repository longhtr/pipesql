//! Check load errors, cancellation and recovery at selected file operations.
//!
//! An effect is a named operation at which the test can intervene. A successful
//! load supplies the operation sequence; later runs fail an operation, shorten
//! an I/O request or cancel the load. The tests check the returned error, memory
//! and disk reservations, and the transaction outcome after reopening.
//!
//! Process-death cases stop a child after selected operations complete, then
//! reopen its database. They exercise recovery without normal stack cleanup.
//! The OS remains running, so these results do not establish power-loss behavior.
//! Root and WAL byte checks use the independent helpers in the parent module.
//! Intentional coordinator panics check that cleanup stops the child and waits
//! for it to exit before removing its input directory.

use crate::effects::{DirectoryKind, Effect, Effects, Faults, LoadEffect};
use crate::storage::legacy::tests::{
    LOAD_EFFECT_COUNT, TempDir, config, independent_namespace_matches, row,
};
use crate::storage::recovery::{ROOT_A_NAME, UNIT_NAME, UNITS_NAME};
...
fn ambiguous_publication_retains_reservation_until_recovery() {
    ...
    fs::write(&input, row("1", "100", "0.08", "1994-01-01")).unwrap();
    // Peak disk use: 28,672 metadata bytes, 38 bytes in staging and another 38
    // in the unit. Keep this expected amount independent of the layout function.
    ...
    for (case, cut) in cuts.into_iter().enumerate() {
        ...
        // Closing releases the lease without resolving the commit or deleting
        // its unit. An open that fails before recovery must leave that file intact.
        assert_eq!(
            fs::metadata(path.join(UNITS_NAME).join(UNIT_NAME))
                .unwrap()
                .len(),
...
fn short_load_io_is_either_progress_or_a_healable_typed_failure() {
    // A short input read is legal progress; a short fixed-size metadata transfer
    // is a failure. Hooks on operations without byte counts need not change them.
...
fn every_load_cleanup_failure_retains_debt_and_reopen_heals() {
    ...
    // Keep the original payload-write failure and add a second failure during
    // its rollback. A single fault in a successful load cannot exercise this path.
...
fn process_death_at_publication_boundaries_recovers() {
    ...
    if std::env::var_os(CHILD).is_some() {
        ...
        let target = std::env::var("PIPESQL_LOAD_CUT").unwrap();
        let target = if target == "first" {
            None
        } else {
            Some(target.parse::<u64>().unwrap())
        };
        let mut database = Database::open(&database_path, config(2_000_000, 1_000_000)).unwrap();
        let mut effects = Effects::with_faults(Faults {
            after_action: Some(Box::new(move |index, _| {
                if target.is_none_or(|target| index == target) {
    ...
    let cuts: Vec<(u64, u64, u64)> = observed
        ...
        .filter_map(|(index, effect)| {
            // These hooks run after completion. With the OS still alive, a
            // completed root rename is visible to reopen even before directory
            // sync. Do not use this expected outcome as a power-loss oracle.
            if *effect == Effect::Load(LoadEffect::RenameRootA) {
                generation = 1;
            }
...
// The guard owns the child before readiness polling or any assertion can panic.
// The caller retains the directory until the guard has stopped the child
// and waited for it to exit.
fn stop_load_child(
    database_path: &Path,
    input_path: &Path,
    marker: &Path,
    ...
    *pid = Some(child.0.id());
    assert_ne!(
        control,
        Some("before-ready"),
        "load coordinator before readiness"
    );
    ...
    assert_ne!(
        control,
        Some("after-ready"),
        "load coordinator after readiness"
    );
    // Kill and wait before asserting readiness. A timeout must not leave a
    // sleeping child holding the lease while the fixture directory is removed.
...
#[test]
fn publication_child_panic_stops_child_before_input_cleanup() {
    let Ok(case) = std::env::var(crate::test_subprocess::CASE) else {
        for case in ["before-ready", "after-ready"] {
            crate::test_subprocess::run(
                concat!(
                    module_path!(),
                    "::publication_child_panic_stops_child_before_input_cleanup"
                ),
                case,
                true,
                "load child exited before input cleanup",
            );
        }
        return;
    };
    assert!(matches!(case.as_str(), "before-ready" | "after-ready"));
    let temp = TempDir::new();
    let database_path = temp.0.join("database");
    let input = temp.0.join("lineitem.tbl");
    let marker = temp.0.join("paused");
    ...
    let outcome = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        stop_load_child(&database_path, &input, &marker, None, Some(&case), &mut pid);
    }));
    let panic = outcome.expect_err("coordinator must panic");
    let message = panic
        .downcast_ref::<String>()
        .expect("assertion diagnostic");
    assert!(message.contains(if case == "before-ready" {
        "load coordinator before readiness"
    } else {
        "load coordinator after readiness"
    }));
    // The process ID must no longer exist, and the database lease must be available
    // while the input directory is still present. The child has no descendants.
    assert!(
    ...
    Database::open(&database_path, config(2_000_000, 1_000_000))
        ...
        .close()
        .unwrap();
    println!("load child exited before input cleanup");
}
