//! Check the legacy loader from input validation through reopening its result.
//!
//! Healthy cases compare stored column bytes and block sizes with explicit
//! expected values, including empty input and values split across blocks. Other
//! cases change input or temporary files during loading to check that invalid
//! data cannot become a successful commit.
//!
//! Failures must also leave the right kind of usable state. Refusal before any
//! file changes allows retry; an interrupted repair or panic during mutation
//! makes the handle unusable until reopen. Tests inspect reservations and saved
//! files as well as the returned error, so an error alone cannot hide bad cleanup.

use crate::effects::{DirectoryKind, Effect, Effects, Faults, LoadEffect};
use crate::storage::format::{self};
use crate::storage::legacy::LOAD_MEMORY_BYTES;
use crate::storage::legacy::staging::COLUMNS;
use crate::storage::legacy::tests::{TempDir, config, independent_namespace_matches, row};
use crate::storage::recovery::recover_namespace;
use crate::storage::recovery::{
    PRIVATE_NAME, ROOT_A_NAME, ROOT_B_NAME, UNIT_NAME, UNITS_NAME, WAL_NAME,
};
        ...
        let mut effects = Effects::with_faults(Faults {
            action: Some(Box::new(move |_, effect| {
                if !changed && effect == Effect::Load(LoadEffect::OpenStaging) {
                    // The writer has saved its checksum, but assembly has not
                    // read this column yet. New unit checksums must not bless
                    // bytes damaged in that interval.
...
fn interrupted_load_admission_recovery_requires_reopen() {
    ...
    // Repeat the same missing-root repair, this time failing its directory sync.
    // The repaired name will be visible even though durability was not confirmed.
    fs::remove_file(path.join(ROOT_B_NAME)).unwrap();
...
fn changed_database_identity_refuses_before_load_cleanup() {
    ...
    Database::create(&foreign_path, config(2_000_000, 500_000_000))
        ...
        .unwrap();
    for name in [
        crate::storage::recovery::CONTROL_NAME,
        ROOT_A_NAME,
        ROOT_B_NAME,
    ] {
    ...
    let marker = path
        .join(PRIVATE_NAME)
        .join(crate::storage::recovery::PRIVATE_RECOVERY_NAMES[0]);
...
fn public_load_publishes_one_unit_and_reopens_generation_one() {
    ...
    // Read payload bytes at literal offsets rather than through the unit decoder.
    // Two distinct rows reveal whether values were stored by column or by row.
...
fn load_crosses_double_and_date_block_boundaries_exactly() {
    ...
    let mut descriptor_bytes = [0_u8; format::DESCRIPTOR_BYTES];
    file.read_exact_at(&mut header, 0).unwrap();
    file.read_exact_at(
        &mut descriptor_bytes,
        u64::try_from(format::HEADER_BYTES).unwrap(),
    )
    .unwrap();
    let (metadata, _) = format::decode_unit_metadata(&header, &descriptor_bytes).unwrap();
...
fn committed_unit_metadata_and_extent_mutations_obey_public_open_contract() {
    ...
    // Open validates metadata and total length, but defers data checksums until
    // a query demands the block. This payload-only mutation must still open.
...
fn input_identity_changes_are_detected_between_passes_and_after_build() {
    // Change the source in place, replace its inode, truncate it during pass two,
    // or alter it after assembly. All four paths must abort without retained files.
