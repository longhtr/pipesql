//! Read the legacy loader's input file and detect changes between observations.
//!
//! The loader needs two passes over the same data: one to reserve space and one
//! to write columns. `InputSource` records the original file's identity, size and
//! timestamps. Each pass checks the open file against that record before and
//! after reading; unit construction ends with another check of its pathname.
//!
//! `scan_pass` feeds chunks to the parser in `parser`. Both passes return
//! counts and checksums for comparison; the second also writes parsed rows to
//! `Staging`. Read errors, cancellation or a detected source change stop the load.
//! These checks do not lock the source against other writers. Keeping the file
//! unchanged remains a requirement on the caller.

use super::staging::Staging;
use crate::effects::{Effect, Effects, LoadEffect};
use crate::error::io_error;
use crate::path::validate_requested_path;
use crate::storage::legacy::parser::{InputError, MAX_CHUNK_BYTES, MAX_INPUT_BYTES, Scan, Scanner};
...
impl InputSource {
    // The second pass has closed its input handle. Check the pathname again
    // because replacement during unit assembly would otherwise go unobserved.
...
fn open_input(source: &InputSource, effects: &mut Effects) -> Result<File, Error> {
    ...
    // Check the file actually opened, not another lookup of its name: that name
    // could have been replaced since inspect_input recorded the original object.
