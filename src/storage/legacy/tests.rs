//! Shared fixtures and disk-state checks for the legacy loader tests.
//!
//! The child suites load real input, query the result and interrupt writes at
//! selected operations. This module supplies temporary directories and input
//! rows. Each case chooses its failures and expected outcome.
//!
//! After recovery, the disk-state helper checks root and WAL bytes directly,
//! using literal offsets and its own checksum calculation. A production writer
//! and decoder could share a bug, so their agreement alone is insufficient here.
//! This helper checks matching generations and expected directory contents for
//! an empty or once-loaded database; it is not a complete format validator.

use crate::Config;
use crate::storage::recovery::{PRIVATE_NAME, ROOT_A_NAME, ROOT_B_NAME, UNITS_NAME, WAL_NAME};
use std::fs;
use std::path::{Path, PathBuf};
use std::sync::atomic::{AtomicU64, Ordering};

static NEXT_TEMP: AtomicU64 = AtomicU64::new(1);
// Expected operation count for a one-row load, including metadata, padding and
// data readback. A changed count requires checking the fault schedule's coverage.
