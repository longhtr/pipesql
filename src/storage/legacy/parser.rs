//! Parse lineitem text into the seven columns stored by the legacy loader.
//!
//! Input reads can end in the middle of a row. `Scanner` keeps that unfinished
//! row in a fixed buffer and returns a `ProjectedRow` when it reaches a newline.
//! Every row has sixteen fields, each ending in `|`; fields 5 through 11 become
//! little-endian column values. The remaining fields are counted and checksummed
//! but are not stored for queries.
//!
//! Call `begin_chunk`, consume each byte in order, then call `finish` after the
//! last chunk. Invalid values, exceeded limits and an unfinished final row fail
//! the scan. Its summary includes checksums of both input text and column bytes,
//! allowing the loader to compare its two passes.
//!
//! This parser performs no I/O or heap allocation. `legacy::input` owns file reads,
//! source-change checks and cancellation. The grammar is specified in
//! `docs/cli.md#legacy-input`.

use crate::storage::format::Crc32c;
...
pub(super) struct Scanner {
    // The 512-byte row limit includes LF, which triggers parsing rather than
    // entering this buffer. Leave space for only the other 511 bytes.
...
impl Scanner {
    ...
    pub(super) fn finish(self) -> Result<Scan, InputError> {
        // begin_chunk hashes the whole read before consume validates its rows.
        // A summary is valid only after the caller has consumed all those bytes.
        assert_eq!(
            self.consumed_bytes, self.input_bytes,
...
fn parse_double(bytes: &[u8]) -> Result<f64, InputError> {
    // Rust's float parser accepts more spellings than this input format, such
    // as exponents and a leading plus. Check our grammar before converting.
...
fn parse_fixed_text(bytes: &[u8]) -> Result<u8, InputError> {
    match bytes {
        [byte] => crate::value::fixed_text::StringValue::from_byte(*byte)
            .map(crate::value::fixed_text::StringValue::byte)
...
fn parse_date(bytes: &[u8]) -> Result<i32, InputError> {
    crate::value::date::DateValue::parse(bytes)
        .map(crate::value::date::DateValue::days_since_unix_epoch)
...
mod tests {
    ...
    fn every_supported_calendar_day_matches_a_sequential_oracle() {
        // Count days from a literal starting value instead of repeating the
        // production date-to-day formula. Month lengths supply the next date.
