//! Read bounded UTF-8 input for query preparation and table declaration.
//!
//! Both commands need the same guarantee before interpreting text: a complete
//! regular file, within 4096 bytes, whose identity and metadata stayed unchanged
//! during reading. The caller supplies a fixed buffer with one extra byte to
//! detect growth. A successful read lends its UTF-8 contents and closes the file.
//!
//! Pathname and descriptor checks detect observed changes, not every concurrent
//! write. The caller must keep the source unchanged while it is read. Parsing,
//! database effects and output belong to the consuming command.
