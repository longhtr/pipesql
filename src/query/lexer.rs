//! Divide SQL text into tokens for the parser.
//!
//! For example, `qty + 1` becomes an identifier, a plus sign and a number. Each
//! token stores its kind and a byte range in the original text. The parser can
//! read a name or literal from that range and use it to locate an error, without
//! the lexer allocating a copy of the text.
//!
//! `lex` walks the source into a fixed token array, skipping ASCII whitespace
//! and `#` line comments. It checks spelling and size limits, including quoted
//! text through `text_literal`. A token does not establish that an operator is
//! supported or a column exists; parsing and binding make those decisions.
...
pub(super) fn lex(source: &str) -> Result<Tokens, Error> {
    ...
    while cursor < bytes.len() {
        ...
        if byte.is_ascii_digit() {
            // A leading sign is a separate token: the parser decides whether
            // '-' means negation or subtraction. An exponent sign stays here.
        ...
        if matches!(byte, b'\'' | b'"') {
            let (_, length) = crate::query::text_literal::TextLiteral::parse(&source[start..])
...
// Reservation prevents a word from being used as an unquoted name; it does not
// enable its SQL feature. The parser also recognizes nonreserved words by context.
