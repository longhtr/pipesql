...
//!
//! The first left column pairs with the first right column, regardless of names,
//! and so on. `SetPlan` records each pair's identities and type facts. Inputs
//! must have equal widths and matching types; no conversion is inserted here.
//!
//! Each output position gets its own new identity. For example, left `(a, a)`
//! can pair with right `(b, c)`: sharing the two output identities would wrongly
//! assert that those result values are always equal. Binding creates the mapping;
//! validation checks it against both original input relations.
//!
//! Execution decides which rows survive and how many copies to emit. UNION
//! DISTINCT uses a UNION ALL plan followed by the separate DISTINCT operator.
...
impl SetPlan {
    ...
    pub(crate) fn output(&self, position: usize) -> Option<SemanticColumn> {
        ...
        Some(SemanticColumn::new(
            identity,
            left.data_type(),
            // UNION can return either side's NULL; EXCEPT returns only left
            // values. INTERSECT can return NULL only when both sides allow it.
            match self.kind {
...
mod tests {
    ...
    fn set_binding_preserves_positions_and_rejects_corrupt_mappings() {
        ...
        for mode in [
            ...
            // The repeated left value pairs with two different right columns.
            // Outputs need distinct identities; only UNION can emit the right
            // side's nullable first column with this nonnullable left input.
