//! Give a LEFT JOIN's right-hand values new, nullable column identities.
//!
//! When a left row has no match, the join emits NULL for every right-hand value.
//! A right column declared nonnullable can therefore become nullable in the
//! result. `NullExtension` records that change under a new identity. The original
//! identity keeps its original type facts while the right input is evaluated.
//!
//! This mapping includes values still accessible through a qualified name, even
//! if a projection hid them from the ordinary output list. Otherwise a later
//! qualified reference could incorrectly treat a missing right value as non-NULL.
//! Binding creates the mapping and validation checks it against the right input;
//! `execution::blocking::join` owns row matching and emission.
...
pub(crate) struct NullExtension {
    // Inputs are unique and ordered by identity. Visible positions can repeat
    // an input, so preserve their order separately from this complete set.
...
mod tests {
    use super::*;
    use crate::query::{Stage, validate};
    ...
    fn nullable_join_identities_preserve_inputs_and_reject_corrupt_mappings() {
        ...
        for mutation in 0..13 {
            ...
            // Keep the original right input nonnullable. Only its replacement
            // is nullable; mutations must not let either identity escape its scope.
            match mutation {
                0 => (),
