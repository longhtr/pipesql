//! Assign column identities to the result of complete-row DISTINCT.
//!
//! DISTINCT removes duplicate rows by comparing every visible value. If a query
//! selects the same column twice, comparing it once is sufficient. For example,
//! `SELECT a, a AS copy |> DISTINCT` retains two output positions but gives them
//! the same new identity. The two names still refer to one value.
//!
//! `DistinctPlan` records the unique input columns and maps output positions to
//! them. Binding builds this mapping; validation checks it against the original
//! input relation. Row storage and duplicate removal belong to
//! `execution::blocking::order`.
...
pub(crate) struct DistinctPlan {
    ...
    // Each visible position indexes inputs. Its new identity is first + index,
    // allowing repeated projections to share both a comparison and a result.
...
impl DistinctPlan {
    ...
    pub(super) fn validate(
        ...
        for (position, id) in input.iter().enumerate() {
            // Derive duplicate positions from the original relation, rather
            // than trusting the grouping recorded by the binder.
...
mod tests {
    ...
    fn distinct_identity_order_and_descriptor_mutations() {
        ...
        for mutation in 0..11 {
            ...
            // Case zero is the healthy control. The mutations challenge the
            // identity range, repeated-column mapping, type facts and unused tail.
            match mutation {
                0 => (),
