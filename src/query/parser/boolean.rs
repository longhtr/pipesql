//! Compile a WHERE Boolean expression into forward branches between comparisons.
//!
//! For `a > 0 OR b > 0`, a true first comparison skips the second one. Each
//! comparison records where to go when it matches and when it does not: another
//! comparison, the next pipe stage, or rejection of the row. Execution follows
//! these branches without retaining or recursively walking a Boolean tree.
//!
//! Parsing first builds a temporary tree using fixed operator and value stacks.
//! AND binds more tightly than OR; NOT binds above both. A backward walk through
//! that tree then gives every comparison its forward destinations. Binding later
//! checks column names, types and constants.
//!
//! WHERE needs TRUE, while NOT asks its operand to be FALSE. UNKNOWN satisfies
//! neither request. Keeping the requested truth separate from the comparison
//! operator preserves NULL and NaN behavior when negating a predicate.
...
struct Node {
    kind: Operator,
    // Child indices refer to earlier nodes. `first` is the first parsed stage
    // in this subtree, where execution enters it.
    left: u8,
    right: u8,
    first: u8,
}
...
impl Syntax {
    ...
    fn finish(&mut self, parsed: &mut Parsed, span: SourceSpan) -> Result<(), Error> {
        ...
        // Parents were appended after their children. Walking backward sets each
        // child's truth request and destinations before that child is processed.
        ...
        for index in (0..self.count).rev() {
            ...
            match node.kind {
                ...
                Operator::And | Operator::Or => {
                    ...
                    // AND needs both operands TRUE; OR needs both FALSE under
                    // NOT. Otherwise the first match can decide the whole subtree.
...
impl Parser<'_> {
    ...
    fn boolean_leaf(
        ...
        if self.is_word("NOT") {
            ...
            // Negate the complete IN or BETWEEN result, just as prefix NOT does.
            // Reversing individual comparisons would change NULL and NaN behavior.
            syntax.operator(Operator::Not)?;
        }
        if self.is_word("IS") {
            self.word("IS")?;
            ...
            self.word("IN")?;
            self.take(Kind::LeftParen)?;
            // Expand IN to equality comparisons joined by OR. Each candidate
            // consumes one stage, keeping membership under the ordinary stage
            // limit and giving it the same short-circuit rules as explicit OR.
            loop {
                let literal = if self.is_word("NULL") {
                    self.word("NULL")?;
                    ParsedLiteral::Null
