//! Read quoted strings from SQL and store their decoded text in query plans.
//!
//! A token's written form differs from its value: `\n` occupies two input bytes
//! and decodes to a newline. `TextLiteral::parse` returns the value and the number
//! of input bytes consumed, including the quotes. The lexer uses that length
//! to find the next token; the binder keeps the value for execution.
//!
//! Values fit in a fixed byte array, so storing a constant does not require a
//! separate allocation or keep the original query text alive. Invalid tokens
//! return an explanation; the caller adds the token's position in the query.
...
impl TextLiteral {
    // Plan validation checks both the used text and the unused padding. Keeping
    // padding zero gives each value one byte representation, not arbitrary tails.
    ...
    /// Decode a token beginning at `source[0]` and report its length in bytes.
    /// The text between quotes and the decoded value must each fit in 32 bytes.
    /// Checking both limits prevents long escape sequences from hiding a large
    /// amount of input behind a short decoded string.
...
mod tests {
    ...
    fn pinned_escape_vectors_and_payload_bounds() {
        // Literal expectations follow GoogleSQL 0e7d707, public/strings.cc and
        // strings_test.cc. They are not calculated by the parser under test.
