//! Challenge the semantic validator with deliberately damaged prepared plans.
//!
//! Start with a query that prepares successfully, then change one part of its
//! plan and run validation again. Mutations include overlapping projection ranges,
//! unavailable column references and aggregate arguments with the wrong type.
//! The original plan is the healthy control; every damaged variant must fail.
//!
//! These tests exercise rejection of internal plans, not execution or SQL results.
//! Expected failures are chosen in each case rather than obtained from binding.
...
fn projection_ranges_are_complete_disjoint_and_bounded() {
    ...
    // Each SELECT must consume its own consecutive slice of projection storage.
    // Test overlap, gaps, out-of-bounds ranges and data beyond the used tail.
    for mutation in 0..7 {
...
fn semantic_plan_mutations_refuse() {
    ...
    for mutation in 0..14 {
        ...
        // AVG supplies the result and COUNT supplies the filter; SUM is unused.
        // Mutations must not revive original source columns after aggregation or
        // let the final output claim a different aggregate's identity.
        match mutation {
