//! Calculate and allocate the storage that a bound query plan will retain.
//!
//! Plans store expressions, aggregate arguments and column mappings. A direct
//! projection can reuse a column identity; a computation needs its own instructions.
//! `BindingBudget` counts those needs from the parsed query and includes the
//! plan, its vectors and the allowance for each allocation.
//!
//! The binder reserves that budget before allocating. A reservation is permission
//! to use memory, not a guarantee that the allocator can supply it. `allocate`
//! therefore checks both allocation failures and the resulting vector capacities.
//! On failure, its partial vectors drop before the caller releases the charge.
//!
//! Completed vectors move into the plan. The prepared query retains their charge.
//! Temporary name scopes have a separate owner in the parent module because they
//! are freed before preparation returns.
...
// The caller owns the reservation for these vectors. Free them before releasing
// that reservation, or transfer both to the prepared query.
...
impl BindingBudget {
    pub(super) fn calculate(parsed: &Parsed) -> Result<Self, Error> {
        ...
        // SET creates a replacement identity even for `SET a = b`. SELECT and
        // EXTEND can reuse a direct column, so only their expressions need storage.
    ...
    pub(super) fn allocate(&self) -> Result<Descriptors, Error> {
        ...
        // try_reserve_exact may still return extra capacity. Enforce the budget's
        // chosen slot count instead of silently retaining unaccounted slots.
