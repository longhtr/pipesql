//! Describe query values and stages independently of physical batch positions.
//!
//! Column identities survive renames and projections; computed values receive
//! new identities. The binder constructs these descriptors and validation checks
//! their relationships before execution can trust them. OwnedPlan holds the
//! fallibly allocated representation; PreparedQuery owns its reservation and pin.

use super::column_set::ColumnSet;
use super::distinct::DistinctPlan;
use super::join::NullExtension;
use super::lexer::reserved_identifier;
use super::scalar::{Expression, Op};
use super::set_operation::{SetKind, SetPlan};
use super::string_length::Unit as StringLengthUnit;
use crate::value::{DataType, DateValue};
use crate::{DatabaseId, Error, SourceSpan};
use std::mem::size_of;

pub(super) const MAX_SOURCE_BYTES: usize = 4096;
pub(super) const MAX_TOKENS: usize = 160;
...
pub(super) const MAX_PROJECTIONS: usize = MAX_TOKENS / 2;
pub(crate) const MAX_ORDER_ITEMS: usize = MAX_TOKENS / 2;
pub(super) const MAX_NAME_BYTES: usize = 32;
...
pub(super) const PREPARED_ALLOCATION_ALLOWANCE: usize = 4096;
...
pub(crate) const MAX_ROW_VALUES: usize = crate::batch::MAX_COLUMNS;
const _: () = assert!(MAX_ROW_VALUES == MAX_COLUMNS * 2);
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct ColumnId(pub(super) u32);
...
// A value's identity and type facts, without a name or storage location.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct SemanticColumn {
    pub(super) identity: ColumnId,
    pub(super) kind: DataType,
...
// Connect a query value to a stored column. Two occurrences of the same table
// share storage positions but receive different query identities, so a join can
// distinguish the left value from the right value.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct SourceColumn {
    pub(super) identity: ColumnId,
    pub(super) storage: u8,
    pub(super) kind: DataType,
    pub(super) nullable: bool,
}
...
pub(super) const CATALOG: [(SourceColumn, &str); 7] = [
...
pub(super) struct Name {
    pub(super) bytes: [u8; MAX_NAME_BYTES],
    pub(super) len: u8,
}
...
pub(crate) enum FilterLiteral {
    ...
    Int64(i64),
    Date(DateValue),
    String(crate::query::text_literal::TextLiteral),
}

impl FilterLiteral {
    pub(super) fn valid_for(self, data_type: DataType) -> bool {
        match self {
            Self::Null => true,
...
/// Branches between the comparisons in one WHERE expression.
/// Zero rejects the row; a positive offset skips forward from the current leaf.
/// `end` locates the continuation after this WHERE. `negated` requests a FALSE
/// predicate result; SQL UNKNOWN does not satisfy either truth requirement.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
pub(super) struct Group {
    pub(super) name: Name,
    pub(super) input: SemanticColumn,
}
...
pub(crate) struct AggregatePlan {
    ...
    pub(super) groups: [Group; MAX_AGGREGATE_COLUMNS - 1],
    pub(super) first_output: ColumnId,
...
impl AggregatePlan {
    ...
    pub(super) fn output(&self, index: usize) -> Option<Output> {
    ...
    #[cfg(test)]
    pub(super) fn needs(&self, column: SourceColumn, demand: u16) -> bool {
...
// Relation zero is the first source occurrence; stage i produces relation i + 1.
// Later sources have explicit Source stages. All real inputs point backward,
// so following dependencies cannot cycle.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
// Read a relation's visible row without storing a complete row description at
// every stage. get() follows inherited positions backward to their definitions.
#[derive(Clone, Copy)]
...
impl RelationColumns<'_> {
    ...
    pub(super) fn identity_set(self) -> Result<ColumnSet, Error> {
...
// One mention of a table in the query, with its own slice of source columns.
// Repeated mentions remain separate even when their persistent table IDs match.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct SourceOccurrence {
    pub(crate) table: Option<crate::storage::schema::TableId>,
    pub(super) start: u8,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(super) struct SetAssignment {
...
pub(crate) const MAX_PARTITION_KEYS: usize = 8;
pub(crate) const MAX_WINDOW_ORDER_KEYS: usize = 8;

/// Values that identify a window partition. An empty key list means the whole
/// input. Keep this inline with the computation; keys need no separate allocation.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct PartitionKeys {
    pub(super) keys: [Option<SemanticColumn>; MAX_PARTITION_KEYS],
}

impl PartitionKeys {
    pub(crate) const EMPTY: Self = Self {
        keys: [None; MAX_PARTITION_KEYS],
    };

    pub(crate) fn columns(&self) -> impl Iterator<Item = SemanticColumn> + Clone + '_ {
        self.keys.iter().flatten().copied()
    }

    pub(super) fn validate(&self, available: &[SemanticColumn]) -> Result<(), Error> {
        let mut ended = false;
        for key in self.keys {
            match key {
                ...
                Some(column)
                    if !ended
                        && column.data_type() != DataType::Double
                        && available.contains(&column) => {}
                Some(_) => return Err(Error::Corrupt("invalid window partition keys")),
            }
        }
        Ok(())
    }
...
/// An ordered SUM keeps one result per input row. Equal order keys are peers:
/// every row in that group receives the sum through the group's final row.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct RunningSum {
    pub(crate) argument: SemanticColumn,
    pub(crate) partition: PartitionKeys,
    pub(crate) order: [Option<WindowOrderKey>; MAX_WINDOW_ORDER_KEYS],
}

impl RunningSum {
    pub(super) fn validate(&self, available: &[SemanticColumn]) -> Result<(), Error> {
        if self.argument.data_type() != DataType::Int64 || !available.contains(&self.argument) {
            return Err(Error::Corrupt(
                "running SUM requires an available INT64 input",
            ));
        }
        self.partition.validate(available)?;
        if self.order[0].is_none() {
            return Err(Error::Corrupt("running SUM requires order keys"));
        }
        let mut ended = false;
        for key in self.order {
            match key {
                ...
                Some(key)
                    if !ended
                        && key.column.data_type() != DataType::Double
                        && available.contains(&key.column) => {}
                Some(_) => return Err(Error::Corrupt("invalid running SUM order key")),
            }
        }
        Ok(())
    }
...
pub(crate) enum Computation {
    ...
    Numeric(Expression),
    Constant(Constant),
    WindowCount(PartitionKeys),
    WindowSum(RunningSum),
}

impl Computation {
    pub(crate) fn is_analytic(&self) -> bool {
        matches!(self, Self::WindowCount(_) | Self::WindowSum(_))
    }

    pub(crate) fn columns(&self) -> impl Iterator<Item = SemanticColumn> {
        let (single, expression, partition, order) = match self {
            Self::Copy(column)
            | Self::StringLength { input: column, .. }
            | Self::DateYear(column) => (Some(*column), None, None, None),
            Self::Numeric(expression) => (None, Some(expression), None, None),
            Self::Constant(_) => (None, None, None, None),
            Self::WindowCount(keys) => (None, None, Some(keys), None),
            Self::WindowSum(sum) => (
                Some(sum.argument),
                None,
                Some(&sum.partition),
        ...
        single
            ...
            .chain(partition.into_iter().flat_map(PartitionKeys::columns))
            .chain(
                order
                    .into_iter()
                    .flat_map(|keys| keys.iter().flatten().map(|key| key.column)),
            )
    }
    ...
    pub(crate) fn numeric(&self) -> Result<&Expression, Error> {
        match self {
            ...
            | Self::Constant(_)
            | Self::WindowCount(_)
            | Self::WindowSum(_) => Err(Error::Corrupt(
    ...
    pub(super) fn data_type(&self) -> DataType {
        match self {
            ...
            | Self::DateYear(_)
            | Self::WindowCount(_)
            | Self::WindowSum(_) => DataType::Int64,
        }
    }
    ...
    pub(super) fn nullable(&self) -> bool {
        match self {
            ...
            Self::Numeric(expression) => expression.nullable(),
            Self::Constant(_) | Self::WindowCount(_) => false,
            Self::WindowSum(_) => true,
        }
    }
    ...
    pub(super) fn validate(&self, available: &[SemanticColumn]) -> Result<(), Error> {
        match self {
            ...
            Self::WindowCount(keys) => keys.validate(available),
            Self::WindowSum(sum) => sum.validate(available),
...
impl Computed {
    pub(super) fn allocation_capacity(count: usize) -> Option<usize> {
        // Reserve whole descriptor slots within the rounded buffer allowance.
        // Spare slots count toward memory admission, not the SQL expression limit.
        count
...
// Nodes refer into the descriptor pools below by index or contiguous range.
// The validator checks those references before execution can borrow the plan.
// Fixed arrays use explicit counts; unused capacity is not part of the query.
pub(crate) struct Plan {
    pub(crate) database: DatabaseId,
    pub(crate) generation: u64,
    pub(super) occurrences: [SourceOccurrence; MAX_STAGES],
    pub(super) occurrence_count: u8,
    pub(super) source_bytes: u16,
    pub(super) source_columns: [SourceColumn; MAX_COLUMNS],
    pub(super) catalog_columns: [u32; MAX_COLUMNS],
    pub(super) source_count: u8,
    pub(super) stages: [Node; MAX_STAGES],
    // Qualified names can retain original values after SET or DROP.
    // Keep that membership separately; it must not add public output positions.
    pub(super) range_columns: [ColumnSet; MAX_STAGES + 1],
    pub(super) assignments: [SetAssignment; MAX_PROJECTIONS],
    pub(super) assignment_count: u8,
    pub(super) count: u8,
    pub(super) projections: [ColumnId; MAX_PROJECTIONS],
    pub(super) projection_count: u8,
    pub(super) order_items: [OrderKey; MAX_ORDER_ITEMS],
    pub(super) order_count: u8,
    pub(super) outputs: [Output; MAX_COLUMNS],
    pub(super) output_count: u8,
...
// Keep the large Plan off the caller's stack. A one-element Vec permits fallible
// allocation; PreparedQuery drops it before releasing the memory reservation.
pub(crate) struct OwnedPlan {
    pub(super) values: Vec<Plan>,
}

impl OwnedPlan {
    pub(super) fn as_mut(&mut self) -> &mut Plan {
        &mut self.values[0]
    }

    pub(super) fn new(plan: Plan, limit: u64) -> Result<Self, Error> {
...
impl Plan {
    ...
    pub(crate) fn table(&self) -> Option<crate::storage::schema::TableId> {
        self.occurrences[0].table
    }

    pub(super) fn occurrence_column(&self, source: u8, index: usize) -> Option<ColumnId> {
    ...
    // Expressions may read visible outputs and retained qualified values.
    // Identity membership deliberately collapses repeated output positions.
    ...
    // An unused analytic result must not demand its argument or keys. The
    // complete projection has already been checked for one shared specification.
    pub(crate) fn demanded_analytic(
        &self,
        relation: RelationId,
        demanded: ColumnSet,
    ) -> Result<Option<&Computation>, Error> {
        let node = self.node(relation)?;
        let (Stage::Select { start, len } | Stage::Extend { start, len }) = node.stage else {
            return Err(Error::Corrupt("window outside a projection"));
        };
        let outputs = self
            .projections
            .get(usize::from(start)..usize::from(start) + usize::from(len))
            .ok_or(Error::Corrupt("window projection range"))?;
        Ok(self.computed.iter().find_map(|definition| {
            if definition.input == node.input
                && outputs.contains(&definition.column.identity())
                && demanded.contains(definition.column.identity())
                && definition.expression.is_analytic()
            {
    ...
    // SELECT can hide a sort key without disturbing row order. Follow stages
    // that preserve that order until its defining ORDER or ordered aggregate.
    // A join, set operation or other ordering boundary ends the search.
    ...
    pub(crate) fn aggregate_demand(&self, target: u8) -> u16 {
        // Start with result columns and work backward to the values they need.
        // Filters, sort keys and duplicate comparisons also demand values even
        // when those values are absent from the result. Keep this calculation
        // independent of execution's physical slot masks.
...
pub(super) struct ColumnFacts<'a> {
    pub(super) sources: &'a [SourceColumn],
    pub(super) aggregates: &'a [AggregatePlan],
    pub(super) computed: &'a [Computed],
    pub(super) distinct: &'a [DistinctPlan],
    pub(super) set_operations: &'a [SetPlan],
    pub(super) null_extensions: &'a [NullExtension],
}

impl ColumnFacts<'_> {
    pub(super) fn column(&self, id: ColumnId) -> Option<SemanticColumn> {
...
pub(super) fn source_column(id: ColumnId, sources: &[SourceColumn]) -> Option<SourceColumn> {
...
pub(super) fn initial_outputs() -> ([Output; MAX_COLUMNS], [SourceColumn; MAX_COLUMNS], u8) {
