//! Let tests intervene at named filesystem operations in the real engine.
//!
//! Callers pass an `Effects` value through operations such as loading, querying
//! and recovery. Before each instrumented operation, the caller checks whether
//! the test requests a failure or a short transfer. Production uses the default
//! schedule, which injects neither. Successful operations still use real files.
//!
//! Attempts are numbered from zero within that `Effects` value. A healthy run
//! provides the sequence; a later run can fail one attempt or both an operation
//! and its cleanup. Callbacks can also cancel work, change a fixture or pause a
//! child process. Callers explicitly mark the completion points used by tests
//! that interrupt execution after an operation has succeeded.
//!
//! These hooks cover operations selected by the engine, not every OS call.
//! Native failure campaigns separately observe calls at the filesystem interface.
//! The operation's caller decides the commit outcome and owns cleanup or recovery.
...
/// Test interventions for one sequence of attempted effects, numbered from zero.
#[cfg(test)]
#[derive(Default)]
pub(crate) struct Faults {
    pub(crate) fail_at: Option<u64>,
    /// Adds another refusal, usually during cleanup after the first failure.
    pub(crate) second_fail_at: Option<u64>,
    pub(crate) short_at: Option<u64>,
    /// Runs before the attempt is counted, even if that attempt will be refused.
    pub(crate) action: Option<Box<dyn FnMut(u64, Effect)>>,
    /// Runs only where the caller explicitly reports completion with `after`.
...
impl Effects {
    ...
    /// Count an attempt, including an injected refusal. Return an I/O error for
    /// refusal, `true` to request a short transfer, or `false` to proceed normally.
    /// The caller performs the operation and decides how to implement shortening.
    ...
    /// Report completion of the most recently counted attempt without adding a
    /// new attempt. The caller must place this after successful work and before
    /// another `before` call; `Effects` cannot infer completion from the OS result.
    ...
    pub(crate) fn read(
        ...
        // A real EOF means the fixed-size record on disk is incomplete. An
        // injected short transfer above keeps its I/O cause so tests can follow
        // that failure through the caller's cleanup and commit classification.
...
// Positional helpers preserve I/O errors; their callers decide whether a short
// file is corrupt. Buffers must be nonempty when a test shortens the transfer.
...
/// Write a construction record whose buffer the caller guarantees is nonempty.
/// A short-write fault writes all but its last byte, then returns `WriteZero`.
/// An empty buffer under that fault panics, exposing a broken caller precondition.
