//! Test database creation, reopening and exclusive access through real files.
//!
//! Opening takes an exclusive lock. A handle retains it until close or drop;
//! a failed open must release it. The lock alone does not authorize repairs:
//! tests replace directories, roots and links while opening to check that
//! recovery only changes files it owns.
//! Other cases damage known files, inject I/O failures and retry the operation.
//!
//! Most fixtures use the fixed-schema database format; catalog initialization
//! also runs in the `creation` child. That module checks why a fresh database
//! must match its intended empty state rather than merely be recoverable.
//! These tests exercise production recovery with simulated reported failures;
//! separate format fixtures and process-interruption campaigns provide other
//! kinds of evidence.
...
// Separate concurrent cases by process and counter. TempDir owns the parent,
// including renamed files that deliberately move outside its database child.
...
fn random_identity_validation_has_no_boxed_diagnostic() {
    // A zero identity must be rejected without allocating a custom I/O cause.
    // Each single nonzero byte is a control for preserving valid random input.
...
fn failed_creation_keeps_lease_until_cleanup_finishes() {
    ...
    // Attempt a competing write at the first cleanup deletion. If creation
    // released its lock early, cleanup could delete the peer's committed data.
...
fn stale_lease_cannot_recover_a_replacement_namespace() {
    ...
    // Rename the directory after the opener has obtained its LOCK descriptor.
    // A new database at the same path must not be cleaned through that old lock.
...
fn opened_fence_checks_link_ownership_again_before_recovery() {
    // Add the hard link after initial inspection, at either WAL open boundary.
    // Rechecking the opened file must prevent a repair from changing the canary.
    for point in [
...
fn reopen_retries_an_interrupted_repair_barrier() {
    ...
    // Rename has made ROOT.B visible, but the following directory sync failed.
    // A retry must synchronize again even though both root files now exist.
...
fn valid_foreign_root_is_not_repaired_as_replica_damage() {
    ...
    Database::create(&foreign, config())
        ...
        .close()
        .unwrap();
    // Both replacements have valid checksums: one belongs to another database,
    // the other is this database's B replica under the A name. Neither is damage
    // that recovery may overwrite using the remaining root.
...
fn fallible_path_join_matches_unix_geometry_and_bounds() {
    // Use the standard library as the path-joining oracle, including non-UTF-8
    // bytes. The separate boundary cases check our allocation limit.
    let parts: &[&[u8]] = &[
...
fn empty_root_recovery_repairs_missing_corrupt_and_stale_replicas() {
    ...
    database.close().unwrap();
    // These expected bytes use the production encoder. This checks recovery
    // selection and replacement, not independent correctness of the encoding.
    let expected_a = format::encode_root(format::Root {
        database: database_id,
        issued: 0,
        replica: format::Replica::A,
        state: format::RootState::Empty,
    })
    .unwrap();
    ...
    fs::write(path.join(WAL_NAME), [0_u8; format::WAL_BYTES]).unwrap();
    assert!(matches!(
        Database::open(&path, config()),
        Err(Error::Corrupt(_))
    ));
...
fn changed_opened_root_identity_is_not_repairable_damage() {
    ...
    let foreign = format::encode_root(format::Root {
...
fn wrong_root_extent_does_not_hide_recognizable_future_state() {
    // The first 12 bytes identify the format version. Truncation or extra bytes
    // must not turn an unsupported version into damage that recovery overwrites.
    for length in [
        12,
        format::ROOT_BYTES - 1,
        format::ROOT_BYTES,
...
fn trailing_root_bytes_do_not_hide_foreign_identity_or_generation() {
    for future_generation in [false, true] {
        ...
        database.close().unwrap();
        let mut bytes = format::encode_root(format::Root {
            issued: 0,
        ...
        if future_generation {
            ...
            let checksum = format::crc32c(&bytes);
...
fn namespace_symlinks_and_unsupported_version_fail_closed() {
    ...
    let mut old_control = [0_u8; format::CONTROL_BYTES];
    old_control[..8].copy_from_slice(b"PIPESQL\0");
    old_control[12] = format::CONTROL_BYTES as u8;
    ...
    fs::write(
        ...
        include_bytes!("../../test/data/rejected-single-table-format/CONTROL"),
    )
    .unwrap();
    assert!(matches!(
        Database::open(&path, config()),
    ...
    // Retained prototype CONTROL files must remain unsupported. Recovery must
    // not rewrite their files as though they were damaged supported databases.
    for (version, control) in [
        (
            3,
            include_bytes!("../../test/data/rejected-multi-table-format/CONTROL"),
        ),
        (
            5,
            include_bytes!("../../test/data/candidate-multi-table-format/CONTROL"),
        ),
    ] {
    ...
    let mut bytes = include_bytes!("../../test/data/current-single-table-format/CONTROL").to_vec();
    bytes[8..12].copy_from_slice(&3_u32.to_le_bytes());
...
fn control_mutations_lengths_and_unexpected_entries_fail_closed() {
    for offset in 0..format::CONTROL_BYTES {
        let temp = TempDir::new();
        let path = temp.database();
        Database::create(&path, config()).unwrap().close().unwrap();
    ...
    for length in 0..format::CONTROL_BYTES {
        let temp = TempDir::new();
        let path = temp.database();
        Database::create(&path, config()).unwrap().close().unwrap();
...
fn every_create_effect_refuses_without_silent_debt() {
    ...
    Database::create_with_effects(&baseline_path, config(), &mut baseline)
        ...
        .close()
        .unwrap();
    // This fixed count includes reading back the initial files and synchronizing
    // them. If it changes, account for the new or removed effect before updating
    // the count; creation must not silently acquire a recovery rewrite.
...
// Populate the fixed files and directories used by creation cleanup. Empty
// bytes are intentional: this checks deletion, not opening a valid database.
...
fn injected_cleanup_failure_is_reported() {
    ...
    // Pair creation failures with a later cleanup failure. This sweep needs
    // examples of both a reported cleanup error and files left for the caller;
    // it does not assert that every pair reaches the same cleanup operation.
...
fn bounded_short_control_io_fails_and_cleans_or_releases() {
    ...
    // Only byte transfers respond to the short-I/O hook. Nine creation effects
    // must fail; shortening inspection or synchronization has no effect.
...
fn pathname_scratch_case() {
    ...
    // Long symlink targets accumulate unresolved path components. The final
    // location is short, but resolving the chain needs more scratch than the
    // small budget permits. Refusal must precede database creation or repair.
