//! Check database path spelling and construct paths without unchecked growth.
//!
//! Validate the caller's path before asking the OS to resolve it. Require an
//! absolute path with a final component, no NUL or dot components, and at most
//! 4,096 bytes. Inspecting the spelling matters: normalization could otherwise
//! erase a forbidden component such as `..`. These checks do not prove that a
//! pathname still refers to the same file; native identity checks do that later.
//!
//! `try_join_path` allocates room for a complete joined path before copying its
//! components. `CanonicalizeScratch` supplies extra working space when the native
//! symlink walker exceeds its inline storage. Its growth holds reservations for
//! both old and new buffers during copying, preserving the old buffer on failure.
//! The filesystem crate limits traversal work; this module translates exhaustion
//! into a database resource error.
...
/// Extra space for native path traversal, charged to the database memory budget.
/// Paths that fit the walker's inline storage leave this buffer empty.
...
impl filesystem::PathScratch for CanonicalizeScratch<'_> {
    ...
    fn grow(&mut self, required: usize) -> Result<(), Error> {
        ...
        // Keep both reservations during copying. Free the old buffer before
        // replacing its reservation, so the released budget is truly available.
...
// Follow Unix joining rules: an absolute child replaces the parent. Allocate
// once for the complete result so either push cannot need unchecked growth.
// PathBuf::join would first clone the parent and could then allocate again.
...
pub(crate) fn validate_requested_path(path: &Path) -> Result<(), Error> {
    ...
    // Inspect bytes because Path::components would hide an interior `.`.
    // The caller's spelling must pass this rule before symlinks are resolved.
    if bytes
        .split(|byte| *byte == b'/')
...
mod tests {
    ...
    fn scratch_growth_admits_copy_overlap_and_preserves_state_on_refusal() {
        // Growth needs the 8-KiB old buffer and 16-KiB new buffer simultaneously,
        // each with a 4-KiB allowance. Test one byte below that 32-KiB peak as well
        // as an exact fit. Refusal must leave the traversal's saved bytes intact.
