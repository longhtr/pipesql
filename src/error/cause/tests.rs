//! Check that converting errors into causes keeps their original information.
//!
//! Compare fields and formatted messages before and after conversion, then follow
//! `std::error::Error::source` to check that callers can still inspect the cause.
//! These comparisons check preservation, not independently chosen message wording.
//!
//! The cleanup case combines two failures and retains the recovery version from
//! the first. The arithmetic case drops the query text before reading its error
//! position, checking that the cause owns its byte offsets.
...
fn arithmetic_cause_owns_the_original_source_range() {
    ...
    let cause =
        ErrorCause::from_error(crate::query::scalar::ArithmeticFailure::Add.into_error(span));
    drop(text);
    assert!(
