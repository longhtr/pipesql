//! Represent SQL dates and perform calendar arithmetic.
//!
//! `DateValue` stores a date as the number of days since 1970-01-01. Earlier dates
//! have negative numbers. This lets comparisons use integer ordering without
//! converting back to a year, month and day. Dates have no time of day or timezone.
//!
//! The constructors accept only dates in years 0001 through 9999. Parsing,
//! formatting and arithmetic all use the Gregorian calendar, including its leap
//! year rules. Invalid input or an arithmetic result outside the supported range
//! returns `None`.
...
impl DateValue {
    ...
    fn civil(year: i64, month: i64, day: i64) -> Option<Self> {
        ...
        // Start each arithmetic year in March, placing the leap day at its end.
        // Gregorian leap years repeat every 400 years (146097 days). Splitting
        // into these cycles avoids iterating through years or months. The final
        // offset makes 1970-01-01 day zero. Validated fields keep the arithmetic
    ...
    /// Add days, months or years. Month/year shifts keep the original day unless
    /// the destination month is shorter, then use its last day. For example,
    /// January 31 plus one month becomes February 28, or 29 in a leap year.
