    ...
    for populated in [false, true] {
        ...
        for fault in 0..events.len() {
            ...
            if std::fs::read_dir(&private).unwrap().next().is_some() {
                ...
                assert!(matches!(
                    crate::namespace::inspect_namespace(db.path(), &db.memory, &mut inspect),
...
fn legacy_scratch_reopen_rejects_unowned_debris_before_cleanup() {
    for populated in [false, true] {
        for invalid in [
            ...
            if invalid == "alias" {
                std::fs::hard_link(&path, private.join(crate::namespace::SCRATCH_NAMES[1]))
                    .unwrap();
            }
            if invalid == "corrupt-source" {
                let control = directory.database().join(crate::namespace::CONTROL_NAME);
