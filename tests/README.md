...
| --- | --- |
| `append.rs`, `snapshots.rs` | Append failure/drop, date admission, pinned generations, two threaded readers across publication/reclamation and early drop, composed report spill/cancellation schedules, shared-budget refusal, a pinned-catalog unlink control, reopen, and receipts. |
...
| `computed.rs`, `constant_projection.rs`, `boolean.rs`, `membership.rs`, `text_filter.rs`, `null_predicate.rs` | SELECT/EXTEND expression demand and scalar/predicate semantics; COALESCE covers nullable defaults and skipped/demanded dependency errors; DIV/MOD cover exact signed quotients/remainders, integer extremes, NULLs and integer-only binding; FLOOR/CEIL/CEILING/ROUND cover DOUBLE promotion, conversion boundaries, fractional and halfway rounding, demanded errors and buckets; SQRT covers DOUBLE results, NULLs, skipped/demanded negative-domain errors and owned spans; LN and LOG10 cover promotion, independent approximate answers, finite domain errors, negative-infinity NaN, demand and composition; LOG10 also checks a complete power-ratio scale; EXP covers approximate scalar/geometric answers, exact exceptional values, underflow boundaries, demanded overflow spans and composition; POW/POWER cover represented exponent parity, exact exceptional values, independent finite and compounded answers, domain/overflow spans and binary program bounds; shared numeric tests cover stored bits and reopen; SIGN covers signed classification, zero/NaN bits, stored-value composition and reopen; ABS covers type preservation, minimum-integer overflow, nesting and deviations; division and SAFE_DIVIDE cover coercion, precedence, NULL/zero/overflow, argument errors, typed NULL constants, Boolean demand, grouped ratios, cancellation and cleanup; typed constants cover source lifetime, malformed unused values, full widths and composition; membership has an independent nullable-set model; infix NOT IN and NOT BETWEEN cover typed results, NULL/NaN negation, precedence, rejected forms and demanded errors. |
| `cast.rs` | Explicit numeric conversion, literal precision-boundary bit oracles, nullable schema, stored DOUBLE bits, nested calls, typed identities, grouping collisions, wrong-target/type spans, input-overflow order, conditional demand, cancellation and abandonment. |
| `date_year.rs` | Literal Gregorian boundaries, DATE preservation, constant ownership, nullable INT64 identities, composition, demanded error spans, rejection, cancellation, abandonment and logical-plan rendering. |
| `logical_plan.rs` | Literal public logical-plan reports, relation and identity distinctions, two-input plans, scalar definitions, formatting failure, reservation release and retained snapshots. |
...
| `byte_length.rs`, `char_length.rs` | STRING-to-INT64 projections, independent UTF-8 byte/scalar counts, combining and joined characters, empty/maximal/NULL values, literal ownership, aliases, conditional dependencies, typed materialization, rejected forms, cancellation, abandonment and reopen. |
